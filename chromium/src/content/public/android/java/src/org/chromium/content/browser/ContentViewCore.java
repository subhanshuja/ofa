...
public class ContentViewCore implements AccessibilityStateChangeListener, DisplayAndroidObserver,
    ...
    /**
     * Suspend all audio element playback.
     */
    public void suspendAudio() {
        if (mNativeContentViewCore != 0) {
            nativeSuspendAudio(mNativeContentViewCore);
        }
    }

    /**
     * Resume suspended audio element playback.
     */
    public void resumeSuspendedAudio() {
        if (mNativeContentViewCore != 0) {
            nativeResumeSuspendedAudio(mNativeContentViewCore);
        }
    }
    ...
    @CalledByNative
    private void onTouchHandleEvent(int eventType, Rect boundsRectDip) {
        Rect boundsRect = mRenderCoordinates.fromDipToPix(boundsRectDip);
        getContentViewClient().onTouchHandleEvent(eventType, boundsRect);
    }
    ...
    @SuppressWarnings("unused")
    @CalledByNative
    private void showSelectionPreview(Bitmap previewBitmap, Rect focusRectDip) {
        Rect focusRect = mRenderCoordinates.fromDipToPix(focusRectDip);
        getContentViewClient().onShowSelectionPreview(previewBitmap, focusRect);
    }
    ...
    /**
     * Request a selection preview for a specific focus point.
     * @param focusRect Selection focus point in screen coordinates.
     * @param bitmapWidth Preview bitmap width.
     * @param bitmapHeight Preview bitmap height.
     * @param scaleThreshold Preview scale factor threshold. No preview will
     *                       be generated if the scale (zoom factor) of the
     *                       preview bitmap doesn't meet this threshold.
     */
    public void requestSelectionPreview(Rect focusRect, int bitmapWidth, int bitmapHeight,
            float scaleThreshold) {
        Rect focusRectDip = mRenderCoordinates.fromPixToDip(focusRect);

        nativeRequestSelectionPreview(mNativeContentViewCore, focusRectDip.left, focusRectDip.top,
                focusRectDip.width(), focusRectDip.height(), bitmapWidth, bitmapHeight,
                scaleThreshold);
    }
    ...
    private native void nativeRequestSelectionPreview(long nativeContentViewCoreImpl,
            int focusX, int focusY, int focusWidth, int focusHeight,
            int previewWidth, int previewHeight, float scaleThreshold);
    ...
    private native void nativeSuspendAudio(long nativeContentViewCoreImpl);
    private native void nativeResumeSuspendedAudio(long nativeContentViewCoreImpl);
