...
public class RenderCoordinates {
    ...
    /**
     * @return Physical on-screen rectangle converted to local DIP coordinates.
     */
    public Rect fromPixToDip(Rect rect) {
        return new Rect(Math.round(fromPixToDip(rect.left)), Math.round(fromPixToDip(rect.top)),
                Math.round(fromPixToDip(rect.right)), Math.round(fromPixToDip(rect.bottom)));
    }

    /**
     * @return Local DIP rectangle converted to physical coordinates.
     */
    public Rect fromDipToPix(Rect rect) {
        return new Rect(Math.round(fromDipToPix(rect.left)), Math.round(fromDipToPix(rect.top)),
                Math.round(fromDipToPix(rect.right)), Math.round(fromDipToPix(rect.bottom)));
    }
}
