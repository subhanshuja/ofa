...
class CONTENT_EXPORT ContentClient {
 public:
  ...
  // Returns the user agent override.  Content may cache this value.
  // When a NavigationEntry requires the user agent to be overridden,
  // this user agent string is used instead of
  // GetUserAgent(). Currently, the only use case we have for
  // overriding a user agent involves spoofing a desktop Linux user
  // agent for "Request desktop site".
  virtual std::string GetUserAgentOverride() const;
