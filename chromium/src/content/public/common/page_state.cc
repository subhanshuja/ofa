...
void RecursivelyReplaceLegacyOperaScheme(ExplodedFrameState* state) {
  if (!state->url_string.is_null()) {
    GURL url(state->url_string.string());
    if (url.SchemeIs(content::kLegacyOperaUIScheme)) {
      ReplaceLegacyOperaScheme(&url);
      state->url_string = base::NullableString16(
          base::UTF8ToUTF16(url.possibly_invalid_spec()), false);
    }
  }
  for (auto& child : state->children)
    RecursivelyReplaceLegacyOperaScheme(&child);
}
...
#if defined(OPERA_DESKTOP) || defined(OPERA_TVSDK) || defined(OPERA_WAM)
PageState PageState::ReplaceLegacyOperaScheme() const {
  if (data_.empty())
    return *this;
  ...
  RecursivelyReplaceLegacyOperaScheme(&state.top);
