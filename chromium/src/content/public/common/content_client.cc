...
std::string ContentClient::GetUserAgentOverride() const {
  // Spoof a desktop Linux user agent, i.e. return what
  // BuildUserAgentFromProduct() would return for a desktop Linux
  // build.
  const char kLinuxInfoStr[] = "X11; Linux x86_64";
  std::string product = GetProduct();
  return BuildUserAgentFromOSAndProduct(kLinuxInfoStr, product);
}
