...
class PageStateSerializationTest : public testing::Test {
 public:
  ...
  void PopulateFrameStateForBackwardsCompatTest(
    ...
    frame_state->text_wrap_width = 130;
...
// NOTE(wmaslowski): Putting a note here so there is a merge conflict if
// chromium adds any more tests. If that happens then we need to update
// the data files in content/test/date/page_state/. See patch_dat_file.py
// in that file for more info.
