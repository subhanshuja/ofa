...
IPC_ENUM_TRAITS(blink::WebAudioElementPlayState)
...
// Suspend and resume audio playback.
IPC_MESSAGE_ROUTED0(ViewMsg_SuspendAudio)
IPC_MESSAGE_ROUTED0(ViewMsg_ResumeSuspendedAudio)
...
// Notifies the browser about audio element play state changes.
// This is an aggregate state per view.
IPC_MESSAGE_ROUTED1(ViewHostMsg_UpdatePlayState,
...
// Notification that the selection state have changed.
IPC_MESSAGE_ROUTED1(ViewHostMsg_SelectionStateChanged,
                    bool /* is_anchor_first */)

#if defined(OS_ANDROID)
// Notification that a new selection preview bitmap is ready to be shown.
// |bitmap_size| is the bitmap size in physical pixels. |bitmap_id| is the
// transportable preview bitmap. |focus_rect| is the bounding rectangle for the
// selection focus point in window coordinates.
IPC_MESSAGE_ROUTED3(ViewHostMsg_ShowSelectionPreview,
                    gfx::Size /* bitmap_size */,
                    cc::SharedBitmapId /* bitmap_id */,
                    gfx::Rect /* focus_rect */)

// An acknowledge that the selection preview bitmap has been received and
// processed. This will cause the view to release the bitmap.
IPC_MESSAGE_ROUTED0(ViewMsg_ShowSelectionPreview_ACK)

// Request a new selection preview bitmap. |focus_rect| is the bounding
// rectangle for the selection focus point that a preview is requested for.
// It's given in window coordinates. |preview_size| is the requested bitmap
// size measured in physical pixels. |scale_threshold| specifies the minimum
// scale factor that's required in order to generate a preview bitmap.
IPC_MESSAGE_ROUTED3(ViewMsg_RequestSelectionPreview,
                    gfx::Rect /* focus_rect */,
