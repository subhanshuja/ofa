...
cc::LayerTreeSettings RenderWidgetCompositor::GenerateLayerTreeSettings(
  ...
  if (cmd.HasSwitch(cc::switches::kTileCompressionHeuristic)) {
    int heuristic;
    if (GetSwitchValueAsInt(cmd, cc::switches::kTileCompressionHeuristic, 0,
                            std::numeric_limits<int>::max(), &heuristic)) {
      settings.renderer_settings.tile_compression_heuristic_flags = heuristic;
    }
  }
  ...
  settings.initial_debug_state.log_fps_counter =
      cmd.HasSwitch(cc::switches::kLogFPSCounter);

  // These flags should be mirrored by UI versions in ui/compositor/.
  settings.initial_debug_state.show_debug_borders =
      cmd.HasSwitch(cc::switches::kShowCompositedLayerBorders);
  settings.initial_debug_state.log_tile_memory_usage =
      cmd.HasSwitch(cc::switches::kLogTileMemoryUsage);
  ...
  if (use_low_memory_policy) {
    ...
    if (!cmd.HasSwitch(switches::kDisableRGBA4444Textures)) {
      ...
      settings.renderer_settings.preferred_tile_format_with_alpha =
          cc::RGBA_4444;
    }
  } else {
  ...
  if (cmd.HasSwitch(switches::kEnableRGBA4444Textures) &&
    ...
    settings.renderer_settings.preferred_tile_format = cc::RGBA_4444;
    settings.renderer_settings.preferred_tile_format_with_alpha = cc::RGBA_4444;
  }

  if (cmd.HasSwitch(cc::switches::kEnableTileCompression)) {
    // TODO(peterp): ETC1 is too slow for real time usage.
    // settings.renderer_settings.preferred_tile_format = cc::ETC1;

    const auto& caps = compositor_deps->GetSharedMainThreadContextProvider()
        ->ContextCapabilities();
    if (caps.texture_format_atc) {
      settings.renderer_settings.preferred_tile_format = cc::ATC;
      settings.renderer_settings.preferred_tile_format_with_alpha = cc::ATC_IA;
    }
    if (caps.texture_format_dxt1)
      settings.renderer_settings.preferred_tile_format = cc::DXT1;
    if (caps.texture_format_dxt5)
      settings.renderer_settings.preferred_tile_format_with_alpha = cc::DXT5;
  }
...
void RenderWidgetCompositor::setEnableTextWrap(bool enableTextWrap) {
  layer_tree_host_->SetEnableTextWrap(enableTextWrap);
}
...
void RenderWidgetCompositor::SetTextWrapInterestPoint(
    const gfx::Point& interest_point) {
  delegate_->SetTextWrapInterestPoint(interest_point);
}

void RenderWidgetCompositor::StartTextWrap() {
  delegate_->StartTextWrap();
}
