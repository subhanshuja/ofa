...
bool MediaInfoLoader::willFollowRedirect(
  ...
  std::string x_opera_video_redirect =
      redirectResponse.httpHeaderField("x-opera-video-redirect").utf8();
  if (!x_opera_video_redirect.empty()) {
    compressed_video_url_ = GURL(x_opera_video_redirect);
    // For compressed video redirects, we check
    // single origin when the load is finished.
    return true;
  }
...
void MediaInfoLoader::didReceiveResponse(
  ...
  if (response.httpStatusCode() == kHttpOK ||
    ...
    if (url_.SchemeIsFile()) {
      std::string x_opera_video_redirect =
          response.httpHeaderField("x-opera-video-redirect").utf8();
      if (!x_opera_video_redirect.empty()) {
        compressed_video_url_ = GURL(x_opera_video_redirect);
      }
      // In case the video could not be optimized after all
      // and was redirected back to original url, mark this as single_origin.
      if (compressed_video_url_.is_valid() && single_origin_) {
        single_origin_ = compressed_video_url_.GetOrigin() == url_.GetOrigin();
      }
    }
    DidBecomeReady(kOk);
    return;
...
GURL MediaInfoLoader::GetCompressedVideoUrl() const {
  return compressed_video_url_;
}
