...
WebMediaPlayerAndroid::WebMediaPlayerAndroid(
    ...
    : frame_(frame),
      ...
      video_compression_enabled_(false),
...
void WebMediaPlayerAndroid::DoLoad(LoadType load_type,
  ...
  if (render_view) {
    ...
    video_compression_enabled_ = webkit_preferences.video_compression_enabled;
  }
...
void WebMediaPlayerAndroid::DidLoadMediaInfo(
  ...
  if (video_compression_enabled_ && player_type_ == MEDIA_PLAYER_TYPE_URL) {
    // We must initialize the player with the new URL so that the native
    // player is handed the direct URL to the video compressor service.
    if (info_loader_->GetCompressedVideoUrl().is_valid()) {
      redirected_url_ = info_loader_->GetCompressedVideoUrl();
    }
  }

  InitializePlayer(redirected_url_, first_party_for_cookies,
