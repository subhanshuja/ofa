...
void RenderView::ApplyWebPreferences(const WebPreferences& prefs,
  ...
  settings->setTextWrappingEnabled(prefs.text_wrapping_enabled);
...
bool RenderViewImpl::OnMessageReceived(const IPC::Message& message) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderViewImpl, message)
    ...
    IPC_MESSAGE_HANDLER(ViewMsg_SuspendAudio, OnSuspendAudio)
    IPC_MESSAGE_HANDLER(ViewMsg_ResumeSuspendedAudio, OnResumeSuspendedAudio)
    ...
    IPC_MESSAGE_HANDLER(ViewMsg_RequestSelectionPreview,
                        OnRequestSelectionPreview)
    IPC_MESSAGE_HANDLER(ViewMsg_ShowSelectionPreview_ACK,
                        OnShowSelectionPreviewComplete)
#elif defined(OS_MACOSX)
...
void RenderViewImpl::OnSuspendAudio() {
  webview()->suspendAudio();
}

void RenderViewImpl::OnResumeSuspendedAudio() {
  webview()->resumeSuspendedAudio();
}
...
void RenderViewImpl::playStateChanged(
    blink::WebAudioElementPlayState play_state) {
  Send(new ViewHostMsg_UpdatePlayState(routing_id_, play_state));
}
...
#if defined(OS_ANDROID)
void RenderViewImpl::OnRequestSelectionPreview(const gfx::Rect& focus_rect,
                                               const gfx::Size& preview_size,
                                               float scale_threshold) {
  DCHECK(!preview_size.IsEmpty());
  if (focus_rect.height() == 0)
    return;

  // Check if there is already a bitmap in the processing.
  if (!current_selection_preview_bitmap_) {
    ShowSelectionPreview(focus_rect, preview_size, scale_threshold);
  } else {
    pending_selection_preview_.focus_rect = focus_rect;
    pending_selection_preview_.size = preview_size;
    pending_selection_preview_.scale_threshold = scale_threshold;
  }
}

void RenderViewImpl::OnShowSelectionPreviewComplete() {
  if (current_selection_preview_bitmap_)
    current_selection_preview_bitmap_.reset();

  if (pending_selection_preview_.focus_rect != gfx::Rect()) {
    ShowSelectionPreview(
        pending_selection_preview_.focus_rect,
        pending_selection_preview_.size,
        pending_selection_preview_.scale_threshold);
    pending_selection_preview_.focus_rect = gfx::Rect();
  }
}

gfx::Size RenderViewImpl::CreateZoomedBitmap(
    const gfx::Rect& bounds_rect,
    float scale,
    std::unique_ptr<cc::SharedBitmap>* shared_bitmap) {
    gfx::Size canvas_size =
        gfx::ScaleToCeiledSize(bounds_rect.size(), scale* device_scale_factor_);

  // Get a canvas that draws into shared memory.
  cc::SharedBitmapManager* manager =
      RenderThreadImpl::current()->shared_bitmap_manager();
  *shared_bitmap = manager->AllocateSharedBitmap(canvas_size);
  if (!(*shared_bitmap))
    return gfx::Size();
  ...
  canvas.translate(-bounds_rect.x() * device_scale_factor_,
                   -bounds_rect.y() * device_scale_factor_);

  // Fill the bitmap with the web widget's background color, this is useful in
  // case bounds_rect extends beyond what the web widget can paint.
  canvas.drawColor(webview_->backgroundColor());

  DCHECK(webview_->isAcceleratedCompositingActive());
  // TODO(aelias): The disambiguation popup should be composited so we
  // don't have to call this method.
  webview_->paintIgnoringCompositing(&canvas, bounds_rect);

  return canvas_size;
}

void RenderViewImpl::ShowSelectionPreview(
    const gfx::Rect& focus_rect,
    const gfx::Size& preview_size,
    float scale_treshold) {
  gfx::SizeF dip_preview_size =
      gfx::ScaleSize(gfx::SizeF(preview_size), 1.0 / device_scale_factor_);
  gfx::RectF client_focus_rect =
      gfx::ScaleRect(gfx::RectF(focus_rect), 1.0 / webview()->pageScaleFactor());

  blink::WebFloatPoint viewportOffset(webview()->visualViewportOffset());
  client_focus_rect += gfx::Vector2d(viewportOffset.x, viewportOffset.y);

  float scale = dip_preview_size.height() / client_focus_rect.height();

  // Don't create a preview bitmap if the relative scale (scale of preview
  // content compared to the current page scale) doesn't meet the threshold.
  float relative_scale = scale / webview()->pageScaleFactor();
  if (relative_scale < scale_treshold)
    return;

  float client_width = dip_preview_size.width() / scale;
  float client_height = dip_preview_size.height() / scale;

  gfx::Rect client_rect(client_focus_rect.x() - client_width / 2,
                        client_focus_rect.y(), client_width, client_height);

  gfx::Size bitmap_size = CreateZoomedBitmap(
      client_rect, scale, &current_selection_preview_bitmap_);
  if (current_selection_preview_bitmap_) {
    Send(new ViewHostMsg_ShowSelectionPreview(
        routing_id_, bitmap_size, current_selection_preview_bitmap_->id(),
        focus_rect));
  }
}
#endif
