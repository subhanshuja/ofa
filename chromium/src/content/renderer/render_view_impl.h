...
class CONTENT_EXPORT RenderViewImpl
 ...
 public:
  ...
  void playStateChanged(blink::WebAudioElementPlayState play_state) override;
 ...
 private:
  ...
  void OnRequestSelectionPreview(const gfx::Rect& bounds_rect,
                                 const gfx::Size& preview_size,
                                 float scale_threshold);
  void OnShowSelectionPreviewComplete();
...
#if defined(OS_ANDROID)
  // Paints the widget into a bitmap. |bounds_rect| bounding rectangle in
  // client coordinates to make a bitmap of. |scale| scale that the
  // |bounds_rect| content should be rendered in. |shared_bitmap| shared
  // preview bitmap. On success the function returns the size of the rendered
  // bitmap and updates the |shared_bitmap| pointer. On failure the function
  // returns an empty size and sets the |shared_bitmap| reset to NULL.
  gfx::Size CreateZoomedBitmap(const gfx::Rect& bounds_rect, float scale,
                               std::unique_ptr<cc::SharedBitmap>* shared_bitmap);

  // Creates a selection preview bitmap of the context surrounding the
  // selection focus point and sends it to the browser process in order to
  // display it. |focus_rect| is the bounding rectangle for the selection focus
  // point in window coordinates. |preview_size| is the requested bitmap size
  // measured in physical pixels. |scale_threshold| specifies the minimum scale
  // factor that's required in order to generate a preview bitmap.
  void ShowSelectionPreview(const gfx::Rect& focus_rect,
                            const gfx::Size& preview_size,
                            float scale_threshold);
#endif
...
#if defined(OS_ANDROID)
  // Selection preview bitmap which is currently being processed.
  std::unique_ptr<cc::SharedBitmap> current_selection_preview_bitmap_;

  // Input parameters for the pending selection preview.
  struct {
    // Selection focus point bounds.
    gfx::Rect focus_rect;

    // Selection preview bitmap size.
    gfx::Size size;

    // Scale treshold for creating a bitmap.
    float scale_threshold;
  } pending_selection_preview_;
#endif
