...
void SendCookieChangedOnIO(net::URLRequestContext* context,
                           const SendSetCookieData& data) {
  net::HttpTransactionFactory* http_transaction_factory =
      context->http_transaction_factory();
  // NOTE(msimonides): not all URLRequestContexts have an
  // http_transaction_factory, e.g. the one used by extensions doesn't.
  if (http_transaction_factory) {
    net::HttpNetworkSession* network_session =
        http_transaction_factory->GetSession();
    network_session->turbo_session_pool()
        ->turbo_cookie_synchronizer()
        ->OnCookieChanged(data.cookie_store, data.url, data.cookie_line);
  }
}

void SendCookieChanged(net::URLRequestContext* context,
                       const SendSetCookieData& data,
                       bool success) {
  if (success) {
    BrowserThread::PostTask(BrowserThread::IO, FROM_HERE,
                            base::Bind(SendCookieChangedOnIO, context, data));
  }
}
...
void RenderFrameMessageFilter::SetCookie(int32_t render_frame_id,
  ...
  if (GetContentClient()->browser()->AllowSetCookie(
    ...
    // Synchronize the cookie to the Turbo proxy, unless it comes from a secure
    // URL.
    if (!url.SchemeIsCryptographic()) {
      SendSetCookieData data;

      data.url = url;
      data.cookie_store = context->cookie_store();
      data.cookie_line = cookie;
      ...
      callback = base::Bind(SendCookieChanged, context, data);
    }

    context->cookie_store()->SetCookieWithOptionsAsync(
        url, cookie, options, callback);
  }
}
