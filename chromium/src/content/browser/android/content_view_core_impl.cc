...
ContentViewCoreImpl::ContentViewCoreImpl(
  ...
  // Automatically set the user agent override for all WebContents so
  // that it is available when a NavigationEntry requires the user
  // agent to be overridden.
  web_contents->SetUserAgentOverride(
      content::GetContentClient()->GetUserAgentOverride());
...
void ContentViewCoreImpl::SuspendAudio(JNIEnv* env, jobject obj) {
  RenderViewHost* rvh = web_contents_->GetRenderViewHost();
  rvh->SuspendAudio();
}

void ContentViewCoreImpl::ResumeSuspendedAudio(JNIEnv* env, jobject obj) {
  RenderViewHost* rvh = web_contents_->GetRenderViewHost();
  rvh->ResumeSuspendedAudio();
}
...
void ContentViewCoreImpl::OnTouchHandleEvent(ui::TouchHandleEventType event,
                                             const gfx::Rect& bounds) {
  JNIEnv* env = AttachCurrentThread();
  ScopedJavaLocalRef<jobject> j_obj = java_ref_.get(env);
  if (j_obj.is_null())
  ...
  Java_ContentViewCore_onTouchHandleEvent(
      env, j_obj.obj(), event, bounds_rect.obj());
}
