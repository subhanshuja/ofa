...
DownloadInterruptReason BaseFile::MoveFileAndAdjustPermissions(
  ...
  if (new_path.IsContentUri()) {
    if (!base::CopyFile(full_path_, new_path))
      return LogSystemError("Move to uri", errno);
    if (full_path_.IsContentUri()) {
      // TODO(ollel): Delete old uri if we need to handle this case in the
      // future.
      NOTREACHED();
    } else {
      unlink(full_path_.value().c_str());
    }
