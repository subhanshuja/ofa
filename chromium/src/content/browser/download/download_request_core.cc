...
DownloadInterruptReason DownloadRequestCore::HandleRequestStatus(
  ...
  // Some servers will close the idle tcp connection while download is paused
  // for some time (e.g. 1 minute or longer), so we opt to treat this as a
  // transient error.
#if 0
