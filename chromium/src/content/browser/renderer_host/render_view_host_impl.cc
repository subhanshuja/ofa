...
using blink::WebAudioElementPlayState;
...
#if defined(OS_ANDROID)
    IPC_MESSAGE_HANDLER(ViewHostMsg_ShowSelectionPreview,
                        OnShowSelectionPreview)
#endif
    IPC_MESSAGE_HANDLER(ViewHostMsg_DidZoomURL, OnDidZoomURL)
    IPC_MESSAGE_HANDLER(ViewHostMsg_UpdatePlayState, OnUpdatePlayState)
...
#if defined(OS_ANDROID)
void RenderViewHostImpl::OnShowSelectionPreview(
  ...
  std::unique_ptr<cc::SharedBitmap> bitmap =
      HostSharedBitmapManager::current()->GetSharedBitmapFromId(bitmap_size,
                                                                bitmap_id);
  if (!bitmap) {
    bad_message::ReceivedBadMessage(
        GetProcess(), bad_message::RWH_SHARED_BITMAP);
    return;
  }
  DCHECK(bitmap->pixels());

  SkBitmap preview_bitmap;
  preview_bitmap.setInfo(
      SkImageInfo::MakeN32Premul(bitmap_size.width(), bitmap_size.height()));
  preview_bitmap.setPixels(bitmap->pixels());

  RenderWidgetHostViewBase* view = render_widget_host_->GetView();
  if (view)
    view->ShowSelectionPreview(preview_bitmap, focus_rect);

  Send(new ViewMsg_ShowSelectionPreview_ACK(GetRoutingID()));
}
#endif
...
void RenderViewHostImpl::SuspendAudio() {
  Send(new ViewMsg_SuspendAudio(GetRoutingID()));
}

void RenderViewHostImpl::ResumeSuspendedAudio() {
  Send(new ViewMsg_ResumeSuspendedAudio(GetRoutingID()));
}
...
void RenderViewHostImpl::OnUpdatePlayState(
    WebAudioElementPlayState play_state) {
  delegate_->UpdatePlayState(play_state);
}
