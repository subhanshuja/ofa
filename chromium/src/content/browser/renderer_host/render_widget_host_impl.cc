...
void RenderWidgetHostImpl::RenderWidgetFrameReady() {
  FrameCallbackList::iterator it;
  for (it = frame_callbacks_.begin(); it < frame_callbacks_.end(); it++)
    it->Run();
...
bool RenderWidgetHostImpl::OnMessageReceived(const IPC::Message &msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderWidgetHostImpl, msg)
    ...
    IPC_MESSAGE_HANDLER(ViewHostMsg_SelectionStateChanged,
...
void RenderWidgetHostImpl::RequestFrameCallback(const base::Closure& callback) {
...
void RenderWidgetHostImpl::OnSelectionStateChanged(bool is_start_anchor) {
  if (view_) {
    view_->SelectionStateChanged(is_start_anchor);
  }
}
