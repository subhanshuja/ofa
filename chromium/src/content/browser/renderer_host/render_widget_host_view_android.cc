...
void RenderWidgetHostViewAndroid::GetScaledContentBitmap(
  ...
  gfx::Size bounds = content_size_in_layer_;
...
void RenderWidgetHostViewAndroid::SelectionStateChanged(bool is_start_anchor) {
  if (selection_controller_)
    selection_controller_->set_is_start_anchor(is_start_anchor);
}
...
void RenderWidgetHostViewAndroid::ShowSelectionPreview(
    const SkBitmap& preview_bitmap,
    const gfx::Rect& focus_rect) {
  if (content_view_core_)
    content_view_core_->ShowSelectionPreview(preview_bitmap, focus_rect);
}
...
void RenderWidgetHostViewAndroid::InternalSwapCompositorFrame(
  ...
  // Run frame callbacks after processing frame data.
  base::ScopedClosureRunner frame_callback_runner(
      base::Bind(&RenderWidgetHostImpl::RenderWidgetFrameReady,
...
void RenderWidgetHostViewAndroid::OnTouchHandleEvent(
    ui::TouchHandleEventType event,
    const gfx::Rect& bounds) {
  DCHECK(content_view_core_);
  content_view_core_->OnTouchHandleEvent(event, bounds);
}
