...
void ResourceDispatcherHostImpl::ContinuePendingBeginRequest(
  ...
  if (request_data.resource_type == content::RESOURCE_TYPE_MAIN_FRAME)
    new_request->set_turbo_request_type("main-frame");
  else if (request_data.resource_type == content::RESOURCE_TYPE_SUB_FRAME)
    new_request->set_turbo_request_type("sub-frame");
  else if (request_data.resource_type == content::RESOURCE_TYPE_XHR)
    new_request->set_turbo_request_type("xhr");
  new_request->set_turbo_main(request_data.first_party_for_cookies.spec());
