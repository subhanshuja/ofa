...
net::TurboSessionPool* GetTurboSessionPool(
  ...
  return http_network_session->turbo_session_pool().get();
}

void SignalLoadStartOnIOThread(
    scoped_refptr<net::URLRequestContextGetter> context_getter,
    uintptr_t context,
    const GURL& url) {
  net::TurboSessionPool* turbo_session_pool =
      GetTurboSessionPool(context_getter);
  if (turbo_session_pool)
    turbo_session_pool->SignalLoadStart(context, url);
}

void SignalProvisionalLoadStartOnIOThread(
    scoped_refptr<net::URLRequestContextGetter> context_getter,
    uintptr_t context,
    const GURL& url) {
  net::TurboSessionPool* turbo_session_pool =
      GetTurboSessionPool(context_getter);
  if (turbo_session_pool)
    turbo_session_pool->SignalProvisionalLoadStart(context, url);
}

void SignalLoadStopOnIOThread(
    scoped_refptr<net::URLRequestContextGetter> context_getter,
    uintptr_t context,
    const GURL& url,
  ...
  net::TurboSessionPool* turbo_session_pool =
      GetTurboSessionPool(context_getter);
  if (turbo_session_pool) {
    turbo_session_pool->SignalLoadStop(context, url);

    if (completion_flag) {
      // The load has been completed so inform the Turbo client that
      // it is OK to send the list of unclaimed resources back to the Server.
      turbo_session_pool->SendUnclaimed();
    }
  }
}
...
void WebContentsImpl::DidStartProvisionalLoad(
  ...
  if (!render_frame_host->GetParent()) {
    // TODO(jl): Checking the storage partition here is unfortunate,
    // but it returns NULL in unit tests, and that causes the call to
    // GetRequestContextForRenderProcess() to crash.
    if (render_frame_host->GetProcess()->GetStoragePartition()) {
      scoped_refptr<net::URLRequestContextGetter> context_getter(
      ...
      BrowserThread::PostTask(BrowserThread::IO,
                              FROM_HERE,
                              base::Bind(&SignalProvisionalLoadStartOnIOThread,
                                         context_getter,
                                         context,
                                         validated_url));
    }
...
void WebContentsImpl::DidFailProvisionalLoadWithError(
  ...
  if (rfh) {
    BrowserAccessibilityManager* manager = rfh->browser_accessibility_manager();
    if (manager)
...
void WebContentsImpl::UpdatePlayState(
    blink::WebAudioElementPlayState play_state) {
  FOR_EACH_OBSERVER(
      WebContentsObserver, observers_, UpdatePlayState(play_state));
}
...
void WebContentsImpl::DidStartLoading(FrameTreeNode* frame_tree_node,
  ...
  if (entry) {
    ...
    // but it returns NULL in unit tests, and that causes the call to
    // GetRequestContextForRenderProcess() to crash.
    if (render_process_host->GetStoragePartition()) {
      scoped_refptr<net::URLRequestContextGetter> context_getter(
          GetRenderProcessHost()->GetStoragePartition()->
              GetURLRequestContext());
      ...
      BrowserThread::PostTask(
          BrowserThread::IO, FROM_HERE, base::Bind(
              &SignalLoadStartOnIOThread, context_getter, context, url));
    }
  }
...
void WebContentsImpl::DidStopLoading() {
  ...
  if (entry) {
    ...
    RenderProcessHost* render_process_host = GetRenderProcessHost();
    ...
    if (render_process_host->GetStoragePartition()) {
      ...
      BrowserThread::PostTask(BrowserThread::IO,
                              FROM_HERE,
                              base::Bind(&SignalLoadStopOnIOThread,
                                         context_getter,
                                         context,
                                         url,
                                         load_completed_));
