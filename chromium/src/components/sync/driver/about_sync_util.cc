...
#if defined(OPERA_SYNC)
extern const char kExitDuplicationBackoff[] = "exitDuplicationBackoff";
#endif  // OPERA_SYNC
...
#if defined OPERA_SYNC
std::string GetVersionString(syncer::SyncService* service) {
  if (service && service->GetLocalDeviceInfoProvider() &&
      service->GetLocalDeviceInfoProvider()->GetLocalDeviceInfo()) {
    return service->GetLocalDeviceInfoProvider()->GetLocalDeviceInfo()->
        sync_user_agent();
  } else {
    return "n/a";
  }
...
std::string GetConnectionStatus(const SyncService::SyncTokenStatus& status) {
...
#else  // OPERA_SYNC
  switch (status.connection_status.value) {
    case syncer::ConnectionStatus::CONNECTION_NOT_ATTEMPTED:
      base::StringAppendF(&message, "not attempted");
      break;
  ...
  case syncer::ConnectionStatus::CONNECTION_UNAVAILABLE:
    base::StringAppendF(
        &message, "connection unavaiable since %s",
        GetTimeStr(status.connection_status_update_time, "n/a").c_str());
    break;
    default:
      NOTREACHED();
...
#if defined(OPERA_SYNC)
  opera::DuplicateTrackerInterface* tracker = nullptr;
  if (opera::SyncConfig::ShouldDeduplicateBookmarks() && service && service->GetSyncClient()) {
    tracker = service->GetSyncClient()->GetBookmarkDuplicateTracker();
  }
  if (tracker) {
    base::ListValue* duplicate_section =
        AddSection(stats_list, "Bookmark duplicates");
    std::unique_ptr<base::ListValue> tracker_stats = tracker->GetInternalStats();
    DCHECK(tracker_stats.get());
    bool is_desc = true;
    std::unique_ptr<StringSyncStat> string_stat;
    for (auto it(tracker_stats->begin()); it != tracker_stats->end(); it++) {
      std::string value;
      bool ok = (*it)->GetAsString(&value);
      DCHECK(ok);
      DCHECK(!value.empty());
      if (is_desc) {
        string_stat.reset(new StringSyncStat(duplicate_section, value));
      } else {
        DCHECK(string_stat.get());
        string_stat->SetValue(value);
      }
      is_desc = !is_desc;
    }
    DCHECK(is_desc) << "Check your stat list!";
  }
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
  client_version.SetValue(GetVersionString(service));
#else
...
#if defined(OPERA_SYNC)
    syncer::ModelTypeSet opera_type_set = opera::SyncConfig::OperaUserTypes();
    syncer::ModelTypeSet encrypted_type_set = full_status.encrypted_types;
    encrypted_type_set.RetainAll(opera_type_set);
    if (encrypted_type_set.Empty())
      encrypted_types.SetValue("None");
    else
      encrypted_types.SetValue(ModelTypeSetToString(encrypted_type_set));
#else
