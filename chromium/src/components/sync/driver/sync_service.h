...
class SyncService : public DataTypeEncryptionHandler {
 public:
  ...
  struct SyncTokenStatus {
    SyncTokenStatus();
    SyncTokenStatus(const SyncTokenStatus&);
...
#if defined(OPERA_SYNC)
  virtual void AddSyncStateObserver(opera::SyncObserver* observer) = 0;
  virtual void RemoveSyncStateObserver(opera::SyncObserver* observer) = 0;
#endif  // OPERA_SYNC
  ...
  virtual opera::SyncAccount* account() const = 0;

  virtual opera::SyncStatus opera_sync_status() = 0;

  virtual void OnSyncAuthKeeperInitialized() = 0;
#endif  // OPERA_SYNC
