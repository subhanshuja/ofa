...
void SyncPrefs::RegisterProfilePrefs(
  ...
  RegisterDataTypePreferredPref(registry, BOOKMARKS, false);
#else
  ...
  registry->RegisterStringPref(prefs::kSyncKeystoreEncryptionBootstrapToken,
                               std::string());
#if defined(OPERA_SYNC)
  registry->RegisterStringPref(prefs::kOperaSyncEncryptionBootstrapToken,
                               std::string());
  registry->RegisterStringPref(prefs::kOpSyncEncryptionBootstrapToken,
                               std::string());
  registry->RegisterStringPref(prefs::kOperaLastSignedInID, std::string());
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
  pref_service_->ClearPref(prefs::kOperaSyncEncryptionBootstrapToken);
  pref_service_->ClearPref(prefs::kOpSyncEncryptionBootstrapToken);
#endif  // OPERA_SYNC
...
std::string SyncPrefs::GetEncryptionBootstrapToken() const {
  DCHECK(CalledOnValidThread());
#if defined(OPERA_SYNC)
  return pref_service_->GetString(prefs::kOpSyncEncryptionBootstrapToken);
#else
...
void SyncPrefs::SetEncryptionBootstrapToken(const std::string& token) {
  DCHECK(CalledOnValidThread());
#if defined(OPERA_SYNC)
  pref_service_->SetString(prefs::kOpSyncEncryptionBootstrapToken, token);
#else
...
#if defined(OPERA_SYNC)
std::string SyncPrefs::GetLastSignedInID() const {
  DCHECK(CalledOnValidThread());
  return pref_service_->GetString(prefs::kOperaLastSignedInID);
}

void SyncPrefs::SetLastSignedInID(const std::string& id) {
  DCHECK(CalledOnValidThread());
  pref_service_->SetString(prefs::kOperaLastSignedInID, id);
}
#endif  // OPERA_SYNC
...
#ifdef SYNC_HAS_SAVED_NIGORI_STATE
  std::string encoded;
...
#else
  NOTREACHED();
#endif  // SYNC_HAS_SAVED_NIGORI_STATE
}
