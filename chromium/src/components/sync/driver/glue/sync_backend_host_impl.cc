...
#ifdef OPERA_SYNC
syncer::VersionMap SyncBackendHostImpl::invalidation_map() const {
  DCHECK(core_);
  return core_->invalidation_map();
}
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC) && (defined(OS_ANDROID) || defined(OS_IOS))
  // Hack for Opera Mobile/Mini that doesn't do a full shutdown
  bool sync_thread_claimed = true;
#else
  bool sync_thread_claimed = (reason != BROWSER_SHUTDOWN);
#endif  // OPERA_SYNC && (OS_ANDROID || OS_IOS)
