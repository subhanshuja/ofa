...
#else
  NOTREACHED();
#endif  // SYNC_HAS_LOCAL_SET_PASSPHRASE_ENCRYPTION
...
void SyncBackendHostCore::DoOnInvalidatorStateChange(
  ...
  sync_manager_->SetInvalidatorEnabled(state.state ==
...
#if defined(OPERA_SYNC)
        if (!inv_adapter->IsUnknownVersion()) {
          invalidation_map_[type] = inv_adapter->GetVersion();
          VLOG(2) << "New invalidation version for " << type << " is "
                  << inv_adapter->GetVersion();
        }
#endif  // !OPERA_SYNC
...
#if defined(OPERA_SYNC)
syncer::VersionMap SyncBackendHostCore::invalidation_map() const {
...
#if defined(OPERA_SYNC)
  args.sign_params = options->sign_params;
#endif  // OPERA_SYNC
