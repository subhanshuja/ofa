...
bool FakeServer::CreateDefaultPermanentItems() {
  ...
  for (ModelTypeSet::Iterator it = permanent_folder_types.First(); it.Good();
    ...
    if (model_type == syncer::BOOKMARKS) {
      ...
      std::unique_ptr<FakeServerEntity> user_root_bookmarks_entity =
        PermanentEntity::Create(syncer::BOOKMARKS,
        "user_root_bookmarks",
        "User Bookmarks",
        ModelTypeToRootTag(syncer::BOOKMARKS));
      if (user_root_bookmarks_entity == NULL) {
        return false;
      }
      SaveEntity(std::move(user_root_bookmarks_entity));

      std::unique_ptr<FakeServerEntity> unsorted_bookmarks_entity =
        PermanentEntity::Create(syncer::BOOKMARKS,
        "unsorted_bookmarks",
        "Unsorted Bookmarks",
        ModelTypeToRootTag(syncer::BOOKMARKS));
      if (unsorted_bookmarks_entity == NULL) {
        return false;
      }
      SaveEntity(std::move(unsorted_bookmarks_entity));

      std::unique_ptr<FakeServerEntity> shared_bookmarks_entity =
        PermanentEntity::Create(syncer::BOOKMARKS,
        "shared_bookmarks",
        "Shared Bookmarks",
        ModelTypeToRootTag(syncer::BOOKMARKS));
      if (shared_bookmarks_entity == NULL) {
        return false;
      }
      SaveEntity(std::move(shared_bookmarks_entity));

      std::unique_ptr<FakeServerEntity> speeddial_bookmarks_entity =
        PermanentEntity::Create(syncer::BOOKMARKS,
                                "speeddial_bookmarks",
                                "Speed Dial Bookmarks",
                                ModelTypeToRootTag(syncer::BOOKMARKS));
      if (speeddial_bookmarks_entity == NULL) {
        return false;
      }
      SaveEntity(std::move(speeddial_bookmarks_entity));

      std::unique_ptr<FakeServerEntity> trash_bookmarks_entity =
        PermanentEntity::Create(syncer::BOOKMARKS,
                                "trash_bookmarks",
                                "Trash Bookmarks",
                                ModelTypeToRootTag(syncer::BOOKMARKS));
      if (trash_bookmarks_entity == NULL) {
        return false;
      }
      SaveEntity(std::move(trash_bookmarks_entity));
    }
  }
