...
#if defined(OPERA_SYNC)
class LoadBalancerErrorConnectionManager : public ServerConnectionManager {
 public:
  explicit LoadBalancerErrorConnectionManager(CancelationSignal* signal)
      : ServerConnectionManager("unused",
                                0,
                                false,
                                signal,
                                sync_oauth_signer::SignParams()) {}

  ~LoadBalancerErrorConnectionManager() override {}
  bool PostBufferWithCachedAuth(PostBufferParams* params) override {
    SetServerStatus(HttpResponse::LOAD_BALANCER_ERROR);
    return false;
  }
};

TEST_F(SyncerProtoUtilTest, LoadBalancerErrorIsTransientError) {
  CancelationSignal signal;
  LoadBalancerErrorConnectionManager mock_connection_manager(&signal);
  ClientToServerMessage msg;
  SyncerProtoUtil::SetProtocolVersion(&msg);
  msg.set_share("required");
  msg.set_message_contents(ClientToServerMessage::GET_UPDATES);
  sync_pb::ClientToServerResponse response;
  EXPECT_FALSE(SyncerProtoUtil::PostAndProcessHeaders(&mock_connection_manager,
                                                      NULL, msg, &response));
  EXPECT_EQ(SERVER_RETURN_TRANSIENT_ERROR,
            ServerConnectionErrorAsSyncerError(
                mock_connection_manager.server_status()));
}
#endif
