...
#if defined(OPERA_SYNC)
    // Our load balancers can randomly return 504 gateway timeout. In that case
    // we should treat this as a transient error and reconnect without signaling
    // any severe synchronization problem.
    LOAD_BALANCER_ERROR,
#endif
...
struct ServerConnectionEvent {
  ...
  explicit ServerConnectionEvent(HttpResponse::ServerConnectionCode code,
      ...
      : connection_code(code), auth_problem_(problem) {}
#else
  explicit ServerConnectionEvent(HttpResponse::ServerConnectionCode code)
...
#if defined(OPERA_SYNC)
  void SetAuthProblem(opera_sync::OperaAuthProblem problem);
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
  opera_sync::OperaAuthProblem server_auth_problem_;
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
  sync_oauth_signer::SignParams oauth_sign_params_;
#endif  // OPERA_SYNC
