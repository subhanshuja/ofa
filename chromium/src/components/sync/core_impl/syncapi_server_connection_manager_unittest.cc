...
#if defined(OPERA_SYNC)
class ErrorReturningHttpProvider : public BlockingHttpPost {
 public:
  explicit ErrorReturningHttpProvider(int http_error_code)
      : BlockingHttpPost(), http_error_code_(http_error_code) {}
  ~ErrorReturningHttpProvider() override {}

  bool MakeSynchronousPost(int* error_code, int* response_code) override {
    *error_code = net::OK;
    *response_code = http_error_code_;
    return true;
  }
...
class ErrorReturningHttpProviderFactory : public HttpPostProviderFactory {
 public:
  explicit ErrorReturningHttpProviderFactory(int http_error_code)
      : http_error_code_(http_error_code) {}
  ~ErrorReturningHttpProviderFactory() override {}
  void Init(
      const std::string& user_agent,
      const syncer::BindToTrackerCallback& bind_to_tracker_callback) override
  {}
  HttpPostProviderInterface* Create() override {
    return new ErrorReturningHttpProvider(http_error_code_);
  }
  void Destroy(HttpPostProviderInterface* http) override {
    delete static_cast<ErrorReturningHttpProvider*>(http);
  }
...
TEST(SyncAPIServerConnectionManagerTest, GatewayTimeoutIsLoadBalancerError) {
  for (int httperrno = 500; httperrno < 600; httperrno++) {
    CancelationSignal signal;
    SyncAPIServerConnectionManager server(
      "server", 0, true, sync_oauth_signer::SignParams(),
      new ErrorReturningHttpProviderFactory(httperrno), &signal);

    ServerConnectionManager::PostBufferParams params;

    bool result =
      server.PostBufferToPath(&params, "/testpath", "testauth");

    EXPECT_FALSE(result) << httperrno;
    EXPECT_EQ(HttpResponse::LOAD_BALANCER_ERROR,
        params.response.server_status) << httperrno;
  }
}
...
TEST(SyncAPIServerConnectionManagerTest, FailPostWithTimedOut) {
  ...
  SyncAPIServerConnectionManager server(
      ...
      sync_oauth_signer::SignParams(),
#endif
      new FailingHttpPostFactory(net::ERR_TIMED_OUT), &signal);
