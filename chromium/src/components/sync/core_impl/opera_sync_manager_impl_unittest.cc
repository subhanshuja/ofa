...
int64_t MakeServerNode(UserShare* share, ModelType model_type,
                       const std::string& client_tag,
...
TEST_F(SyncApiTest, GetTotalNodeCountOneChild) {
  int64_t type_root = MakeTypeRoot(user_share(), BOOKMARKS);
  int64_t parent = MakeFolderWithParent(user_share(), BOOKMARKS, type_root, NULL);
  {
    ReadTransaction trans(FROM_HERE, user_share());
    ReadNode type_root_node(&trans);
    EXPECT_EQ(BaseNode::INIT_OK,
...
struct FakeServerResponse {
  FakeServerResponse()
      : post_successfull(false), error_code(-1), response_code(-1) {}

  bool post_successfull;  // Wether MakeSynchronousPost succeeds.
  int error_code;         // Network error code. Used when post unsuccessful.
  int response_code;      // Server response code. Valid if no error_code.
};

class TestHttpPostProviderInterface : public HttpPostProviderInterface {
 public:
  explicit TestHttpPostProviderInterface(FakeServerResponse* response)
...
#if defined(OPERA_SYNC)
    *error_code = test_response_->error_code;
    *response_code = test_response_->response_code;
    return test_response_->post_successfull;
#else
    return false;
#endif  // OPERA_SYNC
  }
 ...
 private:
  FakeServerResponse* test_response_;
};

class TestHttpPostProviderFactory : public HttpPostProviderFactory {
 public:
  explicit TestHttpPostProviderFactory(FakeServerResponse* response)
      : test_response_(response) {}
  ...
  HttpPostProviderInterface* Create() override {
    return new TestHttpPostProviderInterface(test_response_);
  }
...
class SyncManagerTest : public testing::Test,
 ...
 protected:
  ...
  void SetUp() {
    ...
    args.post_factory = base::WrapUnique(
        new TestHttpPostProviderFactory(&test_response_));
  ...
  bool SetUpEncryption(NigoriStatus nigori_status,
    ...
    if (nigori_status == WRITE_TO_NIGORI) {
      ...
      cryptographer->GetKeys(nigori.mutable_op_encryption_keybag());
  ...
  void InvalidateToken() {
    // To invalidate a token, we must attempt to a post buffer and make it
    // fail due to SYNC_AUTH_ERROR.
    FakeServerResponse faked_response;
    faked_response.post_successfull = true;
    faked_response.error_code = 200;  // Connection is OK.
    faked_response.response_code = net::HTTP_UNAUTHORIZED;

    // Make sure to undo this setting when we're done
    base::AutoReset<FakeServerResponse> response_reset(&test_response_,
                                                       faked_response);

    // This is only needed to satisfy a broken design.
    HttpResponse response;

    // Try to post a buffer and make sure it fails with SYNC_AUTH_ERROR.
    ServerConnectionManager::PostBufferParams params;
    params.buffer_in = "sending whatever";
    ASSERT_FALSE(sync_manager_.connection_manager_->PostBufferWithCachedAuth(
          &params));
    EXPECT_EQ(net::HTTP_UNAUTHORIZED, params.response.response_code);
    EXPECT_EQ(HttpResponse::SYNC_AUTH_ERROR, params.response.server_status);
  }
#endif  // OPERA_SYNC
...
TEST_F(SyncManagerTest, RefreshEncryptionReady) {
...

  {
    ...
    EXPECT_TRUE(nigori.has_op_encryption_keybag());
    Cryptographer* cryptographer = trans.GetCryptographer();
    EXPECT_TRUE(cryptographer->is_ready());
    EXPECT_TRUE(cryptographer->CanDecrypt(nigori.op_encryption_keybag()));
  }
}
...
TEST_F(SyncManagerTest, EncryptDataTypesWithData) {
  ...
  int64_t folder = MakeFolderWithParent(sync_manager_.GetUserShare(),
                                      BOOKMARKS,
  ...
  {
    ReadTransaction trans(FROM_HERE, sync_manager_.GetUserShare());
    EXPECT_EQ(GetEncryptedTypesWithTrans(&trans),
  ...
  {
    ReadTransaction trans(FROM_HERE, sync_manager_.GetUserShare());
    EXPECT_EQ(GetEncryptedTypesWithTrans(&trans), EncryptableUserTypes());
    EXPECT_TRUE(syncable::VerifyDataTypeEncryptionForTest(
        trans.GetWrappedTrans(),
        BOOKMARKS,
        true /* is encrypted */));
...
TEST_F(SyncManagerTest, SupplyPendingGAIAPass) {
  ...
  Cryptographer other_cryptographer(&encryptor_);
  {
    ...
    EXPECT_EQ(BaseNode::INIT_OK, node.InitTypeRoot(NIGORI));
    sync_pb::NigoriSpecifics nigori;
    other_cryptographer.GetKeys(nigori.mutable_op_encryption_keybag());
...
TEST_F(SyncManagerTest, SupplyPendingExplicitPass) {
  ...
  Cryptographer other_cryptographer(&encryptor_);
  {
    ...
    cryptographer->SetPendingKeys(nigori.op_encryption_keybag());
    EXPECT_TRUE(cryptographer->has_pending_keys());
    nigori.set_op_keybag_is_frozen(true);
    node.SetNigoriSpecifics(nigori);
  }
  EXPECT_CALL(encryption_observer_, OnCryptographerStateChanged(_));
  EXPECT_CALL(encryption_observer_,
...
#if defined(OPERA_SYNC)
  MOCK_METHOD0(OnCredentialsUpdated, void());
#endif  // OPERA_SYNC
};
...
TEST_F(SyncManagerTestWithMockScheduler, BasicConfiguration) {
  ...
  EXPECT_EQ(sync_manager_.InitialSyncEndedTypes(), enabled_types);
  EXPECT_EQ(sync_manager_.GetTypesWithEmptyProgressMarkerToken(
      ModelTypeSet::All()), disabled_types);
}
...
TEST_F(SyncManagerTestWithMockScheduler, ReConfiguration) {
  ...
  EXPECT_EQ(sync_manager_.GetTypesWithEmptyProgressMarkerToken(
      ProtocolTypes()), disabled_types);
}

#if defined(OPERA_SYNC)
// In Opera we allow updating credentials with a token that has resulted in
// receiving a SYNC_AUTH_ERROR in the previous connection attempt.
// Originally, ServerConnectionManager would mark such token as invalid and
// would return SYNC_AUTH_ERROR immediately, without trying to connect to
// the server.
TEST_F(SyncManagerTestWithMockScheduler, UpdateCredentialsWithOldToken) {
  SyncCredentials credentials;
  credentials.email = "foo@bar.com";
  credentials.sync_token = "sometoken";
  credentials.sync_token_secret = "sometokensecret";
  credentials.sync_time_skew = 500;
  OAuth2TokenService::ScopeSet scope_set;
  scope_set.insert(GaiaConstants::kChromeSyncOAuth2Scope);
  credentials.scope_set = scope_set;

  EXPECT_CALL(*scheduler(), OnCredentialsUpdated()).Times(4);
  EXPECT_CALL(manager_observer_, OnConnectionStatusChange(_)).Times(4);
...
TEST_F(SyncManagerTest, PurgeDisabledTypes) {
  ...
  EXPECT_EQ(enabled_types, sync_manager_.InitialSyncEndedTypes());
  ...
  sync_manager_.PurgeDisabledTypes(disabled_types,
                                   ModelTypeSet(),
                                   ModelTypeSet());
  EXPECT_EQ(new_enabled_types, sync_manager_.InitialSyncEndedTypes());
  EXPECT_EQ(disabled_types,
      sync_manager_.GetTypesWithEmptyProgressMarkerToken(ModelTypeSet::All()));
}
