...
#if defined(OPERA_SYNC)
  sync_encryption_handler_.reset(new OperaSyncEncryptionHandlerImpl(
      &share_,
      args->encryptor,
...
void SyncManagerImpl::OnServerConnectionEvent(
  ...
  status.auth_problem = event.auth_problem_;
#endif  // OPERA_SYNC

  if (event.connection_code == HttpResponse::SERVER_CONNECTION_OK) {
#if defined(OPERA_SYNC)
    status.value = ConnectionStatus::CONNECTION_OK;
    FOR_EACH_OBSERVER(SyncManager::Observer, observers_,
  ...
  if (event.connection_code == HttpResponse::SYNC_AUTH_ERROR) {
    observing_network_connectivity_changes_ = false;
#if defined(OPERA_SYNC)
    status.value = ConnectionStatus::CONNECTION_AUTH_ERROR;
    FOR_EACH_OBSERVER(SyncManager::Observer, observers_,
                      OnConnectionStatusChange(status));
#else
    FOR_EACH_OBSERVER(SyncManager::Observer, observers_,
...
#if defined(OPERA_SYNC)
    status.value = ConnectionStatus::CONNECTION_SERVER_ERROR;
    FOR_EACH_OBSERVER(SyncManager::Observer, observers_,
                      OnConnectionStatusChange(status));
#else
    FOR_EACH_OBSERVER(SyncManager::Observer, observers_,
...
#if defined(OPERA_SYNC)
  // Opera-specific whatsoever.
  if (event.connection_code == HttpResponse::CONNECTION_UNAVAILABLE) {
    status.value = ConnectionStatus::CONNECTION_UNAVAILABLE;
    FOR_EACH_OBSERVER(SyncManager::Observer, observers_,
                      OnConnectionStatusChange(status));
  }
#endif  // OPERA_SYNC
