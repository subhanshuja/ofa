...
def ReadRevisionInfo(info_file_path):
  """Read remote repository information from file and build a revision info
  dict.

  The format of |info_file_path| is expected to follow some conditions:
   * Submodule paths are relative to respective parent module.
   * Topmost module path is relative to |info_file_path|.

  Example result (identical to input json sans 'revision'):
    {
      'path': '/home/user/work',
      'remote': 'git.server.com',
      'repository': 'work/component',
      'revision': 'e280eb0ed9c787be3ec85beeb84b61b0c1a7f7e2',
      'submodules': [..]
    }
  """

  def add_revision(parent_dir, module):
    git = lambda *a: GetCommandOutput(['git', '-C', module['path']] + list(a))

    if not os.path.isabs(module['path']):
      module_root = os.path.join(parent_dir, module['path'])
      module['path'] = os.path.realpath(module_root)

    module[u'revision'] = unicode(git('rev-parse', 'HEAD').strip())

    if 'submodules' in module:
      module['submodules'] = map(
          functools.partial(add_revision, module['path']),
          module['submodules'])

    return module

  with open(info_file_path, 'r') as info_file:
    origin_info = json.load(info_file)

  return add_revision(os.path.dirname(info_file_path), origin_info)


def RewriteFileEntries(symfile_content, revision_info):
  """Returns |symfile_content| with FILE entries rewritten based of
  |revision_info| as produced by ReadRevisionInfo.
  """
  if revision_info is None:
    return symfile_content

  re_file = re.compile("^FILE ([0-9]+) (.*)")
  file_pattern = ("FILE %(file_no)s "
                  "git:%(remote)s%(repository)s:%(file_path)s:%(revision)s")

  def _FindModule(module, path):
    # Perform depth-first search to proritize submodules
    for submodule in module['submodules']:
      found_module = _FindModule(submodule, path)
      if found_module:
        return found_module

    if os.path.commonprefix([path, module['path']]) == module['path']:
      return module
    else:
      return None

  def _Rewrite(line):
    line_match = re.match(re_file, line)
    if line_match:
      file_no = line_match.group(1)
      file_path = os.path.realpath(line_match.group(2))

      module = _FindModule(revision_info, file_path)
      ...
      mappings = {
          ...
          'file_path': os.path.relpath(file_path, module['path']),
          'file_no': file_no
      }

      return file_pattern % mappings
    else:
      return line

  content_lines = symfile_content.splitlines()
  return '\n'.join([_Rewrite(l) for l in content_lines])
...
def GenerateSymbols(options, binaries):
  ...
  if options.revision_info_file:
    revision_info = ReadRevisionInfo(options.revision_info_file)
  ...
  def _Worker():
    ...
    while True:
      ...
      if options.revision_info_file:
        syms = RewriteFileEntries(syms, revision_info)
...
def main():
  ...
  parser.add_option('', '--revision-info-file', action='store', metavar='JSON',
                    help=('Write FILE entries relative to repositories '
  ...
  if options.revision_info_file:
    if not os.access(options.revision_info_file, os.R_OK):
      print "Cannot find %s." % options.revision_info_file
      return 1
