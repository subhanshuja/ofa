...
TYPED_TEST_P(InvalidationServiceTest, Basic) {
  ...
  EXPECT_EQ(syncer::INVALIDATIONS_ENABLED, handler.GetInvalidatorState().state);
#else
...
#if defined(OPERA_SYNC)
  EXPECT_EQ(syncer::INVALIDATIONS_ENABLED,
            handler1.GetInvalidatorState().state);
  EXPECT_EQ(syncer::INVALIDATIONS_ENABLED,
            handler2.GetInvalidatorState().state);
  EXPECT_EQ(syncer::INVALIDATIONS_ENABLED,
            handler3.GetInvalidatorState().state);
  EXPECT_EQ(syncer::TRANSIENT_INVALIDATION_ERROR,
            handler4.GetInvalidatorState().state);
#else
...
class BoundFakeInvalidationHandler : public syncer::FakeInvalidationHandler {
 public:
  ...
  // changed.
#if defined(OPERA_SYNC)
  syncer::OperaInvalidatorState GetLastRetrievedState() const;
#else
...
#if defined(OPERA_SYNC)
  syncer::OperaInvalidatorState last_retrieved_state_;
#else
