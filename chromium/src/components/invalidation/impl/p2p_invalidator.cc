...
OperaInvalidatorState P2PInvalidator::GetInvalidatorState() const {
#else
...
void P2PInvalidator::OnNotificationsEnabled() {
  ...
  registrar_.UpdateInvalidatorState(
#if defined(OPERA_SYNC)
...
void P2PInvalidator::OnNotificationsDisabled(
    notifier::OperaNotificationsDisabledReason reason) {
  DCHECK(thread_checker_.CalledOnValidThread());
  registrar_.UpdateInvalidatorState(FromOperaNotifierReason(reason));
}
#else
void P2PInvalidator::OnNotificationsDisabled(
