...
void SyncInvalidationListener::UpdateRegisteredIds(const ObjectIdSet& ids) {
  ...
  if (ticl_state_.state == INVALIDATIONS_ENABLED && registration_manager_) {
#else
...
#if defined(OPERA_SYNC)
  ticl_state_.state = INVALIDATIONS_ENABLED;
#else
...
#if defined(OPERA_SYNC)
    ticl_state_.state = INVALIDATION_CREDENTIALS_REJECTED;
#else
    ticl_state_ = INVALIDATION_CREDENTIALS_REJECTED;
#endif  // OPERA_SYNC
  } else {
#if defined(OPERA_SYNC)
    ticl_state_.state = TRANSIENT_INVALIDATION_ERROR;
#else
...
#if defined(OPERA_SYNC)
  ticl_state_.state = DEFAULT_INVALIDATION_ERROR;
  push_client_state_.state = DEFAULT_INVALIDATION_ERROR;
#else
  ticl_state_ = DEFAULT_INVALIDATION_ERROR;
  push_client_state_ = DEFAULT_INVALIDATION_ERROR;
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
OperaInvalidatorState SyncInvalidationListener::GetState() const {
#else
InvalidatorState SyncInvalidationListener::GetState() const {
#endif  // OPERA_SYNC
  DCHECK(CalledOnValidThread());
#if defined(OPERA_SYNC)
  if (ticl_state_.state == INVALIDATION_CREDENTIALS_REJECTED ||
      push_client_state_.state == INVALIDATION_CREDENTIALS_REJECTED) {
#else
...
#if defined(OPERA_SYNC)
    return OperaInvalidatorState(INVALIDATION_CREDENTIALS_REJECTED,
                                 push_client_state_.problem);
#else
    return INVALIDATION_CREDENTIALS_REJECTED;
#endif  // OPERA_SYNC
  }
#if defined(OPERA_SYNC)
  if (ticl_state_.state == INVALIDATIONS_ENABLED &&
      push_client_state_.state == INVALIDATIONS_ENABLED) {
#else
...
#if defined(OPERA_SYNC)
    return OperaInvalidatorState(INVALIDATIONS_ENABLED,
                                 push_client_state_.problem);
#else
...
#if defined(OPERA_SYNC)
  return OperaInvalidatorState(TRANSIENT_INVALIDATION_ERROR,
                               push_client_state_.problem);
#else
...
#if defined(OPERA_SYNC)
    OperaInvalidatorState invalidator_state) {
#else
