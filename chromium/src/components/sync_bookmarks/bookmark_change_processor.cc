...
namespace {
std::set<bookmarks::BookmarkNode::MetaInfoMap::key_type> g_metainfo_localonly;
}
...
void BookmarkChangeProcessor::UpdateBookmarkWithSyncData(
  ...
  std::unique_ptr<BookmarkNode::MetaInfoMap> new_meta_info_map =
      GetBookmarkMetaInfo(&sync_node);
#if defined(OPERA_SYNC)
  // Merge any local-only meta info in the node to the new map
  const BookmarkNode::MetaInfoMap* old_meta_info_map = node->GetMetaInfoMap();
  if (old_meta_info_map) {
    for (auto it = old_meta_info_map->begin(); it != old_meta_info_map->end();
         ++it) {
      if (!IsMetaInfoLocalOnly(it->first))
        continue;
      (*new_meta_info_map)[it->first] = it->second;
    }
  }
#endif  // OPERA_SYNC
  model->SetNodeMetaInfoMap(node, *new_meta_info_map);
}
...
// static
bool BookmarkChangeProcessor::IsMetaInfoLocalOnly(
    const BookmarkNode::MetaInfoMap::key_type& key) {
  return g_metainfo_localonly.count(key) > 0;
}

// static
void BookmarkChangeProcessor::RegisterLocalOnlyMetaInfo(
    const BookmarkNode::MetaInfoMap::key_type& key) {
  g_metainfo_localonly.insert(key);
}
