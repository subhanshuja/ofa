...
#if defined(OPERA_SYNC)
  // This method actually removes nodes marked for removal after models
  // association.
  // These will be the nodes that are found in the remove journal at
  // browser startup and at the same time not associated with any sync node
  // during model association.
  int64_t DropMarked(const bookmarks::BookmarkNode* start,
                     BookmarkList* for_removal,
                     Context* context);
#endif  // OPERA_SYNC
