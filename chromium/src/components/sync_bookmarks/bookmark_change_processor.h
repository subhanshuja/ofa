...
class BookmarkChangeProcessor : public bookmarks::BookmarkModelObserver,
 ...
 public:
  ...
  // Meta Info Keys that are registered as Local Only are never synced
  // and when modifying a node those keys are still kept even if sync server
  // is not supplying them.
  static bool IsMetaInfoLocalOnly(
      const bookmarks::BookmarkNode::MetaInfoMap::key_type& key);
  static void RegisterLocalOnlyMetaInfo(
      const bookmarks::BookmarkNode::MetaInfoMap::key_type& key);
#endif  // OPERA_SYNC
