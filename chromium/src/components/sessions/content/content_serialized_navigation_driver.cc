...
void ContentSerializedNavigationDriver::Sanitize(
  ...
  // TODO(rchlodnicki): 2015-11-02: Remove after 6 months or so.
  bool rewritten = false;
  if (navigation->virtual_url_.SchemeIs(content::kLegacyOperaUIScheme)) {
    content::ReplaceLegacyOperaScheme(&navigation->virtual_url_);
    rewritten = true;
  }
  if (navigation->original_request_url_.SchemeIs(
          content::kLegacyOperaUIScheme)) {
    content::ReplaceLegacyOperaScheme(&navigation->original_request_url_);
    rewritten = true;
  }
  if (rewritten) {
    navigation->encoded_page_state_ = content::PageState::CreateFromEncodedData(
                                          navigation->encoded_page_state_)
                                          .ReplaceLegacyOperaScheme()
                                          .ToEncodedData();
  }
