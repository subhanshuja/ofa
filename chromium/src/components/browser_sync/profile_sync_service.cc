...
const char kSyncUnrecoverableErrorHistogram[] =
    "Opera.DSK.Sync.UnrecoverableErrors";
#else
...
// Only used by DeviceInfoService
#if !defined(OPERA_SYNC)
...
#if defined(OPERA_SYNC)
class TimerDelayProvider : public opera::SyncDelayProvider {
 public:
  TimerDelayProvider() : timer_(false, false) {}
  void InvokeAfter(base::TimeDelta delay,
                   const base::Closure& callback) override {
    timer_.Start(FROM_HERE, delay, callback);
  }
...
#if defined(OPERA_SYNC)
      , opera_init_params(std::move(other.opera_init_params))
#endif  // OPERA_SYNC
{}
...
ProfileSyncService::ProfileSyncService(InitParams init_params)
    ...
    :
      ...
      sync_service_url_(
#if defined(OPERA_SYNC)
          opera::SyncConfig::SyncServerURL()),
#else
...
#if defined(OPERA_SYNC)
      connection_status_(syncer::ConnectionStatus::CONNECTION_NOT_ATTEMPTED),
#else
      connection_status_(syncer::CONNECTION_NOT_ATTEMPTED),
#endif  // OPERA_SYNC
      last_get_token_error_(GoogleServiceAuthError::AuthErrorNone()),
#if defined(OPERA_SYNC) && defined(OPERA_DESKTOP)
      opera_password_recoverer_(nullptr),
...
#if defined(OPERA_SYNC)
      opera_oauth2_service_(init_params.opera_init_params->auth_service),
      opera_diagnostic_service_(
  ...
  if (!opera::SyncConfig::UsesOAuth2()) {
    sync_client_->CreateSyncAccount(
        std::move(init_params.opera_init_params->login_data_store),
        init_params.opera_init_params->auth_data_updater_factory,
        init_params.opera_init_params->time_skew_resolver_factory,
        base::Bind(&ProfileSyncService::StopAndSuppress,
                   weak_factory_.GetWeakPtr()));
  }
#if defined(OPERA_DESKTOP)
  sync_client_->GetOtherSpeeddialsCleaner();
#endif  // OPERA_DESKTOP
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
  // Store for use in Initialize().
  network_error_show_status_delay_ = init_params.opera_init_params->network_error_show_status_delay;
  show_connection_status_ = init_params.opera_init_params->show_connection_status;
  show_login_prompt_ = init_params.opera_init_params->show_login_prompt;
#endif  // OPERA_SYNC
}
...
void ProfileSyncService::Initialize() {
...
#if defined(OPERA_SYNC)
  opera::SyncAccount* sync_account = nullptr;
  if (!opera::SyncConfig::UsesOAuth2()) {
    sync_account = account();
    DCHECK(sync_account);
  } else {
    DCHECK(opera_oauth2_service_);
  }
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
  if (!opera::SyncConfig::UsesOAuth2()) {
    sync_stopped_reporter_->SetSyncAccount(account());
  }
#endif  // OPERA_SYNC
  ...
  // Disable new DeviceInfoService while DeviceInfoSyncService is alive,
  // including the unused DeviceInfoService cost 50K apk size for Android Mini
#if !defined(OPERA_SYNC)
  ...
  if (opera::SyncConfig::ShouldSynchronizePasswords()) {
    if (sync_prefs_.HasKeepEverythingSynced()) {
      ...
      // Make sure that we preserve the "UNSET" flag on types unconfigured
      // at this moment.
      syncer::ModelTypeSet to_deconfigure = GetUnconfiguredOperaUserTypes();

      // Remember types that are NOT UNSET at this moment.
      syncer::ModelTypeSet configured = opera::SyncConfig::OperaUserTypes();
      configured.RemoveAll(GetUnconfiguredOperaUserTypes());

      // Make sure that all NOT UNSET types are SET to YES.
      // Note that prior to the new dialog we were relying on the
      // KeepEverythingSynced flag.
      syncer::ModelTypeSet to_set = configured;
      to_set.PutAll(GetPreferredDataTypes());

      // What was configured, remains configured.
      to_deconfigure.RemoveAll(to_set);

      // Commence.
      ChangePreferredDataTypes(to_set);
      ClearConfiguredDataTypes(to_deconfigure);
    }
  }
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
  if (!show_connection_status_.is_null()) {
    std::unique_ptr<opera::SyncDelayProvider> delay_provider;
    delay_provider.reset(new TimerDelayProvider);
    sync_error_handler_.reset(sync_client_->CreateSyncErrorHandler(
                                    network_error_show_status_delay_,
                                    std::move(delay_provider),
                                    show_connection_status_,
                                    show_login_prompt_));
    AddSyncStateObserver(sync_error_handler_.get());
  }
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
#if defined(OPERA_DESKTOP)
  if (opera::SyncConfig::ShouldUseAutoPasswordRecovery())
    opera_password_recoverer_ =
        sync_client_->CreateOperaSyncPasswordRecoverer();
#endif  // OPERA_DESKTOP

  if (!opera::SyncConfig::UsesOAuth2() && IsSignedIn()) {
    // The account can already be logged in when the auth data has been restored
    // from prefs. The call to LogIn() will just cause the OnLoggedIn()
    // notification to trigger in such case.
    if (!opera::SyncConfig::ShouldUseAuthTokenRecovery()) {
      // We can not call ProfileSyncServiceFactory::GetForProfile from
      // Initialize as factory has not fully constructed the service yet and
      // calling it from here would result in inifinite recursion.
      // opera::SyncAccount::LogIn() ends up doing that (through
      // SyncStartupTracker) so we have to post a task here.
      base::ThreadTaskRunnerHandle::Get()->PostTask(
          FROM_HERE, base::Bind(&ProfileSyncService::OperaSyncAccountLogIn,
                                weak_factory_.GetWeakPtr()));
    }
  }
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
  // mzajaczkowski: Don't start the service just because Initialize() was
  // called. It will attempt to startup in case the account is already logged on
  // or will attempt to log the account in if command line has credentials (see
  // below). Otherwise it will attempt to start when account becomes logged in,
  // on OnLoggedIn().
  // mzajaczkowski: However, for OAuth2, check the session state and act
  // accordingly.
  if (opera::SyncConfig::UsesOAuth2()) {
...
#else
  if (opera::SyncConfig::UsesOAuth2()) {
    opera_oauth2_service_->RegisterClient(this, "sync");
    opera_oauth2_service_->AddSessionStateObserver(this);
    if (opera_diagnostic_service_) {
      opera_diagnostic_service_->AddSupplier(this);
    }
  } else {
    if (!opera::SyncConfig::ShouldUseAuthTokenRecovery()) {
      if (account()) {
        ...
        // Registration to SyncAuthKeeper performed in
        // OnSyncAuthKeeperInitialized().
      }
    }
  }
#endif  // !OPERA_SYNC
...
void ProfileSyncService::UnregisterAuthNotifications() {
  ...
  if (opera::SyncConfig::UsesOAuth2()) {
    DCHECK(opera_oauth2_service_);
    opera_oauth2_service_->RemoveSessionStateObserver(this);
    opera_oauth2_service_->UnregisterClient(this);
    if (opera_diagnostic_service_) {
      opera_diagnostic_service_->RemoveSupplier(this);
    }
  } else {
    if (opera::SyncConfig::ShouldUseAuthTokenRecovery()) {
#if defined(OPERA_DESKTOP)
      opera::sync::SyncAuthKeeper* keeper =
          GetSyncClient()->GetOperaSyncAuthKeeper();
      DCHECK(keeper);
      keeper->RemoveObserver(this);
#else
      // TODO(mzajaczkowski): SyncAuthKeeper should be extended for mobile.
      NOTREACHED();
#endif  // OPERA_DESKTOP
    } else {
      if (account())
...
SyncCredentials ProfileSyncService::GetCredentials() {
  ...
  if (opera::SyncConfig::UsesOAuth2()) {
    DCHECK(opera_oauth2_service_);
    DCHECK(opera_oauth2_service_->GetSession());
    DCHECK(!opera_oauth2_service_->GetSession()->GetUsername().empty());
    credentials.email = opera_oauth2_service_->GetSession()->GetUsername();
    if (opera_access_token_) {
      credentials.sync_token = opera_access_token_->token();
    }
    if (credentials.sync_token.empty())
      credentials.sync_token = "credentials_lost";
    credentials.scope_set.insert(opera::SyncConfig::OAuth2ScopeSync());
  } else {
    credentials.email = account()->login_data().user_email;
    if (credentials.email.empty())
      credentials.email = account()->login_data().user_name;
    DCHECK(!credentials.email.empty());
    credentials.sync_token = account()->login_data().auth_data.token;
    credentials.sync_token_secret =
      account()->login_data().auth_data.token_secret;
    credentials.sync_time_skew = account()->login_data().time_skew;
    // We're getting DCHECKs if the scope set is empty.
    credentials.scope_set.insert("fake");
  }
#endif  // !OPERA_SYNC
...
void ProfileSyncService::InitializeBackend(bool delete_stale_data) {
  ...
  if (!opera::SyncConfig::UsesOAuth2()) {
    DCHECK(account());
    sign_params = sync_oauth_signer::SignParams(
      opera::SyncConfig::AuthServerURL().host(),
      opera::SyncConfig::ClientKey(),
      opera::SyncConfig::ClientSecret(),
      account()->login_data().auth_data.token,
      account()->login_data().auth_data.token_secret,
      account()->login_data().time_skew);
  }
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
      http_post_provider_factory_getter, sign_params, std::move(saved_nigori_state_));
#else
...
#if defined(OPERA_SYNC)
  SYNC_DUPLICATION_LOG("OnProtocolEvent: " + event.GetType());
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
  SYNC_DUPLICATION_LOG(std::string(syncer::ModelTypeToString(type)) +
                       " requested sync startup");
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
  if (backend_) {
    // backend_ may be not initialized if we're signing out during a deferred
    // start.
...
#if defined(OPERA_SYNC)
  // TODO(mzajaczkowski): We should eventually conver all SyncObserver
  // implementations to SyncServiceObserver. This will be possible out
  // of the box if the SyncService parameter can be omitted for all of
  // them.
  FOR_EACH_OBSERVER(opera::SyncObserver,
                    sync_status_observers_,
                    OnSyncStatusChanged(this));
#endif  // OPERA_SYNC
}
...
#if defined(OPERA_SYNC)
  SYNC_DUPLICATION_LOG("Sync OnUnrecoverableError");
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
void ProfileSyncService::DoChooseTypes() {
  DCHECK(!opera::SyncConfig::ShouldSynchronizePasswords());
  OnUserChoseDatatypes(true, syncer::ModelTypeSet());
}

void ProfileSyncService::SignalSyncUp() {
  DCHECK(!opera::SyncConfig::UsesOAuth2());
  DCHECK(account());
  account()->SyncEnabled();
}

void ProfileSyncService::SignalStartupRefreshNeeded() {
  syncer::ModelTypeSet types = GetPreferredDataTypes();
  types.RemoveAll(syncer::ProxyTypes());

  TriggerRefresh(types);
}

syncer::ModelTypeSet ProfileSyncService::GetUnconfiguredOperaUserTypes() const {
  DCHECK(opera::SyncConfig::ShouldSynchronizePasswords());
  syncer::ModelTypeSet opera_types = opera::SyncConfig::OperaUserTypes();
  syncer::ModelTypeSet unconfigured_types;

  for (syncer::ModelTypeSet::Iterator it = opera_types.First(); it.Good();
      it.Inc()) {
    const char* pref_name = sync_prefs_.GetPrefNameForDataType(it.Get());
    if (!pref_name) {
      NOTREACHED();
      return syncer::ModelTypeSet();
    }
    const PrefService::Preference* preference = sync_client_->GetPrefService()->FindPreference(pref_name);
    if (preference->IsDefaultValue()) {
      unconfigured_types.Put(it.Get());
    }
  }

  return unconfigured_types;
}

void ProfileSyncService::ClearConfiguredDataTypes(
    syncer::ModelTypeSet cleared_types) {
  for (syncer::ModelTypeSet::Iterator it = cleared_types.First(); it.Good();
       it.Inc()) {
    const char* pref_name = sync_prefs_.GetPrefNameForDataType(it.Get());
    if (!pref_name) {
      NOTREACHED();
    ...
    sync_client_->GetPrefService()->ClearPref(pref_name);
  }
}
#endif  // OPERA_SYNC
...
void ProfileSyncService::OnBackendInitialized(
    ...
    bool success) {
#if defined(OPERA_SYNC)
  SYNC_DUPLICATION_LOG(base::StringPrintf("OnBackendInitialized: %s",
  ...
  if (opera::SyncConfig::UsesOAuth2()) {
    ...
    signin_scoped_device_id =
        opera_oauth2_service_->GetSession()->GetSessionId();
  } else {
    signin_scoped_device_id = account()->login_data().session_id;
  }
#endif  // !OPERA_SYNC
...
AuthError ConnectionStatusToAuthError(syncer::ConnectionStatus status) {
  ...
  switch (status) {
#endif  // OPERA_SYNC
#if defined(OPERA_SYNC)
    case syncer::ConnectionStatus::CONNECTION_OK:
#else
    case syncer::CONNECTION_OK:
      ...
      break;
#if defined(OPERA_SYNC)
    case syncer::ConnectionStatus::CONNECTION_AUTH_ERROR:
#else
    case syncer::CONNECTION_AUTH_ERROR:
      ...
      break;
#if defined(OPERA_SYNC)
    case syncer::ConnectionStatus::CONNECTION_UNAVAILABLE:
#else
    // Opera only!
#endif  // OPERA_SYNC
#if defined(OPERA_SYNC)
    case syncer::ConnectionStatus::CONNECTION_SERVER_ERROR:
#else
...
#endif  // !OPERA_SYNC
#if defined(OPERA_SYNC)
  const GoogleServiceAuthError auth_error = ConnectionStatusToAuthError(status);
  VLOG(1) << "Connection status change: " << status.value;
  VLOG(1) << "Connection status change, auth problem: "
          << status.auth_problem.ToString();
  VLOG(1) << "Connection status change, auth error: " << auth_error.ToString();

  UpdateAuthErrorState(auth_error);
  // Token invalidation handled here.
  if (status.value == syncer::ConnectionStatus::CONNECTION_AUTH_ERROR) {
    sync_prefs_.SetSyncAuthError(true);
    if (opera::SyncConfig::UsesOAuth2()) {
      DCHECK(opera_oauth2_service_);
      if (opera_oauth2_service_->GetSession()->IsLoggedIn() &&
          !opera_oauth2_service_->GetSession()->HasAuthError()) {
        if (opera_access_token_) {
          opera_oauth2_service_->SignalAccessTokenProblem(this,
                                                          opera_access_token_);
        }
        RequestAccessToken();
      }
      ...
      DCHECK(account());
      account()->AuthDataExpired(status.auth_problem);
    }
  } else {
    sync_prefs_.SetSyncAuthError(false);
    // Reset backoff time after successful connection.
    if (status.value == syncer::ConnectionStatus::CONNECTION_OK) {
      request_access_token_backoff_.Reset();
    }
  }
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
bool ProfileSyncService::IsUsernameChangeDuringRelogin() {
  DCHECK(opera::SyncConfig::ShouldSynchronizePasswords());
  opera::SyncStatus status = opera_sync_status();
  if (status.server_error() || status.request_login_again()) {
    std::string now_signed_in_id = GetCurrentlySignedInID();
    std::string last_signed_in_id = sync_prefs_.GetLastSignedInID();
    if (now_signed_in_id != last_signed_in_id) {
      return true;
    }
  }
  return false;
...
void ProfileSyncService::OnLoggedIn(opera::AccountService* service,
                                    opera_sync::OperaAuthProblem problem) {
  DCHECK(!opera::SyncConfig::UsesOAuth2());
  DCHECK(thread_checker_.CalledOnValidThread());
  SYNC_DUPLICATION_LOG("Sync OnLoggedIn");
  DCHECK_EQ(service, account()->service().get());
  DCHECK(account()->HasAuthData());
  DCHECK(account()->HasValidAuthData());

  if (opera::SyncConfig::ShouldSynchronizePasswords()) {
    if (IsUsernameChangeDuringRelogin()) {
      ShutdownImpl(syncer::DISABLE_SYNC);
      sync_prefs_.SetLastSignedInID(GetCurrentlySignedInID());
      base::ThreadTaskRunnerHandle::Get()->PostTask(
          FROM_HERE, base::Bind(&ProfileSyncService::OnLoggedIn,
                                weak_factory_.GetWeakPtr(), service, problem));
      return;
    }
  }
  ...
  ClearLoginError();
  ClearUnrecoverableError();
  // Logging out sets this pref to true so clear it if logged in
  // successfully before starting. Otherwise startup may fail prematurely
  // (due to being suppressed).
  sync_prefs_.SetSyncRequested(true);
  sync_prefs_.SetSyncAuthError(false);

  std::string password = account()->login_data().password;
  if (!password.empty()) {
    HandleInitialLogin(password);
  }
  ...
  token_receive_time_ = base::Time::Now();
  last_get_token_error_ = GoogleServiceAuthError::AuthErrorNone();
  last_get_token_error_string_ = "";
  next_token_request_time_ = base::Time();

  HandleOnLoggedIn();

  if (opera::SyncConfig::ShouldSynchronizePasswords() &&
      opera::SyncConfig::ShouldBypassTypeConfiguration()) {
    JustDoSync(DO_SYNC_ALL);
  }
}

void ProfileSyncService::OnLogoutRequested(opera::AccountService* service,
    opera::account_util::LogoutReason logout_reason) {
  DCHECK(!opera::SyncConfig::UsesOAuth2());
  DCHECK(thread_checker_.CalledOnValidThread());
  SYNC_DUPLICATION_LOG("Sync OnLogoutRequested");
  DCHECK(account()->service().get() == service);
  sync_stopped_reporter_->DoCacheAuthHeaderNow();
}

void ProfileSyncService::OnSyncAuthKeeperTokenStateChanged(
    opera::sync::SyncAuthKeeper* keeper,
    opera::sync::SyncAuthKeeper::TokenState state) {
  DCHECK(!opera::SyncConfig::UsesOAuth2());
  DCHECK(opera::SyncConfig::ShouldUseAuthTokenRecovery());
  DCHECK_EQ(keeper, sync_client_->GetOperaSyncAuthKeeper());

  NotifyObservers();
}

void ProfileSyncService::OnSyncAuthKeeperStatusChanged(
    opera::sync::SyncAuthKeeper* keeper) {
  DCHECK(!opera::SyncConfig::UsesOAuth2());
  DCHECK(opera::SyncConfig::ShouldUseAuthTokenRecovery());
  // Empty.
...
void ProfileSyncService::OnSessionStateChanged() {
  DCHECK(opera::SyncConfig::UsesOAuth2());
  DCHECK(opera_oauth2_service_->GetSession());

  auto state = opera_oauth2_service_->GetSession()->GetState();
  VLOG(2) << "Auth service in state "
          << opera::oauth2::SessionStateToString(state);
  switch (state) {
    case opera::oauth2::OA2SS_INACTIVE:
      LoggedOutImpl();
      break;
    case opera::oauth2::OA2SS_STARTING:
      HandleSessionStateChangeToStarting();
      break;
    case opera::oauth2::OA2SS_IN_PROGRESS:
      HandleSessionStateChangeToInProgress();
      break;
    case opera::oauth2::OA2SS_FINISHING:
      // Empty. There seems to be no need to cache the auth header with OAuth2.
      LoggedOutImpl();
      break;
    case opera::oauth2::OA2SS_AUTH_ERROR:
      opera_access_token_ = nullptr;
      UpdateAuthErrorState(GoogleServiceAuthError(TOKEN_INVALID_ERROR));
      break;
    default:
      NOTREACHED();
  }
...
void ProfileSyncService::OnAccessTokenRequestCompleted(
  ...
  DCHECK_EQ(response.scopes_requested().encode(),
            opera::SyncConfig::OAuth2ScopeSync());

  switch (response.auth_error()) {
    case opera::oauth2::OAE_OK:
      DCHECK(response.access_token());
      if (response.access_token()->scopes() !=
          opera::oauth2::ScopeSet::FromEncoded(
              opera::SyncConfig::OAuth2ScopeSync())) {
        VLOG(1) << "Unexpected granted scopes.";
      }
      DCHECK(response.access_token());

      VLOG(1) << "Sync obtained an access token.";
      ...
      UpdateAuthErrorState(AuthError(GoogleServiceAuthError::NONE));
    ...
    default:
      ...
      UpdateAuthErrorState(
          AuthError(TOKEN_INVALID_ERROR));

      break;
  }
}

void ProfileSyncService::OnAccessTokenRequestDenied(
    opera::oauth2::AuthService* service,
    opera::oauth2::ScopeSet requested_scopes) {
  DCHECK(opera::SyncConfig::UsesOAuth2());
  DCHECK_EQ(opera_oauth2_service_, service);
  DCHECK_EQ(requested_scopes.encode(), opera::SyncConfig::OAuth2ScopeSync());
  DCHECK_EQ(nullptr, opera_access_token_.get());
}

std::string ProfileSyncService::GetDiagnosticName() const {
  return "sync_service";
}

std::unique_ptr<base::DictionaryValue>
ProfileSyncService::GetDiagnosticSnapshot() {
  std::unique_ptr<base::DictionaryValue> snapshot(new base::DictionaryValue);
  snapshot->SetString("status", opera_sync_status().ToString());
  return snapshot;
}

void ProfileSyncService::HandleSessionStateChangeToStarting() {
  DCHECK(thread_checker_.CalledOnValidThread());
  DCHECK(opera_oauth2_service_->GetSession());
  DCHECK_EQ(opera::oauth2::OA2SS_STARTING,
            opera_oauth2_service_->GetSession()->GetState());
  ...
  if (opera::SyncConfig::ShouldSynchronizePasswords()) {
    if (IsUsernameChangeDuringRelogin()) {
      ...
      base::ThreadTaskRunnerHandle::Get()->PostTask(
          FROM_HERE,
          base::Bind(&ProfileSyncService::HandleSessionStateChangeToStarting,
                     weak_factory_.GetWeakPtr()));
      return;
    }
...
void ProfileSyncService::HandleSessionStateChangeToInProgress() {
  ClearLoginError();
  ClearUnrecoverableError();
  // Logging out sets this pref to true so clear it if logged in
  // successfully before starting. Otherwise startup may fail prematurely
...
std::string ProfileSyncService::GetCurrentlySignedInID() const {
  DCHECK(opera::SyncConfig::ShouldSynchronizePasswords());
  if (opera::SyncConfig::UsesOAuth2()) {
    DCHECK(opera_oauth2_service_);
    DCHECK(opera_oauth2_service_->GetSession());
    const auto session_state = opera_oauth2_service_->GetSession()->GetState();
    DCHECK(session_state == opera::oauth2::OA2SS_STARTING ||
           session_state == opera::oauth2::OA2SS_IN_PROGRESS);
    DCHECK(!opera_oauth2_service_->GetSession()->GetUsername().empty());
    return opera_oauth2_service_->GetSession()->GetUsername();
  } else {
    DCHECK(account()->IsLoggedIn());
    if (account()->login_data().used_username_to_login)
      return account()->login_data().user_name;
    else
      return account()->login_data().user_email;
  }
}

void ProfileSyncService::ScheduleTokenFetchRetry(
    opera_sync::OperaAuthProblem problem) {
  if (request_access_token_retry_timer_.IsRunning()) {
    NOTREACHED() << "Token fetch retry already in progress";
    return;
  }
  ...
  VLOG(1) << "Authorization data schedule retry request in "
          << delta.InSeconds() << " s";

  next_token_request_time_ = base::Time::Now() + delta;

  request_access_token_retry_timer_.Start(
      FROM_HERE, delta,
      base::Bind(&ProfileSyncService::RequestOperaOAuth1AccessToken,
                 weak_factory_.GetWeakPtr(), problem));
}

void ProfileSyncService::OnLoginError(opera::AccountService* sync_account,
    opera::account_util::AuthDataUpdaterError error_code,
    opera::account_util::AuthOperaComError auth_code,
    const std::string& message,
    opera_sync::OperaAuthProblem problem) {
  DCHECK(thread_checker_.CalledOnValidThread());
  SYNC_DUPLICATION_LOG("Sync OnLoginError: " + problem.ToString());
  VLOG(4) << "[" <<
    opera::account_util::AuthDataUpdaterErrorToString(error_code) << "] " <<
    opera::account_util::AuthOperaComErrorToString(auth_code) << " " <<
  ...
  switch (error_code) {
  case opera::account_util::ADUE_AUTH_ERROR:
    state = this->account()->HasValidAuthData() ?
      AuthError::INVALID_GAIA_CREDENTIALS :
      TOKEN_INVALID_ERROR;

    UMA_HISTOGRAM_ENUMERATION(
        "Opera.DSK.Sync.LoginError.Reason", problem.reason(),
        opera_sync::OperaAuthProblem::Reason::REASON_LAST);
    UMA_HISTOGRAM_ENUMERATION(
        "Opera.DSK.Sync.LoginError.Source", problem.source(),
        opera_sync::OperaAuthProblem::Source::SOURCE_LAST);

    if (!opera::SyncConfig::ShouldUseAuthTokenRecovery()) {
      account()->LogOut(
          opera::account_util::LR_TOKEN_RENEWAL_ERROR_NO_RECOVERY);
    } else {
      // Note that opera::SyncAuthKeeper takes over here.
    }
    break;

  case opera::account_util::ADUE_NETWORK_ERROR:
  case opera::account_util::ADUE_HTTP_ERROR:
  case opera::account_util::ADUE_PARSE_ERROR:
    state = AuthError::CONNECTION_FAILED;
    ScheduleTokenFetchRetry(problem);
    break;

  case opera::account_util::ADUE_INTERNAL_ERROR:
    // Auth data not available while attempting a refresh.
    // We believe that user did manage to log out while auth data was
    // refreshed.
    // Leave the auth state as it is, should have already been cleared
    // in OnLoggedOut().
    DCHECK(!account()->IsLoggedIn());
    state = GetAuthError().state();
    DCHECK(state == AuthError::NONE);
    break;
  ...
  last_get_token_error_ = AuthError(state);
  last_get_token_error_string_ = message;
  UpdateAuthErrorState(AuthError(state));
  sync_prefs_.SetSyncAuthError(true);
}

void ProfileSyncService::OnLoggedOut(opera::AccountService* account,
    opera::account_util::LogoutReason logout_reason) {
  LoggedOutImpl();
}

void ProfileSyncService::LoggedOutImpl() {
  DCHECK(thread_checker_.CalledOnValidThread());
  SYNC_DUPLICATION_LOG("Sync OnLoggedOut");
  ClearLoginError();
...
void ProfileSyncService::OnAuthDataExpired(opera::AccountService* account,
    opera_sync::OperaAuthProblem problem) {
  DCHECK(thread_checker_.CalledOnValidThread());
  SYNC_DUPLICATION_LOG("Sync OnAuthDataExpired");
...
#if defined(OPERA_SYNC)
  if (opera::SyncConfig::ShouldSynchronizePasswords()) {
    if (sync_prefs_.GetEncryptionBootstrapToken().empty() &&
        cached_passphrase_.empty() &&
        !IsUsingSecondaryPassphrase()) {
      if (!opera::SyncConfig::ShouldUseAutoPasswordRecovery()) {
#if defined(OPERA_DESKTOP)
        DCHECK(!opera_password_recoverer_.get());
#endif  // OPERA_DESKTOP
        upgrade_passphrase_status_ = MISSING;
      } else {
#if defined(OPERA_DESKTOP)
        DCHECK(opera_password_recoverer_.get());
        DCHECK(!HasUnrecoverableError());

        VLOG(1) << "Attempt recovery request.";
        // Try to recover the password from the password manager.
        // We'll get false from this call in case recovery is already in
        // progress or has failed.
        opera::SyncPasswordRecoverer::RecoveryFinishedCallback callback =
            base::Bind(&ProfileSyncService::OnSyncPasswordRecoveryFinished,
                       weak_factory_.GetWeakPtr());
        bool scheduled = opera_password_recoverer_->TryRecover(
            GetCurrentlySignedInID(), callback);

        if (!scheduled) {
          if (!opera_password_recoverer_->is_idle()) {
            // A recovery is in progress, do nothing.
            VLOG(1) << "Recovery already in progress.";
          } else {
            switch (opera_password_recoverer_->result()) {
            case opera::SyncPasswordRecoverer::SPRR_INTERNAL_ERROR:
            case opera::SyncPasswordRecoverer::SPRR_CREDENTIALS_NOT_FOUND:
            case opera::SyncPasswordRecoverer::SPRR_CREDENTIALS_INCORRECT:
              VLOG(1) << "Recovery already failed.";
              upgrade_passphrase_status_ = MISSING;
              break;
            case opera::SyncPasswordRecoverer::SPRR_RECOVERED:
              VLOG(1) << "Recovery already succeeded.";
              upgrade_passphrase_status_ = OK;
              break;
            case opera::SyncPasswordRecoverer::SPRR_UNKNOWN:
            default:
              NOTREACHED();
              // Stay safe.
              upgrade_passphrase_status_ = MISSING;
          ...
          DCHECK(!opera_password_recoverer_->is_idle());
          VLOG(1) << "Recovery scheduled.";
          upgrade_passphrase_status_ = RECOVERING;
        }
#else
        NOTREACHED() << "You need to port SyncPasswordRecoverer.";
#endif  // OPERA_DESKTOP
      }  // !opera::SyncConfig::ShouldUseAutoPasswordRecovery()
    } else {
      // This will be needed in case another client upgrades account encryption
      // to explicit and the change syncs back to this client.
      // We don't really care that much about intercepting the implicit
      // passphrase in this scenario since:
      // * the bootstrap token will be generated basing on the explicit
      //   passphrase;
      // * the only way to get back (ATM!) to implicit encryption is to reset
      //   the remote account and that means a kick-out and an explicit login
      //   that will intercept the implicit passphrase.
      upgrade_passphrase_status_ = OK;
    }
  }
#endif  // OPERA_SYNC
...
void ProfileSyncService::OnActionableError(const SyncProtocolError& error) {
#if defined(OPERA_SYNC)
  SYNC_DUPLICATION_LOG("Sync OnActionableError");
#endif  // OPERA_SYNC
  ...
  switch (error.action) {
    ...
    case syncer::DISABLE_SYNC_ON_CLIENT:
      ...
      RequestStop(CLEAR_DATA);
#if defined(OPERA_SYNC)
      UMA_HISTOGRAM_COUNTS("Opera.DSK.Sync.DisableSyncOnClientRequested", 1);
      if (!opera::SyncConfig::UsesOAuth2()) {
        account()->LogOut(
          opera::account_util::LR_DISABLE_SYNC_ON_CLIENT_REQUESTED);
      } else {
        DCHECK(opera_oauth2_service_);
        opera_oauth2_service_->EndSession(
            opera::oauth2::SER_DISABLE_SYNC_ON_CLIENT_REQUESTED);
      }
#else
#if !defined(OS_CHROMEOS)
...
#if defined(OPERA_SYNC)
bool ProfileSyncService::is_configuring() const {
  if (!data_type_manager_)
    return false;

  switch (data_type_manager_->state()) {
    case syncer::DataTypeManager::CONFIGURED:
      return false;
    case syncer::DataTypeManager::CONFIGURING:
    case syncer::DataTypeManager::RETRYING:
    case syncer::DataTypeManager::STOPPING:
      return true;
    default:
      NOTREACHED();
  }
...
bool ProfileSyncService::is_any_type_configured() const {
  syncer::ModelTypeSet opera_types = opera::SyncConfig::OperaUserTypes();
  syncer::ModelTypeSet unconfigured_types = GetUnconfiguredOperaUserTypes();
  opera_types.RemoveAll(unconfigured_types);
  return !opera_types.Empty();
}
#endif  // OPERA_SYNC
...
bool ProfileSyncService::IsSignedIn() const {
  ...
  if (opera::SyncConfig::UsesOAuth2()) {
    DCHECK(opera_oauth2_service_);
    return opera_oauth2_service_->GetSession() &&
           opera_oauth2_service_->GetSession()->IsLoggedIn();
  } else {
    DCHECK(account());
...
#else
  // Mobile gets the last synced time directly, not the delta! This is due
  // to the ui::TimeFormat::Simple() implementation missing there.
  return base::TimeFormatShortDateAndTime(last_synced_time);
#endif  // OPERA_DESKTOP
}
...
#if defined(OPERA_SYNC)
  if (opera::SyncConfig::ShouldSynchronizePasswords()) {
    DCHECK(!sync_prefs_.HasKeepEverythingSynced());
  }
#endif  // OPERA_SYNC
  const syncer::ModelTypeSet registered_types = GetRegisteredDataTypes();
...
#if defined(OPERA_SYNC)
  VLOG(1) << "Apply cached passphrase.";
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
      VLOG(1) << "Cached passphrase successfully decrypted pending keys";
#else
...
#if defined(OPERA_SYNC)
void ProfileSyncService::RequestOperaOAuth1AccessToken(
    opera_sync::OperaAuthProblem problem) {
  DCHECK(!opera::SyncConfig::UsesOAuth2());
  account()->RetryAuthDataRenewal(problem);
}
#endif  // !OPERA_SYNC
...
void ProfileSyncService::RequestAccessToken() {
  ...
  token_request_time_ = base::Time::Now();
  token_receive_time_ = base::Time();
  opera_oauth2_service_->RequestAccessToken(
      this, opera::oauth2::ScopeSet::FromEncoded(
                opera::SyncConfig::OAuth2ScopeSync()));
#endif  // OPERA_SYNC
}
...
#if defined(OPERA_SYNC)
void ProfileSyncService::AddSyncStateObserver(opera::SyncObserver* observer) {
  sync_status_observers_.AddObserver(observer);
}

void ProfileSyncService::RemoveSyncStateObserver(opera::SyncObserver* observer) {
...
#else
  // In Opera assume enabled, we have other, platform-specific means to disable.
  return true;
#endif  // !OPERA_SYNC
}
...
#if defined(OPERA_SYNC)
  return WeakHandle<syncer::JsEventHandler>();
#else
...
#if defined(OPERA_SYNC)
  status.last_get_token_error_string = last_get_token_error_string_;
#endif  // OPERA_SYNC
  if (request_access_token_retry_timer_.IsRunning())
...
#if defined(OPERA_SYNC)
bool ProfileSyncService::was_syncing_this_profile() const {
  return !sync_prefs_.GetLastSignedInID().empty();
}
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
opera::SyncAccount* ProfileSyncService::account() const {
  DCHECK(!opera::SyncConfig::UsesOAuth2());
  DCHECK(sync_client_->GetSyncAccount());
  return sync_client_->GetSyncAccount();
}

opera::SyncStatus ProfileSyncService::opera_sync_status() {
  const ProfileSyncService::SyncStatusSummary summary =
      QuerySyncStatusSummary();

  opera::SyncServerInfo server_info;
  if (summary == ProfileSyncService::INITIALIZED)
    server_info.server_status = opera::SyncServerInfo::SYNC_OK;

  syncer::SyncStatus detailed_status;
  QueryDetailedSyncStatus(&detailed_status);
  switch (detailed_status.sync_protocol_error.error_type) {
    ...
    case syncer::DISABLED_BY_ADMIN:
      server_info.server_status = opera::SyncServerInfo::SYNC_UNKNOWN_ERROR;
      break;
    default:
      break;
  }

  const bool disable_sync = detailed_status.sync_protocol_error.action ==
                            syncer::DISABLE_SYNC_ON_CLIENT;

  const bool busy = detailed_status.syncing;

  bool logged_in = IsSignedIn();

  if (opera::SyncConfig::UsesOAuth2()) {
    DCHECK(opera_oauth2_service_);
    logged_in |= opera_oauth2_service_->GetSession() &&
                 (opera_oauth2_service_->GetSession()->GetState() ==
                  opera::oauth2::OA2SS_STARTING);
  }

  int network_error = net::OK;
  switch (GetAuthError().state()) {
    case AuthError::CONNECTION_FAILED:
      network_error = net::ERR_CONNECTION_FAILED;
      break;
    case TOKEN_INVALID_ERROR:
      server_info.server_status =
          opera::SyncServerInfo::SYNC_AUTH_TOKEN_INVALID;
      break;
    case AuthError::INVALID_GAIA_CREDENTIALS:
      server_info.server_status =
          opera::SyncServerInfo::SYNC_AUTH_TOKEN_EXPIRED;
      break;
    default:
      // And this is basically it. We won't get any more error codes here
      // apart from the OK code.
      DCHECK_EQ(AuthError::NONE, GetAuthError().state());
      break;
  }

  const bool internal_error = HasUnrecoverableError();
  const std::string& internal_error_message = unrecoverable_error_message();
  const bool passphrase_required = IsPassphraseRequiredForDecryption();
  const bool setup_in_progress = IsFirstSetupInProgress();

  syncer::ModelTypeSet unconfigured_types;
  if (opera::SyncConfig::ShouldSynchronizePasswords()) {
    unconfigured_types = GetUnconfiguredOperaUserTypes();
  }

  opera::SyncStatus final_status(network_error, server_info, busy, logged_in,
                                 internal_error, internal_error_message,
                                 disable_sync, passphrase_required,
                                 setup_in_progress, unconfigured_types);

  bool configuring =
      is_configuring() || (summary == ProfileSyncService::SETUP_INCOMPLETE);

  if (opera::SyncConfig::UsesOAuth2()) {
    DCHECK(opera_oauth2_service_);
    configuring |= opera_oauth2_service_->GetSession() &&
                   (opera_oauth2_service_->GetSession()->GetState() ==
                    opera::oauth2::OA2SS_STARTING);
  }

  if (opera::SyncConfig::ShouldSynchronizePasswords()) {
    final_status.set_first_start(!is_any_type_configured());

    bool should_login_again = false;
    switch (upgrade_passphrase_status()) {
      case ProfileSyncService::OK:
        break;
      case ProfileSyncService::RECOVERING:
        DCHECK(opera::SyncConfig::ShouldUseAutoPasswordRecovery());
        // Avoid messing with datatypes at this time.
        configuring = true;
        break;
      case ProfileSyncService::MISSING:
        should_login_again = true;
        break;
      default:
        NOTREACHED();
    }

    final_status.set_is_configuring(configuring);
    final_status.set_request_login_again(should_login_again);
  }

  final_status.set_token_state(opera::SyncStatus::TS_UNSET);
  if (!opera::SyncConfig::UsesOAuth2()) {
    if (opera::SyncConfig::ShouldUseAuthTokenRecovery()) {
      opera::sync::SyncAuthKeeper* keeper =
          GetSyncClient()->GetOperaSyncAuthKeeper();
      DCHECK(keeper);
      switch (keeper->token_state()) {
        case opera::sync::SyncAuthKeeper::TOKEN_STATE_OK:
          final_status.set_token_state(opera::SyncStatus::TS_OK);
          break;
        case opera::sync::SyncAuthKeeper::TOKEN_STATE_RECOVERING:
        case opera::sync::SyncAuthKeeper::TOKEN_STATE_REFRESHING:
          final_status.set_token_state(opera::SyncStatus::TS_WARNING);
          break;
        case opera::sync::SyncAuthKeeper::TOKEN_STATE_LOST:
          final_status.set_token_state(opera::SyncStatus::TS_ERROR);
          break;
        case opera::sync::SyncAuthKeeper::TOKEN_STATE_LOGGED_OUT:
          final_status.set_token_state(opera::SyncStatus::TS_LOGGED_OUT);
          break;
        case opera::sync::SyncAuthKeeper::TOKEN_STATE_UNKNOWN:
        default:
          NOTREACHED() << keeper->token_state_string();
          break;
      }
    }
  } else {  // opera::SyncConfig::UsesOAuth2()
    DCHECK(opera_oauth2_service_);
    const auto& session = opera_oauth2_service_->GetSession();
    if (session) {
      const auto session_state = session->GetState();
      if (session_state == opera::oauth2::OA2SS_STARTING &&
        session->GetStartMethod() == opera::oauth2::SSM_OAUTH1) {
        // Migration in progress.
        final_status.set_token_state(opera::SyncStatus::TS_WARNING);
      } else if (session_state == opera::oauth2::OA2SS_AUTH_ERROR) {
        final_status.set_token_state(opera::SyncStatus::TS_ERROR);
      } else if (session_state == opera::oauth2::OA2SS_IN_PROGRESS &&
                 GetAuthError() != GoogleServiceAuthError::AuthErrorNone()) {
        final_status.set_token_state(opera::SyncStatus::TS_WARNING);
      }
    }
  }

  final_status.set_auth_error(GetAuthError().state());

  return final_status;
}

void ProfileSyncService::OnSyncAuthKeeperInitialized() {
  DCHECK(opera::SyncConfig::ShouldUseAuthTokenRecovery());
  DCHECK(sync_client_);
  DCHECK(sync_client_->GetOperaSyncAuthKeeper());

  sync_client_->GetOperaSyncAuthKeeper()->AddObserver(this);
  bool is_token_lost = sync_client_->GetOperaSyncAuthKeeper()->is_token_lost();

  if (account()->IsLoggedIn()) {
    if (!is_token_lost) {
      base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE,
        base::Bind(&ProfileSyncService::OperaSyncAccountLogIn,
                   weak_factory_.GetWeakPtr()));
    }
  }
}

void ProfileSyncService::OperaSyncAccountLogIn() {
  DCHECK(thread_checker_.CalledOnValidThread());
  SYNC_DUPLICATION_LOG("Sync OperaSyncAccountLogIn");
  account()->LogIn();
}

opera::SyncLoginData ProfileSyncService::login_data() const {
  return account() != NULL ?
      account()->login_data() :
      opera::SyncLoginData();
}

opera::SyncLoginErrorData ProfileSyncService::login_error_data() const {
  return account() != NULL
    ? account()->login_error_data()
    : opera::SyncLoginErrorData();
}

void ProfileSyncService::HandleOnLoggedIn() {
  DCHECK(IsSignedIn());

  if (HasSyncingBackend()) {
    // We're already syncing, this is a token update notification.
    backend_->UpdateCredentials(GetCredentials());
  } else {
    bool start_syncing_now = true;
    if (opera::SyncConfig::ShouldSynchronizePasswords()) {
      start_syncing_now = is_any_type_configured();
    }

    if (start_syncing_now) {
      SetFirstSetupComplete();
      startup_controller_->TryStart();
    } else {
      // Defer the startup until user chooses types.
      DCHECK(opera::SyncConfig::ShouldSynchronizePasswords());
      // This is either a fresh login for the profile or a first
      // persisted login after upgrade to a client that handles new
      // types (also a persisted login after a restart after turning
      // on a datatype feature flag).
      // If this is a fresh login, ALL OperaUserTypes() will be unconfigured.
      // But we don't care much ATM besides that we:
      // 1. Don't start the service automatically;
      // 2. Notify the observers to update the button and popup state.
      NotifyObservers();
    }
  }
}

void ProfileSyncService::HandleInitialLogin(const std::string& password) {
  if (opera::SyncConfig::ShouldSynchronizePasswords()) {
    // We only want to clear the types in case this is an explicit log-in,
    // not a presisted one.
    std::string now_signed_in_id = GetCurrentlySignedInID();
    std::string last_signed_in_id = sync_prefs_.GetLastSignedInID();
    if (now_signed_in_id != last_signed_in_id) {
      // Don't change username while recovering the passphrase.
      DCHECK_NE(upgrade_passphrase_status_, MISSING);

      ChangePreferredDataTypes(syncer::ModelTypeSet());
      ClearConfiguredDataTypes(opera::SyncConfig::OperaUserTypes());
    }
    upgrade_passphrase_status_ = OK;
  }
  cached_passphrase_ = password;
...
syncer::VersionMap ProfileSyncService::invalidation_map() const {
  return backend_->invalidation_map();
}

#if defined(OPERA_DESKTOP)
void ProfileSyncService::OnSyncPasswordRecoveryFinished(
  ...
  DCHECK(opera::SyncConfig::ShouldSynchronizePasswords());
  DCHECK(opera::SyncConfig::ShouldUseAutoPasswordRecovery());
  if (result != opera::SyncPasswordRecoverer::SPRR_RECOVERED) {
    // TODO(mzajaczkowski): Retry on soft errors?
    VLOG(1) << "Finished, failed.";
    upgrade_passphrase_status_ = MISSING;
  } else {
    VLOG(1) << "Finished, succeeded.";
    upgrade_passphrase_status_ = OK;
    // TODO(mzajaczkowski): Is this enough?
    DCHECK(cached_passphrase_.empty());
    cached_passphrase_ = password;
    ConsumeCachedPassphraseIfPossible();
  }
...
void ProfileSyncService::JustDoSync(DoSyncMode mode) {
  DCHECK(opera::SyncConfig::ShouldSynchronizePasswords());
  ...
  syncer::ModelTypeSet final_enabled_types;
  syncer::ModelTypeSet sensitive_types =
      opera::SyncConfig::OperaSensitiveTypes();

  switch (mode) {
  case DO_SYNC_ALL:
    final_enabled_types = opera::SyncConfig::OperaUserTypes();
    break;
  case DO_SYNC_NEW:
  case DO_SYNC_ALL_INSENSITIVE: {
    syncer::ModelTypeSet currently_enabled_types = GetPreferredDataTypes();
    syncer::ModelTypeSet newly_enabled_types = GetUnconfiguredOperaUserTypes();
    final_enabled_types.PutAll(currently_enabled_types);
    final_enabled_types.PutAll(newly_enabled_types);
    }
    break;
  default:
    NOTREACHED();
  ...
  if (mode == DO_SYNC_ALL_INSENSITIVE)
    final_enabled_types.RemoveAll(sensitive_types);

  ChangePreferredDataTypes(final_enabled_types);

  NotifyObservers();

  if (mode == DO_SYNC_ALL_INSENSITIVE)
    ClearConfiguredDataTypes(opera::SyncConfig::OperaSensitiveTypes());
}

void ProfileSyncService::SetLoginErrorData(
  const opera::SyncLoginErrorData& login_error_data) {
  account()->SetLoginErrorData(login_error_data);
}
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
void ProfileSyncService::ClearLoginError() {
  // Don't clear e.g. CONNECTION_FAILED error. Logging in/out does not
  // mean the network is back.
  if (GetAuthError().state() == AuthError::INVALID_GAIA_CREDENTIALS ||
      GetAuthError().state() == TOKEN_INVALID_ERROR)
    UpdateAuthErrorState(AuthError::AuthErrorNone());
}
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
  // 1. For Opera OAuth1 we need this to be always nonempty;
  // 2. For Opera OAuth2 we need this to be the actual token.
  std::string access_token_ = "fake";
  if (opera::SyncConfig::UsesOAuth2()) {
    if (opera_access_token_) {
      access_token_ = opera_access_token_->token();
    }
  }
#endif  // OPERA_SYNC
...
void ProfileSyncService::StopAndSuppress() {
  ...
  UMA_HISTOGRAM_BOOLEAN("Opera.DSK.Sync.StopAndSuppress", true);
}
#endif  // OPERA_SYNC
