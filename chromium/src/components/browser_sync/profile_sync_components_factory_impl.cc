...
void ProfileSyncComponentsFactoryImpl::RegisterCommonDataTypes(
...
#if defined(OPERA_SYNC)
  const bool history_disabled = !opera::SyncConfig::ShouldSynchronizeHistory();
#else
...
#else
  // In Opera, we enable tab sync regardless of whether history sync is enabled.
  if (!disabled_types.Has(syncer::PROXY_TABS) &&
      opera::SyncConfig::ShouldSynchronizeTabs()) {
#endif  // !OPERA_SYNC
    sync_service->RegisterDataTypeController(
  ...
  if (!disabled_types.Has(syncer::PASSWORDS) &&
      opera::SyncConfig::ShouldSynchronizePasswords()) {
#else
  if (!disabled_types.Has(syncer::PASSWORDS)) {
#endif  // OPERA_SYNC
    sync_service->RegisterDataTypeController(
...
// TODO(pstanek): Pay attention to the above condition when removing this since
// it's not 100% correct for us. We should rather check Account::IsLoggedIn().
#ifdef SYNC_CHROMIUM_ATTACHMENT_SERVICE_NOT_MOVED
    ...
    attachment_downloader = syncer::AttachmentDownloader::Create(
        ...
        user_share.sync_credentials.scope_set, token_service_provider,
        store_birthday, model_type);
#endif  // SYNC_CHROMIUM_ATTACHMENT_SERVICE_NOT_MOVED
  }
