...
class BookmarkModel : public BookmarkUndoProvider,
 ...
 public:
  ...
  // Returns the permanent custom node with given |custom_node_name|.
  // Returns NULL when not loaded yet.
  const BookmarkPermanentNode* CustomPermanentNode(
      const std::string& custom_node_name);

  const CustomBookmarkNodes& custom_nodes() const { return custom_nodes_; }
  ...
  // Controls the node persistence.
  void SetTransient(const BookmarkNode* node, bool transient);
  ...
  // If there is any scheduled changes to storage, commit them now
 ...
 private:
  ...
  // Custom nodes. Unlike custom nodes added to |root_| these nodes are
  // saved/loaded to/from storage.
  CustomBookmarkNodes custom_nodes_;
