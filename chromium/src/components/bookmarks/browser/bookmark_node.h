...
class BookmarkNode : public ui::TreeNode<BookmarkNode>,
 ...
 private:
  ...
  // Transient bookmarks are treated as temporary and not saved to persistent
  // storage. Default root nodes cannot be made transient. Only custom root
  // nodes and regular nodes can be transient.
  bool transient_;
...
typedef std::map<std::string, BookmarkPermanentNode*> CustomBookmarkNodes;
