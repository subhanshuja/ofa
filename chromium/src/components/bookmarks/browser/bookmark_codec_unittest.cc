 ...
 protected:
  ...
  bool Decode(BookmarkCodec* codec,
    ...
    CustomBookmarkNodes* custom_nodes =
        const_cast<CustomBookmarkNodes*>(&model->custom_nodes());
...
TEST_F(BookmarkCodecTest, CustomBookmarkNodes) {
  std::unique_ptr<BookmarkModel> model(CreateTestModel1());
  const BookmarkPermanentNode* node1 =
      model->CustomPermanentNode("custom_node_1");
  ASSERT_TRUE(node1);
  model->AddURL(node1, 0, ASCIIToUTF16(kUrl1Title), GURL(kUrl1Url));
  model->AddURL(node1, 0, ASCIIToUTF16(kUrl2Title), GURL(kUrl2Url));
  const BookmarkPermanentNode* node2 =
      model->CustomPermanentNode("custom_node_2");
  ASSERT_TRUE(node2);
  model->AddURL(node2, 0, ASCIIToUTF16(kUrl3Title), GURL(kUrl3Url));
  model->AddURL(node2, 0, ASCIIToUTF16(kUrl4Title), GURL(kUrl4Url));

  std::string checksum;
  std::unique_ptr<base::Value> encoded_value(
      EncodeHelper(model.get(), &checksum));
  ASSERT_TRUE(encoded_value);

  std::string decoded_checksum;
  std::unique_ptr<BookmarkModel> model2(
      DecodeHelper(*encoded_value.get(), checksum, &decoded_checksum, false));
}

TEST_F(BookmarkCodecTest, TransientNodes) {
  std::unique_ptr<BookmarkModel> model(CreateTestModel1());
  const BookmarkPermanentNode* node1 =
      model->CustomPermanentNode("custom_node_1");
  ASSERT_TRUE(node1);
  model->AddURL(node1, 0, ASCIIToUTF16(kUrl2Title), GURL(kUrl2Url));
  model->AddURL(
      model->other_node(), 0, ASCIIToUTF16(kUrl1Title), GURL(kUrl1Url));
  const BookmarkPermanentNode* node2 =
      model->CustomPermanentNode("custom_node_2");
  ASSERT_TRUE(node2);
  ...
  model->SetTransient(node1, true);
  model->SetTransient(node3, true);
  ...
  EXPECT_EQ(1U, model2->custom_nodes().size());
  const BookmarkPermanentNode* model2_node2 =
      model2->CustomPermanentNode("custom_node_2");
  EXPECT_EQ(1, model2_node2->child_count());
  EXPECT_EQ(1, model2->other_node()->child_count());
}
