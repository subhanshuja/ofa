...
void LoadCallback(const base::FilePath& path,
  ...
  if (bookmark_file_exists) {
    ...
    if (root.get()) {
      ...
      codec.Decode(details->bb_node(), details->other_folder_node(),
                   details->mobile_folder_node(), details->custom_nodes(),
  ...
  if (load_index) {
    ...
    for (CustomBookmarkNodes::const_iterator iter =
             details->custom_nodes()->begin();
         iter != details->custom_nodes()->end();
         ++iter)
      AddBookmarksToIndex(details.get(), iter->second);
...
BookmarkLoadDetails::~BookmarkLoadDetails() {
  base::STLDeleteValues(&custom_nodes_);
}
...
void BookmarkStorage::CommitPendingSave() {
  if (writer_.HasPendingWrite())
    SaveNow();
}
