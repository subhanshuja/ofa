...
const char BookmarkCodec::kCustomRoot[] = "custom_root";
...
base::Value* BookmarkCodec::Encode(
    ...
    const CustomBookmarkNodes& custom_nodes,
  ...
  if (!custom_nodes.empty())
    roots->Set(kCustomRoot, EncodeCustomNodes(custom_nodes));
...
bool BookmarkCodec::Decode(BookmarkNode* bb_node,
                           ...
                           CustomBookmarkNodes* custom_nodes,
                           int64_t* max_id,
  ...
  if (!ids_valid_ || computed_checksum() != stored_checksum())
    ReassignIDs(bb_node, other_folder_node, mobile_folder_node, custom_nodes);
...
base::Value* BookmarkCodec::EncodeCustomNodes(
    const CustomBookmarkNodes& nodes) {
  base::DictionaryValue* custom_root = new base::DictionaryValue();
  CustomBookmarkNodes::const_iterator node = nodes.begin();
  while (node != nodes.end()) {
    if (!node->second->transient())
      custom_root->Set(node->first, EncodeNode(node->second));
    ++node;
  }
...
bool BookmarkCodec::DecodeHelper(BookmarkNode* bb_node,
  ...
  DecodeCustomNodes(*roots_d_value, custom_nodes);
...
void BookmarkCodec::DecodeCustomNodes(const base::DictionaryValue& root,
                                      CustomBookmarkNodes* custom_nodes) {
  const base::Value* custom_root = NULL;
  if (!root.Get(kCustomRoot, &custom_root) ||
      custom_root->GetType() != base::Value::TYPE_DICTIONARY)
    return;
  const base::DictionaryValue* custom_d_root =
    static_cast<const base::DictionaryValue*>(custom_root);
  base::DictionaryValue::Iterator custom_folders(*custom_d_root);
  while (!custom_folders.IsAtEnd()) {
    const base::DictionaryValue* custom_folder = NULL;
    if (custom_folders.value().GetAsDictionary(&custom_folder)) {
      std::unique_ptr<BookmarkPermanentNode> new_node(
          new BookmarkPermanentNode(0));
      if (DecodeNode(*custom_folder, NULL, new_node.get()))
        (*custom_nodes)[custom_folders.key()] = new_node.release();
    }
    custom_folders.Advance();
  }
}
...
void BookmarkCodec::ReassignIDs(BookmarkNode* bb_node,
  ...
  for (CustomBookmarkNodes::iterator node = custom_nodes->begin();
       node != custom_nodes->end();
       ++node)
    ReassignIDsHelper(node->second);
