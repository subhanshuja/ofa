...
void BookmarkModel::SetTransient(const BookmarkNode* node, bool transient) {
  FOR_EACH_OBSERVER(
      BookmarkModelObserver, observers_, OnWillChangeBookmarkNode(this, node));

  AsMutable(node)->set_transient(transient);

  if (store_.get())
    store_->ScheduleSave();

  FOR_EACH_OBSERVER(
      BookmarkModelObserver, observers_, BookmarkNodeChanged(this, node));
}
...
const BookmarkPermanentNode* BookmarkModel::CustomPermanentNode(
    const std::string& custom_node_name) {
  if (!loaded_)
    return NULL;
  CustomBookmarkNodes::const_iterator i = custom_nodes_.find(custom_node_name);
  if (i == custom_nodes_.end()) {
    auto new_node =
        base::MakeUnique<BookmarkPermanentNode>(generate_next_node_id());
    BookmarkPermanentNode* node_ptr = new_node.get();
    new_node->set_type(BookmarkNode::FOLDER);
    root_.Add(std::move(new_node), root_.child_count());
    custom_nodes_[custom_node_name] = node_ptr;
    return node_ptr;
  }
...
void BookmarkModel::DoneLoading(std::unique_ptr<BookmarkLoadDetails> details) {
  ...
  custom_nodes_ = details->release_custom_nodes();
  ...
  CustomBookmarkNodes::const_iterator node = custom_nodes_.begin();
  int next_root_pos = root_.child_count();
  while (node != custom_nodes_.end()) {
    root_.Add(base::WrapUnique(node->second), next_root_pos++);
    ++node;
  }
...
void BookmarkModel::CommitPendingChanges() {
  if (loaded_ && store_)
    store_->CommitPendingSave();
}
