...
TEST_F(BookmarkModelTest, AddCustomPermanentNode) {
  // Add custom node.
  EXPECT_EQ(3, model_->root_node()->child_count());
  EXPECT_TRUE(model_->CustomPermanentNode("custom_node"));
  AssertObserverCount(0, 0, 0, 0, 0, 0, 0, 0, 0);

  // Custom node already exists. This should not change the
  // model_| state.
  EXPECT_EQ(4, model_->root_node()->child_count());
  EXPECT_TRUE(model_->CustomPermanentNode("custom_node"));
  EXPECT_EQ(4, model_->root_node()->child_count());
}

TEST_F(BookmarkModelTest, TransientNode) {
  const BookmarkNode* root = model_->bookmark_bar_node();
  const base::string16 title(ASCIIToUTF16("foo"));
  const GURL url("http://foo.com");

  const BookmarkNode* new_node = model_->AddURL(root, 0, title, url);
  EXPECT_FALSE(new_node->transient());
  model_->SetTransient(new_node, true);
  AssertObserverCount(1, 0, 0, 1, 0, 0, 1, 0, 0);
  EXPECT_TRUE(new_node->transient());
}
