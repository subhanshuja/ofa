...
BidiRun* LayoutBlockFlow::computeInlineDirectionPositionsForSegment(
  ...
  LayoutUnit originalAvailableLogicalWidth = availableLogicalWidth;
  limitForTextWrap(availableLogicalWidth);

  expansions.computeExpansionsForJustifiedText(
      firstRun, trailingSpaceRun, totalLogicalWidth, availableLogicalWidth);

  if (document().textWrapEnabled() && !style()->isLeftToRightDirection() &&
      textAlign == JUSTIFY &&
      originalAvailableLogicalWidth > availableLogicalWidth &&
      originalAvailableLogicalWidth > totalLogicalWidth) {
    // Justified and wrapped blocks in RTL mode should be pushed to the block's right border
    // instead of being kept near the left one.
    logicalLeft += (originalAvailableLogicalWidth - totalLogicalWidth);
  }
