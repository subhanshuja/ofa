...
void LayoutBlock::limitForTextWrap(LayoutUnit& outWidth) {
  if (document().textWrapEnabled()) {
    const LayoutUnit maxWidth =
        LayoutUnit(view()->frameView()->textWrapWidth());
    if (maxWidth > 0)
      outWidth = std::min(outWidth, maxWidth);
  }
}
