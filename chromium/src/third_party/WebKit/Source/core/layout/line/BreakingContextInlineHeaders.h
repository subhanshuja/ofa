...
// More negative text-indent than this disables text-wrap.
const int cTextWrapTextIndentLimit = -9998;
...
class BreakingContext {
 ...
 public:
  BreakingContext(InlineBidiResolver& resolver,
      ...
      : m_resolver(resolver),
        ...
        m_replacedBoxHeight(0),
        m_blockHasTextToWrap(false),
        m_textWrapEnabled(block.document().textWrapEnabled()) {
    if (m_textWrapEnabled) {
      // Look for a somewhat lengthy bit of text in the block.
      // Avoid wrapping likely menu items.
      // FIXME: Don't do this. Don't decide on block level whether to wrap or not.
      for (InlineWalker walker(m_block); !walker.atEnd(); walker.advance()) {
        LineLayoutItem o = walker.current();
        Node* node = o.node();

        if (o.isText() && node && node->isTextNode()) {
          ...
          if (length > 25 ||
            ...
            m_blockHasTextToWrap = true;
            break;
          }
        }
      }

      if (m_blockHasTextToWrap) {
        // Heuristics: disable text wrap for blocks indented by text-indent in such a way
        // that text is not supposed be shown and lines are not supposed to be broken. See
        // GREEN-178 for motivation.
        if (m_blockStyle->getTextIndentLine() == TextIndentFirstLine &&
            m_blockStyle->getTextIndentType() == TextIndentNormal &&
            m_block.textIndentOffset() < LayoutUnit(cTextWrapTextIndentLimit))
          m_blockHasTextToWrap = false;
      }
    }
 ...
 private:
  ...
  LayoutUnit m_replacedBoxHeight;
  bool m_blockHasTextToWrap;
  bool m_textWrapEnabled;
...
inline void BreakingContext::increment() {
  m_width.updateAvailableWidth(m_replacedBoxHeight);
...
inline void BreakingContext::handleReplaced() {
  ...
  if (m_atStart) {
    m_replacedBoxHeight = replacedBox.logicalHeight();
    m_width.updateAvailableWidth(m_replacedBoxHeight);
  }
...
inline bool BreakingContext::handleText(WordMeasurements& wordMeasurements,
  ...
  bool limitLineWidthForTextWrap = m_blockHasTextToWrap && m_textWrapEnabled;

  // Don't wrap text in certain kind of containers.
  LineLayoutItem container = m_current.getLineLayoutItem().container();
  while (limitLineWidthForTextWrap && container) {
    if (container.isTextField() || container.isTextArea() ||
        container.isMenuList())
      limitLineWidthForTextWrap = false;
    container = container.container();
  }

  m_width.updateAvailableWidth(m_replacedBoxHeight, limitLineWidthForTextWrap);
  ...
  bool breakWords =
      (m_currentStyle->breakWords() ||
       (limitLineWidthForTextWrap && !m_currentStyle->getTextOverflow())) &&
