...
void FrameView::textLayoutSettingsChanged() const {
  Page* page = frame().page();
  if (page)
    page->chromeClient().textLayoutSettingsChanged();
}
...
void FrameView::setTextWrapWidth(int textWrapWidth) {
  if (m_textWrapWidth == textWrapWidth)
    return;

  m_textWrapWidth = textWrapWidth;

  for (LayoutObject* renderer = layoutView(); renderer;
       renderer = renderer->nextInPreOrder()) {
    if (!renderer->isLayoutBlock())
      continue;

    renderer->setNeedsLayout(LayoutInvalidationReason::SizeChanged);
  }
}

void FrameView::setTextWrapWidthRecursively(int textWrapWidth) {
  for (Frame* frame = m_frame.get(); frame;
       frame = frame->tree().traverseNext(m_frame.get())) {
    if (frame->isLocalFrame()) {
      FrameView* view = toLocalFrame(frame)->view();
      if (view)
        view->setTextWrapWidth(textWrapWidth);
    }
  }
}
