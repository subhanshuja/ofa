...
void Document::suspendAudio() {
    HTMLMediaElement::documentSuspendAudio(this);
}

void Document::resumeSuspendedAudio() {
    HTMLMediaElement::documentResumeSuspendedAudio(this);
}

void Document::clearSuspendedAudio() {
    HTMLMediaElement::documentClearSuspendedAudio(this);
}

bool Document::hasPlayingAudio() {
    return HTMLMediaElement::documentHasPlayingAudio(this);
}

bool Document::hasSuspendedAudio() {
    return HTMLMediaElement::documentHasSuspendedAudio(this);
}
...
bool Document::textWrapEnabled() const {
  if (Document* parent = parentDocument())
    return parent->textWrapEnabled();

  if (!settings() || !settings()->textWrappingEnabled())
    return false;
  ...
  PageScaleConstraints constraints =
      frameHost()->pageScaleConstraintsSet().finalConstraints();
  return constraints.minimumScale != constraints.maximumScale;
}
