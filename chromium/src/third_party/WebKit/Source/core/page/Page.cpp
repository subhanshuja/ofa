...
Page::Page(PageClients& pageClients)
    : SettingsDelegate(Settings::create()),
      ...
      m_playStateTimer(this, &Page::playStateTimerFired),
      m_playState(AudioInactive),
#if ENABLE(ASSERT)
      m_isPainting(false),
#endif
...
void Page::settingsChanged(SettingsDelegate::ChangeType changeType) {
  switch (changeType) {
    ...
    case SettingsDelegate::TextAutosizingChange:
      if (!mainFrame() || !mainFrame()->isLocalFrame())
        break;
      if (mainFrame()->isLocalFrame() && toLocalFrame(mainFrame())->view())
        toLocalFrame(mainFrame())->view()->textLayoutSettingsChanged();
      if (TextAutosizer* textAutosizer =
              deprecatedLocalMainFrame()->document()->textAutosizer())
        textAutosizer->updatePageInfoInAllFrames();
      break;
...
void Page::invalidatePlayState() {
  if (!m_playStateTimer.isActive())
    m_playStateTimer.startOneShot(0, BLINK_FROM_HERE);
}

void Page::playStateTimerFired(TimerBase*) {
  // Based on Document::hasPlayingAudio and ::hasSuspendedAudio for all the
  // documents in the page, calculate the page-wide play state. When there are
  // both playing and suspended audio elements, clear the suspend state of all
  // documents. This is done so that a user manually playing an audio element
  // will forget about the old state and not later try to resume playback of
  // prior suspended elements.

  bool hasPlayingAudio = false;
  bool hasSuspendedAudio = false;
  ...
  for (Frame* frame = mainFrame();
    ...
    if (Document* document = toLocalFrame(frame)->document()) {
      if (document->hasPlayingAudio())
        hasPlayingAudio = true;
      if (document->hasSuspendedAudio())
        hasSuspendedAudio = true;
    }
  }

  if (hasPlayingAudio && hasSuspendedAudio) {
    for (Frame* frame = mainFrame();
         frame;
         frame = frame->tree().traverseNext()) {
      if (!frame->isLocalFrame())
      ...
      if (Document* document = toLocalFrame(frame)->document())
        document->clearSuspendedAudio();
    }
    hasSuspendedAudio = false;
  }

  ASSERT(!m_playStateTimer.isActive());

  WebAudioElementPlayState playState;
  if (hasPlayingAudio)
    playState = AudioPlaying;
  else if (hasSuspendedAudio)
    playState = AudioSuspended;
  else
    playState = AudioInactive;

  if (m_playState != playState) {
    m_playState = playState;
    chromeClient().playStateChanged(playState);
  }
}
