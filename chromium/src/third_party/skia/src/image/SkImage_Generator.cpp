...
public:
    SkImage_Generator(SkImageCacherator* cache, bool decodedWithLosslessCodec)
        : INHERITED(cache->info().width(), cache->info().height(), cache->uniqueID())
        , fCache(cache) // take ownership
        , fDecodedWithLosslessCodec(decodedWithLosslessCodec)
    {}
    ...
    bool isDecodedWithLosslessCodec() const override { return fDecodedWithLosslessCodec; }
    void setIsDecodedWithLosslessCodecForTest(bool is_losless) override { fDecodedWithLosslessCodec = is_losless; }

private:
    SkAutoTDelete<SkImageCacherator> fCache;
    bool fDecodedWithLosslessCodec;
...
sk_sp<SkImage> SkImage::MakeFromGenerator(SkImageGenerator* generator, const SkIRect* subset, bool decodedWithLosslessCodec) {
    if (!generator) {
        return nullptr;
    }
    ...
    return sk_make_sp<SkImage_Generator>(cache, decodedWithLosslessCodec);
}
