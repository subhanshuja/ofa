...
#if defined(OPERA_SYNC)
          const buzz::XmlElement* reason = element->FirstElement();
          if (reason) {
            if (reason->Name() == buzz::QN_SASL_TEMPORARY_AUTH_FAILURE) {
              return Failure(XmppEngine::ERROR_TEMPORARY_AUTH_FAILURE);
            } else if (reason->Name() == buzz::QN_SASL_NOT_AUTHORIZED) {
              const std::string& reason_code = reason->BodyText();
              int reason_code_int = -1;
              if (!base::StringToInt(reason_code, &reason_code_int)) {
                LOG(WARNING) << "Auth error code from push service unreadable: '" << reason_code << "'";
              }
              return Failure(XmppEngine::ERROR_UNAUTHORIZED, reason_code_int);
            } else {
              LOG(WARNING) << "Unknown failure reason! Treating as generic failure.";
            }
          }
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
bool
XmppLoginTask::Failure(XmppEngine::Error reason, int subcode) {
  state_ = LOGINSTATE_DONE;
