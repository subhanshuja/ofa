...
#if defined(OPERA_SYNC)
struct OperaNotificationsDisabledReason {
  explicit OperaNotificationsDisabledReason(
      NotificationsDisabledReason new_reason) {
    reason = new_reason;
  }

  OperaNotificationsDisabledReason(
      NotificationsDisabledReason new_reason,
      opera_sync::OperaAuthProblem new_opera_auth_problem) {
    reason = new_reason;
    opera_auth_problem = new_opera_auth_problem;
  }

  bool operator==(const OperaNotificationsDisabledReason a) {
    return reason == a.reason;
  }

  friend bool operator==(const OperaNotificationsDisabledReason& a,
                         const OperaNotificationsDisabledReason& b);

  NotificationsDisabledReason reason;
  opera_sync::OperaAuthProblem opera_auth_problem;
};
#endif  // OPERA_SYNC
...
#if defined(OPERA_SYNC)
      OperaNotificationsDisabledReason reason) = 0;
#else
