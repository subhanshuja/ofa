...
void XmppPushClient::OnTransientDisconnection() {
  DCHECK(thread_checker_.CalledOnValidThread());
  VLOG(1) << "Push: Transient disconnection";
  base_task_.reset();
#if defined(OPERA_SYNC)
  FOR_EACH_OBSERVER(PushClientObserver, observers_,
                    OnNotificationsDisabled(OperaNotificationsDisabledReason(
...
void XmppPushClient::OnCredentialsRejected(
  ...
  DCHECK(thread_checker_.CalledOnValidThread());
  VLOG(1) << "Push: Credentials rejected";
  base_task_.reset();
  login_.reset();  // Opera-specific change
  FOR_EACH_OBSERVER(PushClientObserver, observers_,
                    OnNotificationsDisabled(OperaNotificationsDisabledReason(
                        NOTIFICATION_CREDENTIALS_REJECTED, auth_problem)));
}

void XmppPushClient::OnAuthDataUnavailable() {
  OnTransientDisconnection();
}
#else
void XmppPushClient::OnCredentialsRejected() {
  DCHECK(thread_checker_.CalledOnValidThread());
...
#if defined(OPERA_SYNC)
    login_->SetTokenGenerator(notifier_options_.token_generator);
#endif  // OPERA_SYNC
