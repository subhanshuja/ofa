...
#if defined(OPERA_SYNC)
    if (notifier_options.fallback_port > 0) {
      net::HostPortPair fallback(notifier_options.xmpp_host_port);
      fallback.set_port(notifier_options.fallback_port);
      servers.push_back(ServerInformation(fallback, DOES_NOT_SUPPORT_SSLTCP));
    }
#endif  // OPERA_SYNC
  } else {
