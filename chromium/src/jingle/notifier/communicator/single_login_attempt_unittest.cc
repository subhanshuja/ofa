...
#if defined(OPERA_SYNC)
void TokenGenerator(const opera::TokenReceiver& receiver_callback) {
  receiver_callback.Run("unittest_token");
}

void EmptyTokenGenerator(const opera::TokenReceiver& receiver_callback) {
...
#if defined(OPERA_SYNC)
  void EnableEmptyTokenGenerator() {
    attempt_.reset(new SingleLoginAttempt(login_settings_, &fake_delegate_,
        base::Bind(&EmptyTokenGenerator)));
  }
#endif  // OPERA_SYNC
...
TEST_F(SingleLoginAttemptTest, EmptyToken) {
  // Check that a reconnect attempt performed while the token
  // generator returns an empty string is served correctly.
  EnableEmptyTokenGenerator();
  attempt_->OnError(buzz::XmppEngine::ERROR_NONE, 0, NULL);
  EXPECT_EQ(AUTH_DATA_UNAVAILABLE, fake_delegate_.state());
}
