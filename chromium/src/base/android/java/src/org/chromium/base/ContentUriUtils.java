...
public abstract class ContentUriUtils {
    ...
    public static int openContentUriForRead(Context context, String uriString) {
        ParcelFileDescriptor pfd = getParcelFileDescriptor(context, uriString, "r");
        if (pfd != null) {
            return pfd.detachFd();
        }
        return -1;
    ...
    /**
     * Opens the content URI for writing, and returns the file descriptor to
     * the caller. The caller is responsible for closing the file desciptor.
     *
     * @param context {@link Context} in interest
     * @param uriString the content URI to open
     ...
     */
    @CalledByNative
    public static int openContentUriForWrite(Context context, String uriString) {
        // Request read-write mode to ensure that we don't get a pipe.
        ParcelFileDescriptor pfd = getParcelFileDescriptor(context, uriString, "rw");
        if (pfd != null) {
            return pfd.detachFd();
        }
        return -1;
    ...
    public static boolean contentUriExists(Context context, String uriString) {
        return getParcelFileDescriptor(context, uriString, "r") != null;
    }
     ...
     * @param context {@link Context} in interest.
     * @param uriString the content URI to open.
     * @param mode file access mode, i.e. "r" or "w".
