...
public class Log {
    ...
    private static String getCallOrigin() {
        ...
        // However since ProGuard can inline functions we don't know the exact number of
        // steps inside the Log class and have to walk the stack until we find the caller.

        int callerStackIndex = 0;
        String logClassName = Log.class.getName();
        loop:
        for (int i = 0; i < st.length; i++) {
            if (st[i].getClassName().equals(logClassName)) {
                for (callerStackIndex = i + 1; callerStackIndex < st.length; callerStackIndex++) {
