...
void BuildBlacklist(TurboBlacklist* blacklist,
  ...
  std::unique_ptr<StringTimeMap> scoped_block(block);
...
void TurboBlacklistManager::Block(const std::string& value,
                                  base::Time expiry) const {
  DCHECK(BrowserThread::CurrentlyOn(BrowserThread::IO));
...
void TurboBlacklistManager::AddBlockPrefOnUI(const std::string& value,
                                             base::Time expiry) const {
  DCHECK(BrowserThread::CurrentlyOn(BrowserThread::UI));
  ...
  if (dict) {
    int64_t t = expiry.ToInternalValue();
    dict->SetWithoutPathExpansion(
        value,
        new base::StringValue(base::Int64ToString(t)));
  }
  return;
}
...
void TurboBlacklistManager::RemoveBlockPrefOnUI(const std::string& value) const {
  DCHECK(BrowserThread::CurrentlyOn(BrowserThread::UI));
  const base::DictionaryValue* cdict =
      pref_service_->GetDictionary(prefs::kTurboUrlBlacklist);
