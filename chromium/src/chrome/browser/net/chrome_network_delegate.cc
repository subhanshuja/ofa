...
void ChromeNetworkDelegate::set_turbo_settings(
    scoped_refptr<TurboSettings> turbo_settings) {
...
void ChromeNetworkDelegate::InitializePrefsOnUIThread(
    ...
    StringPrefMember* turbo_client_id,
  ...
  if (turbo_client_id) {
    turbo_client_id->Init(prefs::kTurboClientID, pref_service);
    turbo_client_id->MoveToThread(
        BrowserThread::GetTaskRunnerForThread(BrowserThread::IO));
  }
...
int ChromeNetworkDelegate::OnBeforeURLRequest(
  ...
  // '!turbo_settings_' => system or incognito delegate,
  // in which we disable Turbo.
  if (turbo_settings_ && content::ResourceRequestInfo::GetRenderFrameForRequest(
                             request, &render_process_id, &render_frame_id))
    use_turbo = turbo_settings_->IsFrameHostUsingTurbo(render_process_id,
                                                       render_frame_id);

  use_turbo =
      use_turbo && (!turbo_blacklist_manager_ ||
                    !turbo_blacklist_manager_->IsURLBlocked(request->url()));
  if (!use_turbo)
...
int ChromeNetworkDelegate::OnHeadersReceived(
  ...
  if (net::HttpStreamFactory::turbo_enabled() &&
      original_response_headers->HasHeader("x-opera-info") &&
      original_response_headers->response_code() == 503) {
    std::string value;
    base::Time expiry_time = base::Time::Now() +
                             base::TimeDelta::FromSeconds(86400);
    if (original_response_headers->EnumerateHeader(NULL,
                                                   "retry-after", &value)) {
      int64_t expiry;
      base::StringToInt64(value, &expiry);
      expiry_time = base::Time::Now() + base::TimeDelta::FromSeconds(expiry);
    }

    size_t iter = 0;
    if (original_response_headers->EnumerateHeader(&iter, "x-turbo-bypass",
                                                   &value)) {
      if (turbo_blacklist_manager_)
        turbo_blacklist_manager_->Block(value, expiry_time);
    }

    return net::ERR_TURBO_BYPASS;
  }
...
void ChromeNetworkDelegate::OnTurboSession(
    bool is_secure,
  ...
  if (turbo_client_id_) {
    std::string stored_base64(turbo_client_id_->GetValue());
    std::string stored_client_id;

    // If the preference holds a valid base64 encoded client id, "return" that
    // instead, overriding the one generated by the network layer.
    if (!stored_base64.empty() &&
        base::Base64Decode(stored_base64, &stored_client_id) &&
        stored_client_id.size() == client_id->size()) {
      *client_id = stored_client_id;
      return;
    }

    // Otherwise, store the new one in the preference.
    base::Base64Encode(*client_id, &stored_base64);
    if (!stored_base64.empty())
      turbo_client_id_->SetValue(stored_base64);
  }
}
