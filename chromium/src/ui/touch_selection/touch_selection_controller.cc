...
void TouchSelectionController::OnSelectionBoundsChanged(
  ...
  if (is_selection_dragging) {
    client_->OnTouchHandleEvent(
        TOUCH_HANDLE_DRAG_MOVE,
        gfx::ToEnclosingRect(is_start_anchor_ ?
            gfx::BoundingRect(end_.edge_top(), end_.edge_bottom()) :
            gfx::BoundingRect(start_.edge_top(), start_.edge_bottom())));
  }
...
void TouchSelectionController::OnDragBegin(
  ...
  client_->OnTouchHandleEvent(TOUCH_HANDLE_DRAG_BEGIN, gfx::Rect());
