...
SkBitmap CreateSkBitmapFromJavaBitmap(const JavaBitmap& jbitmap) {
  ...
  switch (jbitmap.format()) {
    case ANDROID_BITMAP_FORMAT_RGBA_8888:
      if (!skbitmap.tryAllocPixels(SkImageInfo::MakeN32Premul(src_size.width(),
                                                              src_size.height()),
                                   jbitmap.stride())) {
        return skbitmap;
      }
    ...
    case ANDROID_BITMAP_FORMAT_A_8:
      if (!skbitmap.tryAllocPixels(SkImageInfo::MakeA8(src_size.width(),
                                                       src_size.height()),
                                   jbitmap.stride())) {
        return skbitmap;
      }
      break;
    default:
      DCHECK(false) << "Invalid Java bitmap format: " << jbitmap.format();
      return skbitmap;
  }
