...
class SK_API AnalysisCanvas : public SkCanvas, public SkPicture::AbortCallback {
 public:
  enum AnalysisFlags {
    ANALYZE_SOLID_COLOR = 1 << 0,
    ANALYZE_LOSSLESS_BITMAP = 1 << 1,
    ANALYZE_GRADIENT = 1 << 2
  };

  AnalysisCanvas(int width, int height, int analyze_flags);
  ~AnalysisCanvas() override;

  // Returns true when a SkColor can be used to represent result.
  bool GetColorIfSolid(SkColor* color) const;
  bool HasLargeLosslessBitmap() const;
 ...
 private:
  ...
  bool has_large_lossless_bitmap_;
  bool has_gradient_;
  int draw_op_count_;
};
