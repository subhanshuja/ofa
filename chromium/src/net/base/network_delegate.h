...
class NET_EXPORT NetworkDelegate : public base::NonThreadSafe {
 public:
  ...
  void NotifyTurboSession(bool is_secure,
  ...
  void NotifyTurboStatistics(uint32_t compressed, uint32_t uncompressed);

  std::string NotifyTurboChooseSession(const GURL& url);
  std::string NotifyTurboGetDefaultServer();
  void NotifyTurboSuggestedServer(const std::string& suggested_server);
 ...
 private:
  ...
  // Called when when a new session with the Turbo proxy has been
  // established.
  // |is_secure| will be true if the Turbo connection is encrypted.
  // The delegate can substitute the client id with its own, if it has one
  // stored, and should set the device id as appropriate (its default value is
  // not meaningful.)
  // If the device is a phone using a mobile network for network communication
  // (i.e. not when using wifi) then it should set the |mcc| (Mobile Country
  // Code) and |mnc| (Mobile Network Code) values to the ones set by the
  // operator on the SIM card.
  // |key| may be set to an additional key used to encrypt private data on the
  // server. It should only be set if the connection is secure.
  // |headers| is a map of (somewhat) free-form headers, collected by the
  // servers to produce more useful statistics.
  virtual void OnTurboSession(bool is_secure,
                              std::string* client_id,
                              std::string* device_id,
                              std::string* mcc,
                              std::string* mnc,
  ...
  // Called when compression statistics is received from the Turbo proxy.
  virtual void OnTurboStatistics(uint32_t compressed, uint32_t uncompressed) {}

  // Called for every resource being loaded over Turbo 2 in order to determine
  // if the request should go on a zero-rating connection or not.
  //
  // The expected return value for a resource that should be loaded via
  // zero-rating is the hostname of the Turbo server to use.
  //
  // Returning an empty string will have the request bypass the zero-rating
  // system, i.e. Turbo will work as it did before the introduction of
  // zero-rating. N.B. An empty string should be returned if the current data
  // connection is not a mobile connection, e.g. when WiFi is enabled.
  virtual std::string OnTurboChooseSession(const GURL& url);

  // Called before connecting to a Turbo 2 proxy. Returns either an empty
  // string, or a proxy URL to be used for the default connection
  // (non-zero-rated, slot 0). Normally, this is the cached SUGGESTED_SERVER
  // previously sent by the proxy. If an empty string is returned, the
  // compile-time default value will be used.
  virtual std::string OnTurboGetDefaultServer();

  // Called whenever the Turbo 2 proxy sends a SUGGESTED_SERVER frame.
  // Also called with an empty suggested_server when there is an error while
  // connecting to Turbo 2.
  virtual void OnTurboSuggestedServer(const std::string& suggested_server) {}
