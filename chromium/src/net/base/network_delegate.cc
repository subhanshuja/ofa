...
void NetworkDelegate::NotifyTurboSession(
    bool is_secure,
  ...
  DCHECK(CalledOnValidThread());
  OnTurboSession(is_secure, client_id, device_id, mcc, mnc, key, headers);
}

void NetworkDelegate::NotifyTurboStatistics(uint32_t compressed,
                                            uint32_t uncompressed) {
  DCHECK(CalledOnValidThread());
  OnTurboStatistics(compressed, uncompressed);
}

std::string NetworkDelegate::NotifyTurboChooseSession(const GURL& url) {
  DCHECK(CalledOnValidThread());
  return OnTurboChooseSession(url);
}

std::string NetworkDelegate::NotifyTurboGetDefaultServer() {
  DCHECK(CalledOnValidThread());
  return OnTurboGetDefaultServer();
}

void NetworkDelegate::NotifyTurboSuggestedServer(
    const std::string& suggested_server) {
  DCHECK(CalledOnValidThread());
  OnTurboSuggestedServer(suggested_server);
}

std::string NetworkDelegate::OnTurboChooseSession(const GURL& url) {
  return "";
}

std::string NetworkDelegate::OnTurboGetDefaultServer() {
  return "";
}
