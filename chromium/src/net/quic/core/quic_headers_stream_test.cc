 ...
 public:
  ...
  MOCK_METHOD3(OnPushPromise,
               void(SpdyStreamId stream_id,
                    ...
                    bool end));
  MOCK_METHOD1(OnTurboAuthRequest, void(const std::string& challenge));

  MOCK_METHOD2(OnTurboStatistics, void(uint32_t compressed,
                                       uint32_t uncompressed));
  MOCK_METHOD1(OnTurboHost, void(const std::string& host));
  MOCK_METHOD1(OnTurboSuggestedServer,
               void(const std::string& suggested_server));
