...
class NET_EXPORT ProxyServer {
 public:
  ...
  ProxyServer() : scheme_(SCHEME_INVALID), turbo_connection_id_(INT_MIN) {}
  ...
  // Returns true if this ProxyServer is a Turbo proxy.
  bool is_turbo() const { return turbo_connection_id_ > 0; }

  int turbo_connection_id() const { return turbo_connection_id_; }
  void set_turbo_connection_id(int id);
  ...
  bool operator==(const ProxyServer& other) const {
    return scheme_ == other.scheme_ &&
           ...
           turbo_connection_id_ == other.turbo_connection_id_;
  }

  // Comparator function so this can be placed in a std::map.
  bool operator<(const ProxyServer& other) const {
    if (turbo_connection_id_ != other.turbo_connection_id_)
      return turbo_connection_id_ < other.turbo_connection_id_;
 ...
 private:
  ...
  // Used to identify separate connections to the same Turbo proxy.
  int turbo_connection_id_;
};
