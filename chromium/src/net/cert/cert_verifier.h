...
class NET_EXPORT CertVerifier {
 public:
  ...
  enum VerifyFlags {
    ...
    // If set (which should be only for connections to Turbo2 proxy servers),
    // require that the server certificate is issued by the Opera Turbo CA.
    // (DER certificate in net/turbo/turbo_root_cert.h.)
    // All built-in or platform root certificates will be ignored.
    VERIFY_TURBO_CERT = 1 << 6,
  };
