...
int rv = 0;
  if (flags & CertVerifier::VERIFY_TURBO_CERT) {
    rv = TurboCertificate::Verify(cert, hostname, verify_result);
  } else {
    rv = VerifyInternal(cert, hostname, ocsp_response, flags, crl_set,
