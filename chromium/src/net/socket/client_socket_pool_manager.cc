...
int InitSocketPoolHelper(ClientSocketPoolManager::SocketGroupType group_type,
  ...
  const bool use_direct = proxy_info.is_direct() ||
                          (using_ssl && proxy_info.proxy_server().is_turbo());
  if (!use_direct) {
    ...
    if (proxy_info.is_http() || proxy_info.is_https()) {
      ...
      if (proxy_info.is_https()) {
        ...
        proxy_tcp_params = new TransportSocketParams(*proxy_host_port,
                                                     ...
                                                     resolution_callback,
                                                     combine_connect_and_write);
        SSLConfig ssl_config = ssl_config_for_proxy;
        ssl_config.verify_turbo_cert = proxy_server.is_turbo();
        // Set ssl_params, and unset proxy_tcp_params
        ssl_params =
            new SSLSocketParams(proxy_tcp_params, NULL, NULL,
                                *proxy_host_port.get(), ssl_config,
      ...
      http_proxy_params =
          new HttpProxySocketParams(proxy_tcp_params,
                                    ...
                                    proxy_server.turbo_connection_id());
    } else {
