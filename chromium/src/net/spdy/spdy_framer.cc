...
size_t SpdyFramer::GetTurboMinimumSize() const {
  // Size, in bytes, of a TURBO frame sans variable-length part.
  return GetFrameHeaderSize() + 1;
}
...
const char* SpdyFramer::StateToString(int state) {
  switch (state) {
    ...
    case SPDY_TURBO_FRAME_PAYLOAD:
      return "SPDY_TURBO_FRAME_PAYLOAD";
...
size_t SpdyFramer::ProcessInput(const char* data, size_t len) {
  ...
  do {
    ...
    switch (state_) {
      ...
      case SPDY_TURBO_FRAME_PAYLOAD: {
        int bytes_read = ProcessTurboFramePayload(data, len);
        len -= bytes_read;
        data += bytes_read;
        break;
      }
...
void SpdyFramer::ProcessControlFrameHeader(int control_frame_type_field) {
  ...
  switch (current_frame_type_) {
    ...
    case PRIORITY:
      ...
      break;
    case TURBO:
      if (current_frame_length_ < GetTurboMinimumSize()) {
        set_error(SPDY_INVALID_CONTROL_FRAME);
      }
      break;
    default:
  ...
  switch (current_frame_type_) {
    ...
    case PUSH_PROMISE:
      ...
      break;
    case TURBO:
      frame_size_without_variable_data = GetTurboMinimumSize();
      break;
    case CONTINUATION:
  ...
  if (remaining_control_header_ == 0) {
    ...
    switch (current_frame_type_) {
      ...
      case CONTINUATION:
        ...
        break;
      case TURBO:
        CHANGE_STATE(SPDY_TURBO_FRAME_PAYLOAD);
        return original_len - len;
      default:
#ifndef NDEBUG
...
size_t SpdyFramer::ProcessTurboFramePayload(const char* data,
                                            size_t data_len) {
  DCHECK_EQ(SPDY_TURBO_FRAME_PAYLOAD, state_);
  DCHECK_EQ(TURBO, current_frame_type_);
  if (protocol_version() == SPDY3) {
    DCHECK_EQ(current_frame_flags_, 0);
  } else {
    DCHECK(current_frame_flags_ & TURBO_FLAG_END_TURBO_FRAME);
  }

  size_t available_bytes = std::min(data_len, remaining_data_length_);

  if (available_bytes < remaining_data_length_) {
    if (!turbo_frame_buffer_.get())
      turbo_frame_buffer_.reset(new char[remaining_data_length_]);
    memcpy(turbo_frame_buffer_.get() + turbo_frame_buffer_length_,
           data, available_bytes);
    turbo_frame_buffer_length_ += available_bytes;
  } else {
    const char* buffer;
    size_t buffer_len;
    ...
    if (!turbo_frame_buffer_.get()) {
      ...
      buffer = turbo_frame_buffer_.get();
      buffer_len = turbo_frame_buffer_length_;
      memcpy(turbo_frame_buffer_.get() + buffer_len, data, available_bytes);
      buffer_len += available_bytes;
    }

    SpdyFrameReader frame_type_reader(current_frame_buffer_.data(),
                                      current_frame_buffer_.len());

    frame_type_reader.Seek(GetFrameHeaderSize());

    uint8_t frame_type;
    if (!frame_type_reader.ReadUInt8(&frame_type)) {
      set_error(SPDY_INVALID_CONTROL_FRAME);
      return available_bytes;
    }

    SpdyFrameReader reader(buffer, buffer_len);

    switch (frame_type) {
      case TURBO_AUTH_REQUEST:
        {
          base::StringPiece challenge;

          if (!reader.ReadStringPiece16(&challenge)) {
            set_error(SPDY_INVALID_CONTROL_FRAME);
            return available_bytes;
          }

          visitor_->OnTurboAuthRequest(challenge.as_string());
        }
        break;
      ...
      case TURBO_STATS:
        {
          ...
          if (!reader.ReadUInt32(&compressed) ||
              !reader.ReadUInt32(&uncompressed)) {
            set_error(SPDY_INVALID_CONTROL_FRAME);
            return available_bytes;
          }

          visitor_->OnTurboStatistics(compressed, uncompressed);
        }
        break;
      ...
      case TURBO_HOST:
        {
          ...
          if (!reader.ReadStringPiece16(&host)) {
            set_error(SPDY_INVALID_CONTROL_FRAME);
            return available_bytes;
          }

          visitor_->OnTurboHost(host.as_string());
        }
        break;

      case TURBO_VERSION_FEATURES:
        {
          uint16_t version_high, version_low;
          uint32_t features;
          base::StringPiece version_features;

          if (!reader.ReadUInt16(&version_high) ||
              !reader.ReadUInt16(&version_low) ||
              !reader.ReadUInt32(&features)) {
            set_error(SPDY_INVALID_CONTROL_FRAME);
            return available_bytes;
          }
          net::HttpStreamFactory::set_turbo_server_features(features);
        }
        break;

      case TURBO_SUGGESTED_SERVER:
        {
          base::StringPiece suggested_server;

          if (!reader.ReadStringPiece16(&suggested_server)) {
            set_error(SPDY_INVALID_CONTROL_FRAME);
            return available_bytes;
          }

          visitor_->OnTurboSuggestedServer(suggested_server.as_string());
        }
        break;

      default:
        // Ignore other frame types, including unknown ones.  The server might
        // implement a newer version of the protocol than us.  Ignoring the
        // frame might not give meaningful behavior, but signalling an error
        // most certainly won't.
        break;
    }
  }

  remaining_data_length_ -= available_bytes;
  if (remaining_data_length_ == 0)
    CHANGE_STATE(SPDY_FRAME_COMPLETE);
...
bool SpdyFramer::ParseHeaderBlockInBuffer(const char* header_data,
  ...
  bool turbo_enabled = net::HttpStreamFactory::turbo_enabled();
...
class FrameSerializationVisitor : public SpdyFrameVisitor {
 public:
  ...
  void VisitTurbo(const SpdyTurboIR& data) override {
    frame_ = framer_->SerializeTurbo(data);
  }
...
SpdySerializedFrame SpdyFramer::CreateFullCacheList(
    const std::vector<TurboCacheEntry>& entries) const {
  SpdyTurboIR turbo;
  turbo.SetFullCacheList(entries);
  return SerializeTurbo(turbo);
}

SpdySerializedFrame SpdyFramer::CreateIncrementalCacheList(
    const std::vector<TurboCacheEntry>& added_entries,
    const std::vector<TurboCacheEntry>& removed_entries) const {
  SpdyTurboIR turbo;
  turbo.SetIncrementalCacheList(added_entries, removed_entries);
  return SerializeTurbo(turbo);
}

SpdySerializedFrame SpdyFramer::CreateUnclaimedPushedStreams(
    const std::vector<SpdyStreamId>& stream_ids) const {
  SpdyTurboIR turbo;
  turbo.SetUnclaimedPushedStreams(stream_ids);
  return SerializeTurbo(turbo);
}

SpdySerializedFrame SpdyFramer::CreateUnverifiedPushedStreams(
    const std::vector<SpdyStreamId>& stream_ids) const {
  SpdyTurboIR turbo;
  turbo.SetUnverifiedPushedStreams(stream_ids);
  return SerializeTurbo(turbo);
}

SpdySerializedFrame SpdyFramer::CreateHello(
  ...
  SpdyTurboIR turbo;
  turbo.SetHello(
      host, uid, brand, platform, product, version, device, mcc, mnc, key,
      headers);
  return SerializeTurbo(turbo);
}

SpdySerializedFrame SpdyFramer::CreateAuthResponse(const std::string& response) const {
  SpdyTurboIR turbo;
  turbo.SetAuthResponse(response);
  return SerializeTurbo(turbo);
}

SpdySerializedFrame SpdyFramer::CreateImageQuality(int quality) const {
  SpdyTurboIR turbo;
  turbo.SetImageQuality(quality);
  return SerializeTurbo(turbo);
}

SpdySerializedFrame SpdyFramer::CreateUsageStats(const std::string& stats,
                                        uint8_t stats_version) const {
  SpdyTurboIR turbo;
  turbo.SetUsageStats(stats, stats_version);
  return SerializeTurbo(turbo);
}

SpdySerializedFrame SpdyFramer::CreateSetCookies(
    const std::string& host, const std::vector<std::string>& cookies) const {
  SpdyTurboIR turbo;
  turbo.SetSetCookies(host, cookies);
  return SerializeTurbo(turbo);
}

SpdySerializedFrame SpdyFramer::CreatePageLoadTime(
    bool turbo_enabled, const std::string& host, uint32_t milliseconds) const {
  SpdyTurboIR turbo;
  turbo.SetPageLoadTime(turbo_enabled, host, milliseconds);
  return SerializeTurbo(turbo);
}

SpdySerializedFrame SpdyFramer::CreateConnectionType(int type) const {
  SpdyTurboIR turbo;
  turbo.SetConnectionType(type);
  return SerializeTurbo(turbo);
}

SpdySerializedFrame SpdyFramer::CreateServerSideLog(bool enable) const {
  SpdyTurboIR turbo;
  turbo.SetServerSideLog(enable);
  return SerializeTurbo(turbo);
}

SpdySerializedFrame SpdyFramer::CreateVersionFeatures(
    uint16_t turbo_protocol_version_high,
    uint16_t turbo_protocol_version_low,
    uint32_t turbo_feature_flags) const {
  SpdyTurboIR turbo;
  turbo.SetVersionFeatures(turbo_protocol_version_high,
                           turbo_protocol_version_low,
                           turbo_feature_flags);
  return SerializeTurbo(turbo);
}

SpdySerializedFrame SpdyFramer::SerializeTurbo(const SpdyTurboIR& data) const {
  size_t size = GetTurboMinimumSize();

  std::vector<std::string>::const_iterator cookies_iter;

  switch (data.type()) {
    case TURBO_CACHE_INCREMENTAL:
      size += 4 + data.removed_entries()->size() * 4;

    case TURBO_CACHE_FULL:
      size += 4 + data.added_entries()->size() * 4;
      break;

    case TURBO_SET_COOKIE:
      // Obsolete; superseded by TURBO_SET_COOKIES.
      NOTREACHED();
      break;

    case TURBO_UNCLAIMED_PUSHED_STREAMS:
    case TURBO_UNVERIFIED_PUSHED_STREAMS:
      size += 4 + data.stream_ids()->size() * 4;
      break;

    case TURBO_HELLO: {
      // Every string is prepended by its length as a 2 byte int. We
      // therefore must reserve 2 * the number of data fields in 'data' in
      // addition to the actual data.
      const int kDataFieldsSizeTotalLength = 18;
      size +=
          (kDataFieldsSizeTotalLength + data.host().size() + data.uid().size() +
           data.brand().size() + data.platform().size() +
           data.product().size() + data.version().size() +
           data.device().size() + data.mcc().size() + data.mnc().size());
      size += 2 + data.key().size();
      size += 2;
      std::map<std::string, std::string>::const_iterator it;
      for (it = data.headers().begin(); it != data.headers().end(); ++it) {
        size += 4 + it->first.size() + it->second.size();
      }
      break;
    }

    case TURBO_AUTH_RESPONSE:
      size += 2 + data.response().size();
      break;

    case TURBO_IMAGE_QUALITY:
      size += 1;
      break;

    case TURBO_USAGE_STATS:
      size += 5 + data.usage_stats().size();
      break;

    case TURBO_SET_COOKIES:
      size += 3 + data.host().size();
      for (cookies_iter = data.cookies()->begin();
           cookies_iter != data.cookies()->end();
           ++cookies_iter)
        size += 2 + cookies_iter->size();
      break;

    case TURBO_PAGE_LOAD_TIME:
      size += 7 + data.host().size();
      break;

    case TURBO_CONNECTION_TYPE:
      size += 1;
      break;

    case TURBO_SERVER_SIDE_LOG:
      size += 1;
      break;
  ...
  if (protocol_version_ == SPDY3) {
    builder.WriteControlFrameHeader(*this, TURBO, kNoFlags);
  } else {
    builder.BeginNewFrame(*this, TURBO, TURBO_FLAG_END_TURBO_FRAME, 0);
  }
  builder.WriteUInt8(data.type());

  switch (data.type()) {
    case TURBO_CACHE_FULL:
    case TURBO_CACHE_INCREMENTAL:
      {
        const std::vector<TurboCacheEntry>* entries;
        std::vector<TurboCacheEntry>::const_iterator iter;

        entries = data.added_entries();
        builder.WriteUInt32(static_cast<uint32_t>(entries->size()));
        for (iter = entries->begin(); iter != entries->end(); ++iter)
          builder.WriteUInt32(static_cast<uint32_t>(iter->url_crc32()));

        if (data.type() == TURBO_CACHE_INCREMENTAL) {
          entries = data.removed_entries();
          builder.WriteUInt32(static_cast<uint32_t>(entries->size()));
          for (iter = entries->begin(); iter != entries->end(); ++iter)
            builder.WriteUInt32(static_cast<uint32_t>(iter->url_crc32()));
        }
    ...
    case TURBO_UNVERIFIED_PUSHED_STREAMS:
      {
        const std::vector<SpdyStreamId>* stream_ids = data.stream_ids();
        std::vector<SpdyStreamId>::const_iterator iter;

        builder.WriteUInt32(static_cast<uint32_t>(stream_ids->size()));
        for (iter = stream_ids->begin(); iter != stream_ids->end(); ++iter)
          builder.WriteUInt32(static_cast<uint32_t>(*iter));
      }
      break;

    case TURBO_HELLO: {
      builder.WriteStringPiece16(data.host());
      builder.WriteStringPiece16(data.uid());
      builder.WriteStringPiece16(data.brand());
      builder.WriteStringPiece16(data.platform());
      builder.WriteStringPiece16(data.product());
      builder.WriteStringPiece16(data.version());
      builder.WriteStringPiece16(data.device());
      builder.WriteStringPiece16(data.mcc());
      builder.WriteStringPiece16(data.mnc());
      builder.WriteStringPiece16(data.key());
      builder.WriteUInt16(data.headers().size());
      std::map<std::string, std::string>::const_iterator it;
      for (it = data.headers().begin(); it != data.headers().end(); ++it) {
    ...
    case TURBO_AUTH_RESPONSE:
      builder.WriteStringPiece16(data.response());
      break;

    case TURBO_IMAGE_QUALITY:
      builder.WriteUInt8(data.quality());
      break;

    case TURBO_USAGE_STATS:
      builder.WriteUInt8(data.usage_stats_version());
      builder.WriteStringPiece32(data.usage_stats());
      break;

    case TURBO_SET_COOKIES:
      builder.WriteStringPiece16(data.host());
      builder.WriteUInt8(data.cookies()->size());
      for (cookies_iter = data.cookies()->begin();
           cookies_iter != data.cookies()->end();
           ++cookies_iter)
        builder.WriteStringPiece16(*cookies_iter);
      break;

    case TURBO_CONNECTION_TYPE:
      builder.WriteUInt8(data.connection_type());
      break;

    case TURBO_PAGE_LOAD_TIME:
      builder.WriteUInt8(data.turbo_enabled() ? 1 : 0);
      builder.WriteStringPiece16(data.host());
      builder.WriteUInt32(data.milliseconds());
      break;

    case TURBO_VERSION_FEATURES:
      builder.WriteUInt16(data.turbo_protocol_version_high());
      builder.WriteUInt16(data.turbo_protocol_version_low());
      builder.WriteUInt32(data.feature_flags());
      break;

    case TURBO_SERVER_SIDE_LOG:
      builder.WriteUInt8(data.server_side_log());
      break;
  ...
  // We should observe if the frame size doesn't exceed the maximum allowed
  // size. The Turbo2 server doesn't seem to send the SETTINGS_MAX_FRAME_SIZE
  // setting so the permitted maximum can be assumed to be the RFC's minimal
  // size of 2^14 bytes (HTTP2) and 8192 bytes (SPDY31). In case we start to
  // tune this, we'll have to compare to the server-sent maximum here instead of
  // the hardcoded minimal value.
  bool frame_too_big = false;
  if (protocol_version_ == SPDY3) {
    const size_t kMaxSpdy3FrameSize = 8192;
    frame_too_big = (builder.length() > kMaxSpdy3FrameSize);
  } else {
    const size_t kMaxHttp2FrameSize = 16384;
    frame_too_big = (builder.length() > kMaxHttp2FrameSize);
  }
  // We want to measure how often will the client send oversized frames that
  // will probably end in session errors. If this shows to be a problem in
  // processed metics, we'll have to followup on TURBO frame splitting.
  UMA_HISTOGRAM_BOOLEAN("Opera.DSK.Turbo2.FrameTooBig", frame_too_big);
