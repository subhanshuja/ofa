  ...
  switch (version) {
    case SPDY3:
      ...
      if (frame_type_field > SerializeFrameType(version, WINDOW_UPDATE)) {
        if (frame_type_field != SerializeFrameType(version, TURBO))
          return false;
      }
...
SpdyTurboIR::SpdyTurboIR()
    : type_(TURBO_INVALID)
    , added_entries_(NULL)
    , removed_entries_(NULL)
    , stream_ids_(NULL) {
}

void SpdyTurboIR::SetFullCacheList(
    const std::vector<TurboCacheEntry>& entries) {
  DCHECK_EQ(TURBO_INVALID, type_);

  type_ = TURBO_CACHE_FULL;
  added_entries_ = &entries;
}

void SpdyTurboIR::SetIncrementalCacheList(
    const std::vector<TurboCacheEntry>& added_entries,
    const std::vector<TurboCacheEntry>& removed_entries) {
  DCHECK_EQ(TURBO_INVALID, type_);

  type_ = TURBO_CACHE_INCREMENTAL;
  added_entries_ = &added_entries;
  removed_entries_ = &removed_entries;
}

void SpdyTurboIR::SetUnclaimedPushedStreams(
    const std::vector<SpdyStreamId>& stream_ids) {
  DCHECK_EQ(TURBO_INVALID, type_);

  type_ = TURBO_UNCLAIMED_PUSHED_STREAMS;
  stream_ids_ = &stream_ids;
}

void SpdyTurboIR::SetUnverifiedPushedStreams(
    const std::vector<SpdyStreamId>& stream_ids) {
  DCHECK_EQ(TURBO_INVALID, type_);

  type_ = TURBO_UNVERIFIED_PUSHED_STREAMS;
  stream_ids_ = &stream_ids;
}

void SpdyTurboIR::SetHello(const std::string& host,
...
void SpdyTurboIR::SetAuthResponse(const std::string& response) {
  DCHECK_EQ(TURBO_INVALID, type_);

  type_ = TURBO_AUTH_RESPONSE;
  response_ = response;
}

void SpdyTurboIR::SetImageQuality(int quality) {
  DCHECK_EQ(TURBO_INVALID, type_);

  type_ = TURBO_IMAGE_QUALITY;
  quality_ = quality;
}

void SpdyTurboIR::SetUsageStats(const std::string& stats, uint8_t stats_version) {
  DCHECK_EQ(TURBO_INVALID, type_);

  type_ = TURBO_USAGE_STATS;
  usage_stats_ = stats;
  usage_stats_version_ = stats_version;
}

void SpdyTurboIR::SetSetCookies(const std::string& host,
                                const std::vector<std::string>& cookies) {
  DCHECK_EQ(TURBO_INVALID, type_);
  DCHECK_GT(255u, cookies.size());

  type_ = TURBO_SET_COOKIES;
  host_ = host;
  cookies_ = &cookies;
}

void SpdyTurboIR::SetVersionFeatures(uint16_t turbo_protocol_version_high,
                                     uint16_t turbo_protocol_version_low,
                                     uint32_t feature_flags) {
  DCHECK_EQ(TURBO_INVALID, type_);

  type_ = TURBO_VERSION_FEATURES;
...
void SpdyTurboIR::SetPageLoadTime(
    bool turbo_enabled, const std::string& host, uint32_t milliseconds) {
  DCHECK_EQ(TURBO_INVALID, type_);

  type_ = TURBO_PAGE_LOAD_TIME;
  turbo_enabled_ = turbo_enabled;
  host_ = host;
  milliseconds_ = milliseconds;
}

void SpdyTurboIR::SetConnectionType(int connection_type) {
  DCHECK_EQ(TURBO_INVALID, type_);

  type_ = TURBO_CONNECTION_TYPE;
  connection_type_ = connection_type;
}

void SpdyTurboIR::SetServerSideLog(bool enable) {
  DCHECK_EQ(TURBO_INVALID, type_);

  type_ = TURBO_SERVER_SIDE_LOG;
  server_side_log_ = enable;
}

void SpdyTurboIR::Visit(SpdyFrameVisitor* visitor) const {
  return visitor->VisitTurbo(*this);
}
