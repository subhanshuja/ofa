...
enum Http2TurboFlags {
  TURBO_FLAG_END_TURBO_FRAME = 0x04,
};
...
// TURBO control frame type.
enum SpdyTurboFrameType {
  TURBO_INVALID = -1,
  TURBO_CACHE_FULL = 0,
  TURBO_CACHE_INCREMENTAL = 1,
  TURBO_SET_COOKIE = 2,
  TURBO_UNCLAIMED_PUSHED_STREAMS = 3,
  TURBO_HELLO = 4,
  TURBO_AUTH_RESPONSE = 5,
  TURBO_AUTH_REQUEST = 6,
  TURBO_STATS = 7,
  TURBO_HOST = 8,
  TURBO_IMAGE_QUALITY = 9,
  TURBO_UNVERIFIED_PUSHED_STREAMS = 10,
  TURBO_SET_COOKIES = 11,
  TURBO_CLEAR_COOKIES = 12,
  TURBO_PAGE_LOAD_TIME = 13,
  TURBO_VERSION_FEATURES = 14,
  TURBO_CONNECTION_TYPE = 15,
  TURBO_SERVER_SIDE_LOG = 16,
  TURBO_DYNAMIC_CONTENT_CHANNELS = 17,
  TURBO_DYNAMIC_CONTENT_DATA = 18,
  TURBO_TURBO_CACHED_RESOURCES = 19,
  TURBO_DIRECT_REQUEST_LIST = 20,
  TURBO_PRE_LOAD_LIST = 21,
  TURBO_SITE_STATS = 22,
  TURBO_COOKIE_MODE = 23,
  TURBO_SUGGESTED_SERVER = 24,
  TURBO_USAGE_STATS = 28,
};

enum SpdyTurboFeatureFlag {
  TURBO_NO_FEATURES = 0,
  TURBO_FEATURE_VIDEO_COMPRESSION = 1 << 0,
  TURBO_FEATURE_IMAGE_ALPHA_WEBP = 1 << 1,
  TURBO_FEATURE_IMAGE_LOSSLESS_WEBP = 1 << 2,
  // 3..9: other features
  TURBO_FEATURE_HTTPS_REQUESTS = 1 << 10,
  TURBO_FEATURE_ADBLOCK_BLANK = 1 << 12,
};

const uint16_t KTurboProtocolVersionHigh = 2;
const uint16_t KTurboProtocolVersionLow = 0;
...
class SpdyTurboIR : public SpdyFrameIR {
 public:
  SpdyTurboIR();

  // Frame with TURBO_CACHE_FULL.
  void SetFullCacheList(const std::vector<TurboCacheEntry>& entries);
  // Frame with TURBO_CACHE_INCREMENTAL.
  void SetIncrementalCacheList(
      const std::vector<TurboCacheEntry>& added_entries,
      const std::vector<TurboCacheEntry>& removed_entries);
  // Frame with TURBO_UNCLAIMED_PUSHED_STREAMS.
  void SetUnclaimedPushedStreams(const std::vector<SpdyStreamId>& stream_ids);
  // Frame with TURBO_UNCLAIMED_PUSHED_STREAMS.
  void SetUnverifiedPushedStreams(const std::vector<SpdyStreamId>& stream_ids);
  // Frame with TURBO_HELLO.
  void SetHello(const std::string& host,
  ...
  // Frame with TURBO_AUTH_RESPONSE.
  void SetAuthResponse(const std::string& response);
  // Frame with TURBO_IMAGE_QUALITY.
  void SetImageQuality(int quality);
  // Frame with TURBO_USAGE_STATS.
  void SetUsageStats(const std::string& stats, uint8_t stats_version);
  // Frame with TURBO_SET_COOKIES.
  void SetSetCookies(const std::string& host,
                     const std::vector<std::string>& cookies);
  // Frame with TURBO_VERSION_FEATURES
  void SetVersionFeatures(uint16_t version_high, uint16_t version_low,
                          uint32_t feature_flags);
  // Frame with TURBO_PAGE_LOAD_TIME.
  void SetPageLoadTime(
      bool turbo_enabled, const std::string& host, uint32_t milliseconds);
  // Frame with TURBO_CONNECTION_TYPE.
  void SetConnectionType(int type);
  // Frame with TURBO_SERVER_SIDE_LOG.
  void SetServerSideLog(bool enable);

  void Visit(SpdyFrameVisitor* visitor) const override;

  ~SpdyTurboIR() override;

  SpdyTurboFrameType type() const { return type_; }

  const std::vector<TurboCacheEntry>* added_entries() const {
  ...
  const std::vector<SpdyStreamId>* stream_ids() const { return stream_ids_; }
  ...
  const std::string& mcc() const { return mcc_; }
  const std::string& mnc() const { return mnc_; }
  ...
  int connection_type() const { return connection_type_; }
  bool server_side_log() const { return server_side_log_; }
  std::string usage_stats() const { return usage_stats_; }
  uint8_t usage_stats_version() const { return usage_stats_version_; }

  bool turbo_enabled() const { return turbo_enabled_; }
  uint32_t milliseconds() const { return milliseconds_; }

  uint16_t turbo_protocol_version_high() const {
    return turbo_protocol_version_high_;
  }
  uint16_t turbo_protocol_version_low() const {
    return turbo_protocol_version_low_;
  }
  uint32_t feature_flags() const { return feature_flags_; }

 private:
  SpdyTurboFrameType type_;

  const std::vector<TurboCacheEntry>* added_entries_;
  const std::vector<TurboCacheEntry>* removed_entries_;

  std::string host_;
  const std::vector<std::string>* cookies_;

  const std::vector<SpdyStreamId>* stream_ids_;
  ...
  bool server_side_log_;
  std::string usage_stats_;
  uint8_t usage_stats_version_;

  bool turbo_enabled_;
  uint32_t milliseconds_;

  uint16_t turbo_protocol_version_high_;
  uint16_t turbo_protocol_version_low_;
  uint32_t feature_flags_;

  DISALLOW_COPY_AND_ASSIGN(SpdyTurboIR);
};
...
class SpdyFrameVisitor {
 public:
  ...
  virtual void VisitTurbo(const SpdyTurboIR& data) = 0;
