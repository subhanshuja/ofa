...
base::WeakPtr<SpdySession> SpdySessionPool::CreateAvailableSessionFromSocket(
  ...
  bool is_turbo2_proxy = is_turbo_proxy;
#else
  bool is_turbo2_proxy = is_turbo_proxy && HttpStreamFactory::turbo2_enabled();
#endif

  if (is_turbo2_proxy) {
    SSLInfo info;
    connection->socket()->GetSSLInfo(&info);
    if (info.is_valid()) {
      // If this fails it means we have forgotten to check for a valid Turbo
      // certificate somewhere up the chain.
      CHECK(TurboCertificate::ContainsTurboCA(info.public_key_hashes));
      is_secure = true;
    } else {
      is_secure = false;
    }
  ...
  std::unique_ptr<SpdySession> new_session(new SpdySession(
      ...
      is_turbo_proxy, proxy_delegate_, net_log.net_log()));

  if (is_turbo2_proxy) {
    FOR_EACH_OBSERVER(Observer,
                      observer_list_,
                      OnTurboSpdySessionCreated(new_session->GetWeakPtr()));
  }
  ...
  if (is_turbo2_proxy) {
    FOR_EACH_OBSERVER(Observer,
                      observer_list_,
                      OnTurboSpdySessionInitialized(new_session->GetWeakPtr()));
  }
...
void SpdySessionPool::RemoveUnavailableSession(
  ...
  if (unavailable_session->IsTurbo2Proxy()) {
    FOR_EACH_OBSERVER(Observer,
                      observer_list_,
                      OnTurboSpdySessionDestroyed(unavailable_session));
  }
...
void SpdySessionPool::AddObserver(Observer* observer) {
  observer_list_.AddObserver(observer);
}

void SpdySessionPool::RemoveObserver(Observer* observer) {
