...
class NET_EXPORT_PRIVATE BufferedSpdyFramer
 ...
 public:
  BufferedSpdyFramer();
  explicit BufferedSpdyFramer(bool is_turbo2_proxy);
  ...
  SpdySerializedFrame* CreateAuthResponse(const std::string& response);
  SpdySerializedFrame* CreateImageQuality(int quality);
  SpdySerializedFrame* CreateUsageStats(const std::string& stats, uint8_t stats_version);
  ...
  SpdySerializedFrame* CreateConnectionType(int type);
  SpdySerializedFrame* CreateServerSideLog(bool enable);
  SpdySerializedFrame* CreateVersionFeatures(
