...
BufferedSpdyFramer::BufferedSpdyFramer() : BufferedSpdyFramer(false) {}

BufferedSpdyFramer::BufferedSpdyFramer(bool is_turbo2_proxy)
    : spdy_framer_(HTTP2),
      visitor_(NULL),
      header_buffer_valid_(false),
      header_stream_id_(SpdyFramer::kInvalidStream),
      frames_received_(0) {
  spdy_framer_.set_is_turbo2_proxy(is_turbo2_proxy);
}
...
void BufferedSpdyFramer::OnTurboAuthRequest(const std::string& challenge) {
  visitor_->OnTurboAuthRequest(challenge);
}

void BufferedSpdyFramer::OnTurboStatistics(uint32_t compressed,
                                           uint32_t uncompressed) {
  visitor_->OnTurboStatistics(compressed, uncompressed);
}

void BufferedSpdyFramer::OnTurboHost(const std::string& host) {
  visitor_->OnTurboHost(host);
}

void BufferedSpdyFramer::OnTurboSuggestedServer(
    const std::string& suggested_server) {
  visitor_->OnTurboSuggestedServer(suggested_server);
}
...
SpdySerializedFrame* BufferedSpdyFramer::CreateFullCacheList(
    const std::vector<TurboCacheEntry>& entries) {
  return new SpdySerializedFrame(spdy_framer_.CreateFullCacheList(entries));
}

SpdySerializedFrame* BufferedSpdyFramer::CreateIncrementalCacheList(
    const std::vector<TurboCacheEntry>& added_entries,
    const std::vector<TurboCacheEntry>& removed_entries) {
  return new SpdySerializedFrame(spdy_framer_.CreateIncrementalCacheList(
      added_entries, removed_entries));
}

SpdySerializedFrame* BufferedSpdyFramer::CreateUnclaimedPushedStreams(
    const std::vector<SpdyStreamId>& stream_ids) {
  return new SpdySerializedFrame(
      spdy_framer_.CreateUnclaimedPushedStreams(stream_ids));
}

SpdySerializedFrame* BufferedSpdyFramer::CreateUnverifiedPushedStreams(
    const std::vector<SpdyStreamId>& stream_ids) {
  return new SpdySerializedFrame(
      spdy_framer_.CreateUnverifiedPushedStreams(stream_ids));
}

SpdySerializedFrame* BufferedSpdyFramer::CreateHello(
  ...
  return new SpdySerializedFrame(spdy_framer_.CreateHello(
      host, uid, brand, platform, product, version, device, mcc, mnc, key,
      headers));
}

SpdySerializedFrame* BufferedSpdyFramer::CreateAuthResponse(
    const std::string& response) {
  return new SpdySerializedFrame(spdy_framer_.CreateAuthResponse(response));
}

SpdySerializedFrame* BufferedSpdyFramer::CreateImageQuality(int quality) {
  return new SpdySerializedFrame(spdy_framer_.CreateImageQuality(quality));
}

SpdySerializedFrame* BufferedSpdyFramer::CreateUsageStats(
    const std::string& stats,
    uint8_t stats_version) {
  return new SpdySerializedFrame(
      spdy_framer_.CreateUsageStats(stats, stats_version));
}

SpdySerializedFrame* BufferedSpdyFramer::CreateSetCookies(
    const std::string& host, const std::vector<std::string>& cookies) {
  return new SpdySerializedFrame(spdy_framer_.CreateSetCookies(host, cookies));
}

SpdySerializedFrame* BufferedSpdyFramer::CreatePageLoadTime(
    bool turbo_enabled, const std::string& host, uint32_t milliseconds) {
  return new SpdySerializedFrame(
      spdy_framer_.CreatePageLoadTime(turbo_enabled, host, milliseconds));
}

SpdySerializedFrame* BufferedSpdyFramer::CreateConnectionType(int type) {
  return new SpdySerializedFrame(spdy_framer_.CreateConnectionType(type));
}

SpdySerializedFrame* BufferedSpdyFramer::CreateServerSideLog(bool enable) {
  return new SpdySerializedFrame(spdy_framer_.CreateServerSideLog(enable));
}

SpdySerializedFrame* BufferedSpdyFramer::CreateVersionFeatures(
    uint16_t turbo_protocol_version_high,
    uint16_t turbo_protocol_version_low,
    uint32_t turbo_feature_flags) const {
  return new SpdySerializedFrame(
      spdy_framer_.CreateVersionFeatures(turbo_protocol_version_high,
