...
// Base Turbo client features.
const uint32_t kTurboBaseClientFeatures =
  TURBO_FEATURE_IMAGE_ALPHA_WEBP |
  TURBO_FEATURE_IMAGE_LOSSLESS_WEBP;

// Maximum number of seconds a pushed stream can remain unclaimed before it is
// reported to the Turbo proxy as unclaimed.
const int kTurboUnclaimedStreamMaxAgeSeconds = 60;

// Frequency at which a session with the Turbo proxy checks for expired
// unclaimed pushed streams.
const int kTurboCheckUnclaimedInterval = 5;

// After how long time (since the first was signaled) all unverified streams are
// reported back to the Turbo proxy.
const int kTurboUnverifiedDelay = 5;
...
SpdySession::SpdySession(const SpdySessionKey& spdy_session_key,
    ...
    : in_io_loop_(false),
      ...
      send_unclaimed_(false),
      connection_at_risk_of_loss_time_(
          base::TimeDelta::FromSeconds(kDefaultConnectionAtRiskOfLossSeconds)),
      hung_interval_(base::TimeDelta::FromSeconds(kHungIntervalSeconds)),
      proxy_delegate_(proxy_delegate),
      is_turbo2_proxy_(is_turbo2_proxy),
      turbo_image_quality_(-1),
      turbo_protocol_version_high_(0),
      turbo_protocol_version_low_(0),
      turbo_feature_flags_(0),
      turbo_features_override_(false),
      server_side_log_(false),
      usage_stats_(""),
      usage_stats_version_(1),
      turbo_unclaimed_streams_timer_(true, true),
      turbo_unclaimed_stream_max_age_(kTurboUnclaimedStreamMaxAgeSeconds),
      turbo_unverified_streams_timer_(false, false),
  ...
  if (command_line->HasSwitch("turbo-pushed-stream-max-age")) {
    std::string value = command_line->
        GetSwitchValueASCII("turbo-pushed-stream-max-age");
    base::StringToInt(value, &turbo_unclaimed_stream_max_age_);
  }
...
void SpdySession::UnclaimedDataCanBeSent() {
  send_unclaimed_ = true;
}
...
void SpdySession::InitializeWithSocket(
  ...
  buffered_spdy_framer_.reset(new BufferedSpdyFramer(is_turbo2_proxy_));
  ...
  if (is_turbo2_proxy_) {
    SendHello();
    SendConnectionType(NetworkChangeNotifier::GetConnectionType());
    uint32_t feature_flags =
        kTurboBaseClientFeatures | TurboSessionPool::runtime_features();

    // Override features from command line.
    base::CommandLine* command_line = base::CommandLine::ForCurrentProcess();
    if (command_line->HasSwitch("turbo2-features")) {
      std::string value = command_line->
          GetSwitchValueASCII("turbo2-features");
      base::StringToUint(value, &feature_flags);
      turbo_features_override_ = true;
    }

    SendVersionFeatures(KTurboProtocolVersionHigh,
                        KTurboProtocolVersionLow,
                        feature_flags);
  }
...
bool SpdySession::HasPushStream(const GURL& url) const {
  if (availability_state_ == STATE_DRAINING)
    return false;
  ...
  ActiveStreamMap::const_iterator active_it = active_streams_.find(stream_id);
  return active_it != active_streams_.end();
}
...
void SpdySession::CloseSessionOnError(Error err,
  ...
  turbo_unclaimed_streams_timer_.Stop();
  turbo_unverified_streams_timer_.Stop();
...
void SpdySession::EnqueueSessionWrite(
  ...
  DCHECK(frame_type == RST_STREAM || frame_type == SETTINGS ||
         ...
         frame_type == GOAWAY || frame_type == TURBO);
  EnqueueWrite(
      priority, frame_type,
...
void SpdySession::OnConnectionTypeChanged(
    NetworkChangeNotifier::ConnectionType type) {
  SendConnectionType(type);
}
...
bool SpdySession::TryCreatePushStream(SpdyStreamId stream_id,
  ...
  if (!is_turbo2_proxy_) {
    // The following code is one level less indented than it should
    // be, to minimize conflicts when chromium changes this code.
  ...
  // End of intentional wrong indentation
  }
  ...
  FOR_EACH_OBSERVER(Observer, observer_list_, OnSynStream(gurl));
  ...
  if (is_turbo2_proxy_ && !turbo_unclaimed_streams_timer_.IsRunning()) {
    turbo_unclaimed_streams_timer_.Start(
        FROM_HERE, base::TimeDelta::FromSeconds(kTurboCheckUnclaimedInterval),
        base::Bind(&SpdySession::TurboCheckUnclaimedStreams,
                   weak_factory_.GetWeakPtr()));
  }
...
void SpdySession::OnTurboAuthRequest(const std::string& challenge) {
  std::string proxy_auth_secret;

  // If TURBO2_PROXY_AUTH_SECRET is not defined, no connections to the
  // Turbo proxy will be established, and this code will never be
  // reached.
#if defined(TURBO2_PROXY_AUTH_SECRET)
  // A base64 decode error here would indicate invalid compile-time
  // settings.  The 'proxy_auth_secret' string will remain empty,
  // which will lead to an incorrect auth response, and the proxy will
  // close our connection.
  bool base64_decode_result = base::Base64Decode(TURBO2_PROXY_AUTH_SECRET,
                                                 &proxy_auth_secret);
  DCHECK(base64_decode_result);
#endif

  SendAuthResponse(crypto::SHA256HashString(challenge + proxy_auth_secret));
}

void SpdySession::OnTurboStatistics(uint32_t compressed,
                                    uint32_t uncompressed) {
  FOR_EACH_OBSERVER(Observer, observer_list_,
                    OnTurboStatistics(compressed, uncompressed));
}

void SpdySession::OnTurboHost(const std::string& host) {
  pool_->set_turbo_host(host);
}

void SpdySession::OnTurboSuggestedServer(const std::string& suggested_server) {
  FOR_EACH_OBSERVER(Observer, observer_list_,
                    OnTurboSuggestedServer(suggested_server));
}
...
void SpdySession::SendFullCacheList(
    const std::vector<TurboCacheEntry>& entries) {
  DCHECK(is_turbo2_proxy_);
  DCHECK(buffered_spdy_framer_.get());
  std::unique_ptr<SpdySerializedFrame> turbo_frame(
      buffered_spdy_framer_->CreateFullCacheList(entries));
  EnqueueSessionWrite(MEDIUM, TURBO, std::move(turbo_frame));
}

void SpdySession::SendIncrementalCacheList(
    const std::vector<TurboCacheEntry>& added_entries,
    const std::vector<TurboCacheEntry>& removed_entries) {
  DCHECK(is_turbo2_proxy_);
  DCHECK(buffered_spdy_framer_.get());
  std::unique_ptr<SpdySerializedFrame> turbo_frame(
      buffered_spdy_framer_->CreateIncrementalCacheList(
          added_entries, removed_entries));
  EnqueueSessionWrite(MEDIUM, TURBO, std::move(turbo_frame));
}

void SpdySession::SendUnclaimedPushedStreams(
    const std::vector<SpdyStreamId>& stream_ids) {
  DCHECK(is_turbo2_proxy_);
  DCHECK(buffered_spdy_framer_.get());
  std::unique_ptr<SpdySerializedFrame> turbo_frame(
      buffered_spdy_framer_->CreateUnclaimedPushedStreams(stream_ids));
  EnqueueSessionWrite(LOWEST, TURBO, std::move(turbo_frame));
}

void SpdySession::SendUnverifiedPushedStreams() {
  DCHECK(is_turbo2_proxy_);
  DCHECK(buffered_spdy_framer_.get());
  if (!unverified_streams_.empty()) {
    std::vector<SpdyStreamId> stream_ids(unverified_streams_.begin(),
                                         unverified_streams_.end());
    std::unique_ptr<SpdySerializedFrame> turbo_frame(
        buffered_spdy_framer_->CreateUnverifiedPushedStreams(stream_ids));
    EnqueueSessionWrite(LOWEST, TURBO, std::move(turbo_frame));
    unverified_streams_.clear();
  }
}

void SpdySession::SendHello() {
  DCHECK(is_turbo2_proxy_);
  DCHECK(buffered_spdy_framer_.get());

  const size_t kTurboClientIDSize = 32;

  std::string client_id(base::RandBytesAsString(kTurboClientIDSize));
  std::string device(TURBO_DEVICE);
  std::string mcc, mnc, key;
  std::map<std::string, std::string> headers;

  FOR_EACH_OBSERVER(Observer,
                    observer_list_,
                    OnTurboSession(is_secure_, &client_id, &device, &mcc, &mnc,
                                   &key, &headers));

  std::unique_ptr<SpdySerializedFrame> turbo_frame(
      buffered_spdy_framer_->CreateHello(pool_->turbo_host(),
                                         client_id,
                                         TURBO_BRAND,
                                         ...
                                         key,
                                         headers));
  EnqueueSessionWrite(HIGHEST, TURBO, std::move(turbo_frame));
}

void SpdySession::SendAuthResponse(const std::string& response) {
  DCHECK(is_turbo2_proxy_);
  DCHECK(buffered_spdy_framer_.get());
  std::unique_ptr<SpdySerializedFrame> turbo_frame(
      buffered_spdy_framer_->CreateAuthResponse(response));
  EnqueueSessionWrite(HIGHEST, TURBO, std::move(turbo_frame));
}

void SpdySession::SendImageQuality(int quality) {
  DCHECK(is_turbo2_proxy_);
  DCHECK(buffered_spdy_framer_.get());

  if (quality != turbo_image_quality_) {
    std::unique_ptr<SpdySerializedFrame> turbo_frame(
        buffered_spdy_framer_->CreateImageQuality(quality));
    EnqueueSessionWrite(MEDIUM, TURBO, std::move(turbo_frame));
    turbo_image_quality_ = quality;
  }
}

void SpdySession::SendConnectionType(int type) {
  DCHECK(is_turbo2_proxy_);
  DCHECK(buffered_spdy_framer_.get());

  std::unique_ptr<SpdySerializedFrame> turbo_frame(
      buffered_spdy_framer_->CreateConnectionType(type));
  EnqueueSessionWrite(MEDIUM, TURBO, std::move(turbo_frame));
}

void SpdySession::SendServerSideLog(bool enable) {
  DCHECK(is_turbo2_proxy_);
  DCHECK(buffered_spdy_framer_.get());

  if (enable != server_side_log_) {
    std::unique_ptr<SpdySerializedFrame> turbo_frame(
        buffered_spdy_framer_->CreateServerSideLog(enable));
    EnqueueSessionWrite(MEDIUM, TURBO, std::move(turbo_frame));
    server_side_log_ = enable;
  }
}

void SpdySession::SendUsageStats(const std::string& stats,
                                 uint8_t stats_version) {
  DCHECK(is_turbo2_proxy_);
  DCHECK(buffered_spdy_framer_.get());

  if (stats != usage_stats_) {
    std::unique_ptr<SpdySerializedFrame> turbo_frame(
        buffered_spdy_framer_->CreateUsageStats(stats, stats_version));
    EnqueueSessionWrite(IDLE, TURBO, std::move(turbo_frame));
    usage_stats_ = stats;
    usage_stats_version_ = stats_version;
  }
}

void SpdySession::SendVersionFeatures(uint16_t turbo_protocol_version_high,
                                      uint16_t turbo_protocol_version_low,
                                      uint32_t turbo_feature_flags) {
  DCHECK(is_turbo2_proxy_);
  DCHECK(buffered_spdy_framer_.get());

  std::unique_ptr<SpdySerializedFrame> turbo_frame(
      buffered_spdy_framer_->CreateVersionFeatures(
          turbo_protocol_version_high,
          turbo_protocol_version_low,
          turbo_feature_flags));
  EnqueueSessionWrite(HIGHEST, TURBO, std::move(turbo_frame));
  turbo_protocol_version_high_ = turbo_protocol_version_high;
  turbo_protocol_version_low_ = turbo_protocol_version_low;
  turbo_feature_flags_ = turbo_feature_flags;
}

void SpdySession::SendUpdatedFeatures(uint32_t turbo_feature_flags) {
  DCHECK(is_turbo2_proxy_);
  DCHECK_NE(turbo_protocol_version_high_, 0);

  if (turbo_features_override_) {
    return;  // Nothing to do. We're using overridden features instead.
  }
  uint32_t feature_flags = kTurboBaseClientFeatures | turbo_feature_flags;
  if (feature_flags != turbo_feature_flags_) {
    SendVersionFeatures(turbo_protocol_version_high_,
                        turbo_protocol_version_low_,
                        feature_flags);
  }
}

void SpdySession::SignalUnverifiedPushedStream(SpdyStream* stream) {
  unverified_streams_.insert(stream->stream_id());

  if (is_turbo2_proxy_ && !turbo_unverified_streams_timer_.IsRunning()) {
    turbo_unverified_streams_timer_.Start(
        FROM_HERE, base::TimeDelta::FromSeconds(kTurboUnverifiedDelay),
        base::Bind(&SpdySession::SendUnverifiedPushedStreams,
                   weak_factory_.GetWeakPtr()));
  }
}

void SpdySession::SendSetCookies(
    const std::string& host, const std::vector<std::string>& cookies) {
  DCHECK(is_turbo2_proxy_);
  DCHECK(buffered_spdy_framer_.get());
  std::unique_ptr<SpdySerializedFrame> turbo_frame(
      buffered_spdy_framer_->CreateSetCookies(host, cookies));
  EnqueueSessionWrite(MEDIUM, TURBO, std::move(turbo_frame));
}

void SpdySession::SendPageLoadTime(
    bool turbo_enabled, const GURL& url, uint32_t milliseconds) {
  DCHECK(is_turbo2_proxy_);
  DCHECK(buffered_spdy_framer_.get());
  std::unique_ptr<SpdySerializedFrame> turbo_frame(
      buffered_spdy_framer_->CreatePageLoadTime(
          turbo_enabled, url.host(), milliseconds));
  EnqueueSessionWrite(MEDIUM, TURBO, std::move(turbo_frame));
}

void SpdySession::AddObserver(Observer* observer) {
  observer_list_.AddObserver(observer);
}

void SpdySession::RemoveObserver(Observer* observer) {
...
void SpdySession::TurboCheckUnclaimedStreams() {
  if (unclaimed_pushed_streams_.empty()) {
    turbo_unclaimed_streams_timer_.Stop();
    return;
  }

  std::vector<SpdyStreamId> stream_ids;
  bool unreported_stream_found = false;

  base::TimeTicks minimum_freshness = time_func_() -
      base::TimeDelta::FromSeconds(turbo_unclaimed_stream_max_age_);
  UnclaimedPushedStreamContainer::const_iterator unclaimed_it;
  for (unclaimed_it = unclaimed_pushed_streams_.begin();
       unclaimed_it != unclaimed_pushed_streams_.end();
       ++unclaimed_it) {
    ActiveStreamMap::iterator active_it = active_streams_.find(
        unclaimed_it->second.stream_id);
    if (active_it == active_streams_.end())
      continue;
    SpdyStream* stream = active_it->second.stream;
    if (stream->turbo_reported_unclaimed())
      continue;
    unreported_stream_found = true;
    if (minimum_freshness > unclaimed_it->second.creation_time) {
      stream_ids.push_back(stream->stream_id());
      stream->set_turbo_reported_unclaimed();
    }
  }

  if (!stream_ids.empty()) {
    // If the page loading in Turbo mode had been stopped intentionally by the
    // user, so the list of unclaimed resources is not needed to be sent back
    // to the server.
    if (send_unclaimed_) {
      SendUnclaimedPushedStreams(stream_ids);
      // Unset the flag for future use.
      send_unclaimed_ = false;
    }
  } else if (!unreported_stream_found) {
    turbo_unclaimed_streams_timer_.Stop();
  }
}

void SpdySession::TurboForceSendUnclaimedStreams() {
  std::vector<SpdyStreamId> stream_ids;

  // If the page loading in Turbo mode had been aborted by the user, no list
  // of the unclaimed (unused) resources needs to be sent back to the server.
  if (!send_unclaimed_)
    return;
  ...
  for (unclaimed_it = unclaimed_pushed_streams_.begin();
    ...
    ActiveStreamMap::iterator active_it =
        active_streams_.find(unclaimed_it->second.stream_id);
    if (active_it == active_streams_.end())
      continue;
    SpdyStream* stream = active_it->second.stream;
    if (!stream->turbo_reported_unclaimed()) {
