...
class NET_EXPORT_PRIVATE SpdyFramerVisitorInterface {
 public:
  ...
  // Called when a TURBO[auth request] frame has been parsed.
  virtual void OnTurboAuthRequest(const std::string& challenge) = 0;

  // Called when a TURBO[stats] frame has been parsed.
  virtual void OnTurboStatistics(uint32_t compressed,
                                 uint32_t uncompressed) = 0;

  // Called when a TURBO[host] frame has been parsed.
  virtual void OnTurboHost(const std::string& host) = 0;

  // Called when a TURBO[suggested server] frame has been parsed.
  virtual void OnTurboSuggestedServer(const std::string& suggested_server) = 0;
...
class NET_EXPORT_PRIVATE SpdyFramer {
 public:
  ...
  // Create a turbo[full cache list] frame.
  SpdySerializedFrame CreateFullCacheList(
      const std::vector<TurboCacheEntry>& entries) const;
  // Create a turbo[incremental cache list] frame.
  SpdySerializedFrame CreateIncrementalCacheList(
      const std::vector<TurboCacheEntry>& added_entries,
      const std::vector<TurboCacheEntry>& removed_entries) const;
  // Create a turbo[unclaimed pushed streams] frame.
  SpdySerializedFrame CreateUnclaimedPushedStreams(
      const std::vector<SpdyStreamId>& stream_ids) const;
  // Create a turbo[unverified pushed streams] frame.
  SpdySerializedFrame CreateUnverifiedPushedStreams(
      const std::vector<SpdyStreamId>& stream_ids) const;
  // Create a turbo[hello] frame.
  SpdySerializedFrame CreateHello(
  ...
  // Create a turbo[auth response] frame.
  SpdySerializedFrame CreateAuthResponse(const std::string& response) const;
  // Create a turbo[image quality] frame.
  SpdySerializedFrame CreateImageQuality(int quality) const;
  // Create a turbo[usage stats] frame.
  SpdySerializedFrame CreateUsageStats(
      const std::string& stats, uint8_t stats_version) const;
  // Create a turbo[set cookies] frame.
  SpdySerializedFrame CreateSetCookies(
      const std::string& host, const std::vector<std::string>& cookies) const;
  // Create a turbo[page load time] frame.
  SpdySerializedFrame CreatePageLoadTime(
      bool turbo_enabled, const std::string& host, uint32_t milliseconds) const;
  // Create a turbo[connection type] frame.
  SpdySerializedFrame CreateConnectionType(int type) const;
  // Create a turbo[server side log] frame.
  SpdySerializedFrame CreateServerSideLog(bool enable) const;

  // Create a turbo[version features] frame.
  SpdySerializedFrame CreateVersionFeatures(uint16_t turbo_protocol_version_high,
                                   uint16_t turbo_protocol_version_low,
                                   uint32_t turbo_feature_flags) const;
  SpdySerializedFrame SerializeTurbo(const SpdyTurboIR& data) const;
  ...
  // For ease of testing and experimentation we can tweak compression on/off.
  void set_is_turbo2_proxy(bool value) {
    is_turbo2_proxy_ = value;
  }
  ...
  size_t GetTurboMinimumSize() const;
 ...
 private:
  ...
  size_t ProcessTurboFramePayload(const char* data, size_t len);
  ...
  std::unique_ptr<char[]> turbo_frame_buffer_;
  uint32_t turbo_frame_buffer_length_;
