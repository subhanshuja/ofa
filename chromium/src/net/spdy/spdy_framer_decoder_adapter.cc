...
void SpdyFramerVisitorAdapter::OnTurboAuthRequest(
    const std::string& challenge) {
  visitor_->OnTurboAuthRequest(challenge);
}

void SpdyFramerVisitorAdapter::OnTurboStatistics(uint32_t compressed,
                                                 uint32_t uncompressed) {
  visitor_->OnTurboStatistics(compressed, uncompressed);
}

void SpdyFramerVisitorAdapter::OnTurboHost(const std::string& host) {
  visitor_->OnTurboHost(host);
}

void SpdyFramerVisitorAdapter::OnTurboSuggestedServer(
