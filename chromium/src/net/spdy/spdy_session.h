...
class NET_EXPORT SpdySession
 ...
 public:
  ...
  // Check if this session has a pushed stream for a given |url|.
  bool HasPushStream(const GURL& url) const;

  // If this function is called, it means that the page has been loaded
  // successfully and the unclaimed data can be sent to the server.
  void UnclaimedDataCanBeSent();
  ...
  bool IsTurbo2Proxy() const { return is_turbo2_proxy_; }

  bool ServerSideLog() const { return server_side_log_; }

  // Send a TURBO[full cache list] frame.
  void SendFullCacheList(const std::vector<TurboCacheEntry>& entries);

  // Send a TURBO[incremental cache list] frame.
  void SendIncrementalCacheList(
      const std::vector<TurboCacheEntry>& added_entries,
      const std::vector<TurboCacheEntry>& removed_entries);

  // Send a TURBO[unclaimed pushed streams] frame.
  void SendUnclaimedPushedStreams(const std::vector<SpdyStreamId>& stream_ids);

  // Send a TURBO[unverified pushed streams] frame.
  void SendUnverifiedPushedStreams();

  // Send a TURBO[hello] frame.
  void SendHello();

  // Send a TURBO[auth response] frame.
  void SendAuthResponse(const std::string& response);

  // Send a TURBO[image quality] frame.
  void SendImageQuality(int quality);

  // Send a TURBO[usage stats] frame.
  void SendUsageStats(const std::string& stats, uint8_t stats_version);

  // Send A TURBO[connection type] frame.
  void SendConnectionType(int type);

  // Send a TURBO[server side log] frame.
  void SendServerSideLog(bool enable);

  // Send a TURBO[version features] frame.
  void SendVersionFeatures(uint16_t turbo_protocol_version_high,
                           uint16_t turbo_protocol_version_low,
                           uint32_t turbo_feature_flags);

  // Send a TURBO[version features] frame with updated feature flags.
  void SendUpdatedFeatures(uint32_t turbo_feature_flags);

  // Signal that a pushed stream failed verification and could not be used.
  void SignalUnverifiedPushedStream(SpdyStream* stream);

  // Send a TURBO[set cookie] frame.
  void SendSetCookies(const std::string& host,
                      const std::vector<std::string>& cookie);

  // Send a TURBO[page load time] frame.
  void SendPageLoadTime(
      bool turbo_enabled, const GURL& url, uint32_t milliseconds);

  // Instantly send all unclaimed streams.
  void TurboForceSendUnclaimedStreams();

  class NET_EXPORT Observer {
   public:
    virtual void OnSynStream(const GURL& url) = 0;
    virtual void OnSynReply(const GURL& url) = 0;
    virtual void OnTurboSession(
        bool is_secure,
    ...
    virtual void OnTurboStatistics(uint32_t compressed, uint32_t uncompressed) = 0;
    virtual void OnTurboSuggestedServer(
        const std::string& suggested_server) = 0;
 ...
 private:
  ...
  void OnTurboAuthRequest(const std::string& challenge) override;
  void OnTurboStatistics(uint32_t compressed, uint32_t uncompressed) override;
  void OnTurboHost(const std::string& host) override;
  ...
  void OnConnectionTypeChanged(NetworkChangeNotifier::ConnectionType) override;
  ...
  void TurboCheckUnclaimedStreams();
  ...
  // A flag to indicate if the unclaimed data can be sent.
  bool send_unclaimed_;
  ...
  // This session is with the Turbo server.
  bool is_turbo2_proxy_;

  // Current image quality.
  int turbo_image_quality_;

  // Sent turbo version information. Used for updating client feature list.
  uint16_t turbo_protocol_version_high_;
  uint16_t turbo_protocol_version_low_;

  // Current turbo features reported to the server.
  uint32_t turbo_feature_flags_;

  // If turbo feature flags were overridden. Suppresses updating feature list.
  bool turbo_features_override_;

  // Current server side log state.
  ...
  base::Timer turbo_unclaimed_streams_timer_;
  int turbo_unclaimed_stream_max_age_;

  std::set<SpdyStreamId> unverified_streams_;
  base::Timer turbo_unverified_streams_timer_;
