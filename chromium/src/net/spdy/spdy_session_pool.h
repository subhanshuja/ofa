...
class NET_EXPORT SpdySessionPool
 ...
 public:
  ...
  void SetTurboSpdySession(const base::WeakPtr<SpdySession>& spdy_session);
  ...
  const std::string& turbo_host() { return turbo_host_; }
  void set_turbo_host(const std::string& value) { turbo_host_ = value; }
  ...
  class NET_EXPORT Observer {
   public:
    virtual void OnTurboSpdySessionCreated(
        const base::WeakPtr<SpdySession> session) = 0;
    virtual void OnTurboSpdySessionInitialized(
        const base::WeakPtr<SpdySession> session) = 0;
    virtual void OnTurboSpdySessionDestroyed(
        const base::WeakPtr<SpdySession> session) = 0;
