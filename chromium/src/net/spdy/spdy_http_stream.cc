...
int SpdyHttpStream::SendRequest(const HttpRequestHeaders& request_headers,
  ...
  CreateSpdyHeadersFromHttpRequest(*request_info_, request_headers, direct_,
                                   &headers);
  if (spdy_session_->IsTurbo2Proxy()) {
    headers["x-opera-requesttype"] = request_info_->turbo_request_type;
    headers["x-opera-main"] = request_info_->turbo_main;
    if (spdy_session_->ServerSideLog() &&
      request_info_->turbo_request_type == "main-frame")
      headers["x-opera-uuid"] = base::GenerateGUID();
  }
...
SpdyResponseHeadersStatus SpdyHttpStream::OnResponseHeadersUpdated(
  ...
  if (spdy_session_->IsTurbo2Proxy() && stream_->type() == SPDY_PUSH_STREAM) {
    // Verify that a pushed stream has response that is compatible with the
    // request.

    if (response_info_->headers->response_code() == 304 &&
        (request_info_->load_flags & LOAD_VALIDATE_CACHE) == 0) {
  ...
  if (response_headers.find("x-oc") == response_headers.end() &&
      !response_callback_.is_null()) {
...
/* Implementing this here is a hack.  Coding style (enforced by some compilers)
   prevents us from defining it inlined in the class definition, and
   HttpStream has no .cc file of its own. */
bool HttpStream::ShouldRestart() const {
  return false;
}

bool SpdyHttpStream::ShouldRestart() const {
  return should_restart_;
}

void SpdyHttpStream::VerificationFailed() const {
  if (stream_) {
    DCHECK(stream_->type() == SPDY_PUSH_STREAM);
    spdy_session_->SignalUnverifiedPushedStream(stream_.get());
  }
}
