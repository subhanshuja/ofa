...
std::string FilterTypeAsString(Filter::FilterType type_id) {
  switch (type_id) {
    ...
    case Filter::FILTER_TYPE_LZMA:
      return "FILTER_TYPE_LZMA";
...
Filter::FilterType Filter::ConvertEncodingToType(
  ...
  } else if (base::LowerCaseEqualsASCII(filter_type, kLzma)) {
    type_id = FILTER_TYPE_LZMA;
  } else {
...
// static
std::unique_ptr<Filter> Filter::InitLzmaFilter(
    FilterType type_id,
    int buffer_size) {
  std::unique_ptr<LzmaFilter> lzma_filter(new LzmaFilter());
  lzma_filter->InitBuffer(buffer_size);
  return std::move(lzma_filter);
}
...
std::unique_ptr<Filter> Filter::PrependNewFilter(
  ...
  switch (type_id) {
    ...
    case FILTER_TYPE_SDCH_POSSIBLE:
      ...
      break;
    case FILTER_TYPE_LZMA:
      first_filter = InitLzmaFilter(type_id, buffer_size);
      break;
    default:
      break;
  }
