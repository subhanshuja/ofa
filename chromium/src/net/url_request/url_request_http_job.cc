...
void URLRequestHttpJob::Start() {
  ...
  if (HttpStreamFactory::turbo2_enabled()) {
    request_info_.turbo_request_type = request_->turbo_request_type();
    request_info_.turbo_main = request_->turbo_main();
  }
}
...
void URLRequestHttpJob::AddExtraHeaders() {
  ...
  if (!request_info_.extra_headers.HasHeader(
    ...
    // headers.
    std::string advertised_encodings = "gzip, deflate, lzma";
    if (advertise_sdch)
...
void URLRequestHttpJob::AddCookieHeaderAndStart() {
  ...
  if (cookie_store && !(request_info_.load_flags & LOAD_DO_NOT_SEND_COOKIES)) {
    request_info_.cookie_store = cookie_store;
...
void URLRequestHttpJob::OnStartCompleted(int result) {
  ...
  if (result == OK) {
    ...
    // 502 and 503 with an "x-opera-info" header is used to signal various
    // errors including the "bypass" error (something the Turbo server is
    // configured to reject), or things like DNS/connection failure, which may
    // indicate that the URL is on the local network.
    // If the x-opera-info header is "e=3", this means that the client has used
    // the wrong slot for the requested URL. Using a correct slot would solve
    // the problem and Turbo can still be used without the need for bypassing
    // it. If we had traffic routing support, that is.
    if (net::HttpStreamFactory::turbo_enabled() && headers &&
        (headers->response_code() == 502 || headers->response_code() == 503) &&
        headers->HasHeader("x-opera-info"))
      bypass_turbo = true;

    if (network_delegate()) {
      if (transaction_->GetResponseInfo() != NULL)
        request_->set_bypass_turbo(
            transaction_->GetResponseInfo()->bypass_turbo);
      ...
      if (error != OK) {
        if (error == ERR_IO_PENDING) {
          ...
          return;
        } else if (error == net::ERR_TURBO_BYPASS) {
          bypass_turbo = true;
        } else {
          std::string source("delegate");
          request_->net_log().AddEvent(
  ...
  } else if (net::HttpStreamFactory::turbo_enabled() &&
             !(request_info_.load_flags & LOAD_BYPASS_TURBO) &&
             (result == ERR_PROXY_CONNECTION_FAILED ||
              result == ERR_PROXY_CERTIFICATE_INVALID ||
              result == ERR_SPDY_PROTOCOL_ERROR ||
              result == ERR_TIMED_OUT)) {
    // Retry without turbo in case the error is due to a captive portal.
    bypass_turbo = true;
    // We notify TurboSessionPool through the message loop here because in case
    // of a protocol error, the SPDY session may still exist at this point, but
    // in that case, a task to destroy it has already been posted by now.
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE, base::Bind(&TurboSessionPool::SignalConnectionFailed,
                              request_->context()
  ...
  if (bypass_turbo) {
    // Restart the connection with LOAD_BYPASS_TURBO to bypass turbo proxy
    request_info_.load_flags |= LOAD_BYPASS_TURBO;
    request_->SetLoadFlags(request_->load_flags() | LOAD_BYPASS_TURBO);
