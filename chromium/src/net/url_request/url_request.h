...
class NET_EXPORT URLRequest : NON_EXPORTED_BASE(public base::NonThreadSafe),
 ...
 public:
  ...
  void set_turbo_request_type(const std::string& value);
  const std::string& turbo_request_type() const { return turbo_request_type_; }

  void set_turbo_main(const std::string& value);
  const std::string& turbo_main() const { return turbo_main_; }

  void set_bypass_turbo(bool value);
  bool bypass_turbo() const { return bypass_turbo_; }
 ...
 private:
  ...
  // Turbo server.
  std::string turbo_request_type_;

  // Used for the X-Opera-Main header, if the request is sent through the Turbo
  // server.
  std::string turbo_main_;

  // True if the request did not use Turbo proxy while Turbo proxy is on.  Can
  // happen for Intranet requests.
  bool bypass_turbo_;
