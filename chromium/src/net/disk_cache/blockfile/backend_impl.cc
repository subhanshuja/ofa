...
BackendImpl::BackendImpl(
    ...
    : background_queue_(this, cache_thread),
      ...
      observer_list_(new ObserverList),
      ptr_factory_(this) {}
...
void BackendImpl::AddObserver(Observer* observer) {
  observer_list_->AddObserver(observer);
}

void BackendImpl::RemoveObserver(Observer* observer) {
...
void BackendImpl::NotifyEntryDoomed(const std::string& key) {
  observer_list_->Notify(FROM_HERE, &Observer::OnEntryDoomed, key);
}

void BackendImpl::NotifyCacheCleared() {
  observer_list_->Notify(FROM_HERE, &Observer::OnCacheCleared);
}
