...
HttpProxyClientSocketWrapper::HttpProxyClientSocketWrapper(
    ...
    : next_state_(STATE_NONE),
      ...
      turbo_connection_id_(turbo_connection_id),
      using_spdy_(false),
...
LoadState HttpProxyClientSocketWrapper::GetConnectLoadState() const {
  switch (next_state_) {
    ...
    case STATE_CHECK_EXISTING_SPDY_SESSION:
...
int HttpProxyClientSocketWrapper::DoBeginConnect() {
  ...
  next_state_ = STATE_CHECK_EXISTING_SPDY_SESSION;
  return OK;
}

int HttpProxyClientSocketWrapper::DoCheckExistingSpdySession() {
  bool using_ssl = ssl_params_.get() != NULL;
  if (tunnel_ && (is_turbo() || using_ssl)) {
    SpdySessionKey key(GetDestination().host_port_pair(),
                       ProxyServer::Direct(),
                       PRIVACY_MODE_DISABLED);
    key.set_unique_id(turbo_connection_id_);
...
int HttpProxyClientSocketWrapper::DoSSLConnectComplete(int result) {
  ...
  using_spdy_ = negotiated_protocol_ == kProtoHTTP2 || is_turbo();
