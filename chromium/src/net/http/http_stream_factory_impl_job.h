...
class HttpStreamFactoryImpl::Job {
 ...
 private:
  ...
  enum State {
    STATE_START,
    STATE_INTERNAL_IP_CHECK,
    STATE_INTERNAL_IP_CHECK_COMPLETE,
    STATE_RESOLVE_PROXY,
  ...
  int DoStart();
  int DoInternalIpCheck();
  int DoInternalIpCheckComplete(int result);
  int DoResolveProxy();
  ...
  // Tries to resolve the Turbo proxy. Returns true if Turbo should be
  // used and stores it in |proxy_info_|.
  bool TryResolveTurboProxy();

  // Do we want to make this request through Turbo? This means we have Turbo
  // set as the proxy server, and the URL's scheme is either http or (if https
  // compression is enabled) https.
  bool CanCompressWithTurbo() const;

  // Is this a HTTPS request we want to make through Turbo?
  bool UseTurboCompressSSL() const;
  ...
  // Container for DNS response when checking if destination is local
  AddressList origin_addresses_;
  std::unique_ptr<HostResolver::RequestInfo> origin_request_info_;
  ...
  // Request handle for DNS lookup when checking if server is local
  std::unique_ptr<HostResolver::Request> out_req_;
  ...
  // Set to true during DNS lookup if the address is in Private network range.
  // We prevent a proxy from taking over during actual connection startup
  // if address is in private network range.
  bool private_network_address_;
