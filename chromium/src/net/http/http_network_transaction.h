...
class NET_EXPORT_PRIVATE HttpNetworkTransaction
 ...
 private:
  ...
  std::string HexAsciiEncode(std::string indata);
  std::string CreateTurboProxyAuth(int connection_id = 11);
  static char turbo_proxy_auth_challenge_buf_[200];
  static char turbo_host_[200];
  // Are we retrying a turbo proxy request with authentication?
  bool turbo_proxy_auth_retry_;
  static int turbo_connection_id_counter_;
  int turbo_connection_id_;
  bool turbo1_enabled_;

  unsigned int turbo_proxy_auth_buf_len_;
  unsigned char* turbo_proxy_auth_buf_;
