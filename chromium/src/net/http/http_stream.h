...
class NET_EXPORT_PRIVATE HttpStream {
 public:
  ...
  // Checks whether the request has failed in such a way that it should be
  // restarted.  This happens for instance if a SPDY server pushed a response
  // that later turned out to be incompatible with the request.
  // NOTE: This method is implemented in net/spdy/spdy_http_stream.cc.
  virtual bool ShouldRestart() const;

  // Signal that verification of the response (against the request) failed, and
  // that the response will therefore be discarded.
  virtual void VerificationFailed() const {}
