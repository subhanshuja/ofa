...
HttpCache::~HttpCache() {
  ...
  SetHttpCacheForCurrentSession(true);
...
int HttpCache::GetBackend(disk_cache::Backend** backend,
  ...
  int result = CreateBackend(backend, base::Bind(&HttpCache::OnCacheInitialized,
                                                 GetWeakPtr(), callback));

  if (result == OK)
    SetHttpCacheForCurrentSession(false);
...
void HttpCache::WriteMetadata(const GURL& url,
  ...
  if (!disk_cache_.get() && !building_backend_) {
    int result = CreateBackend(NULL,
                               base::Bind(&HttpCache::OnCacheInitialized,
                                          GetWeakPtr(), CompletionCallback()));
...
int HttpCache::CreateTransaction(RequestPriority priority,
  ...
  if (GetSession() && GetSession()->turbo_session_pool())
    GetSession()->turbo_session_pool()->CheckTurboEnabled();
...
HttpNetworkSession* HttpCache::GetSession() {
  return network_layer_ ? network_layer_->GetSession() : NULL;
}
...
void HttpCache::SetHttpCacheForCurrentSession(bool clear) {
  HttpNetworkSession* session = GetSession();
  if (!session)
    return;
  base::WeakPtr<TurboSessionPool> pool = session->turbo_session_pool();
  if (!pool)
    return;
  TurboCacheSynchronizer* synchronizer = pool->turbo_cache_synchronizer();
  if (!synchronizer)
    return;
  if (clear)
    synchronizer->ClearHttpCache();
  else
    synchronizer->SetHttpCache(this);
}
...
void HttpCache::OnCacheInitialized(const base::WeakPtr<HttpCache>& cache,
                                   const CompletionCallback& callback,
                                   int result) {
  if (cache.get() && result == OK)
    cache->SetHttpCacheForCurrentSession(false);
