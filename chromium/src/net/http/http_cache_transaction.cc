...
ValidationType HttpCache::Transaction::RequiresValidation() {
  ...
  if (response_.headers->HasHeader("x-iq")) {
    std::string image_quality_header;
    response_.headers->GetNormalizedHeader("x-iq", &image_quality_header);

    int used_image_quality;
    if (!base::StringToInt(image_quality_header, &used_image_quality))
      // Shouldn't happen.
      used_image_quality = 0;

    int current_image_quality;
    if (HttpStreamFactory::turbo_enabled())
      current_image_quality = TurboSessionPool::image_quality();
    else
      // Pretend "no turbo" means "high quality".
      current_image_quality = 4;

    if (used_image_quality < current_image_quality) {
      turbo_invalid_response_ = true;
      return VALIDATION_SYNCHRONOUS;
    }
  }
