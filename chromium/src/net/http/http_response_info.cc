...
enum {
  ...
  // This bit is set if the request was requested to use Turbo proxy server,
  // but it decided not to.  Use high bit number to minimize the chance of
  // bit conflict in future Chromium intakes.
  RESPONSE_INFO_BYPASS_TURBO = 1 << 30,
...
bool HttpResponseInfo::InitFromPickle(const base::Pickle& pickle,
  ...
  bypass_turbo = (flags & RESPONSE_INFO_BYPASS_TURBO) != 0;
...
void HttpResponseInfo::Persist(base::Pickle* pickle,
  ...
  if (bypass_turbo)
    flags |= RESPONSE_INFO_BYPASS_TURBO;
