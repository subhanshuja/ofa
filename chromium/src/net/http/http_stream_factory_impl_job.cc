...
// Check if the hostname of a given URL specifies a numeric IP address
// that points to reserved address space such as 10/8 or 192.168/16.
bool IsHostReservedIP(const GURL& url) {
  IPAddress ip_address;
  return ParseURLHostnameToAddress(url.host(), &ip_address) &&
      ip_address.IsReserved();
}

bool UseTurboForRequest(const HttpRequestInfo& request_info) {
  if (request_info.load_flags &
      (LOAD_BYPASS_PROXY | LOAD_BYPASS_TURBO))
    return false;
  if ((request_info.load_flags & LOAD_MEDIA_CONTENT) &&
      (!TurboSessionPool::video_compression_enabled() ||
       !HttpStreamFactory::get_turbo_server_supports_video_compression()))
    return false;
  if (IsHostReservedIP(request_info.url))
    return false;
  if (request_info.load_flags & LOAD_FORCE_TURBO)
    return true;
  return HttpStreamFactory::turbo_enabled();
}
...
HttpStreamFactoryImpl::Job::Job(Delegate* delegate,
    ...
    : request_info_(request_info),
      ...
      private_network_address_(false),
...
bool HttpStreamFactoryImpl::Job::bypass_turbo() const {
  return proxy_info_.is_direct() || proxy_info_.is_empty() ||
      !proxy_info_.proxy_server().is_turbo();
}
...
SpdySessionKey HttpStreamFactoryImpl::Job::GetSpdySessionKey() const {
  ...
  // origin server.
  if (IsHttpsProxyAndHttpUrl() || CanCompressWithTurbo()) {
    SpdySessionKey key(proxy_info_.proxy_server().host_port_pair(),
                       ProxyServer::Direct(), PRIVACY_MODE_DISABLED);
    key.set_unique_id(proxy_info_.proxy_server().turbo_connection_id());
    return key;
  }
  ...
  do {
    ...
    switch (state) {
      case STATE_START:
        ...
        rv = DoStart();
        break;
      case STATE_INTERNAL_IP_CHECK:
	DCHECK_EQ(OK, rv);
	rv = DoInternalIpCheck();
	break;
      case STATE_INTERNAL_IP_CHECK_COMPLETE:
	rv = DoInternalIpCheckComplete(rv);
	break;
...
static bool IsPrivateAddr(struct sockaddr* sa) {
  struct sockaddr_in *s_in = reinterpret_cast<struct sockaddr_in *>(sa);
  uint32_t addr = ntohl(s_in->sin_addr.s_addr);
  bool res = (addr & 0xff000000) == 0x00000000 || // 0.0.0.0/8
...
int HttpStreamFactoryImpl::Job::DoInternalIpCheck() {
  next_state_ = STATE_INTERNAL_IP_CHECK_COMPLETE;

  origin_request_info_.reset(new HostResolver::RequestInfo(destination_));
  return session_->params().host_resolver->Resolve(
      *(origin_request_info_.get()),
...
int HttpStreamFactoryImpl::Job::DoInternalIpCheckComplete(int result) {
  next_state_ = STATE_INIT_CONNECTION;

  out_req_.reset();

  if (result != OK) {
    return OK; // We ignore lookup errors and let normal proxy resolve handle it.
  }

  for (AddressList::const_iterator ai = origin_addresses_.begin(); ai != origin_addresses_.end(); ++ai) {
    if (ai->GetFamily() == ADDRESS_FAMILY_IPV4) {
      SockaddrStorage storage;
      if (ai->ToSockAddr(storage.addr, &storage.addr_len)) {
        if (IsPrivateAddr(storage.addr)) {
          next_state_ = STATE_RESOLVE_PROXY_COMPLETE;
          proxy_info_.UseDirect();
          private_network_address_ = true;
          return OK;
        }
      }
    }
...
int HttpStreamFactoryImpl::Job::DoInitConnectionImpl() {
  ...
  using_ssl_ = using_ssl_ && !UseTurboCompressSSL();
  using_spdy_ = CanCompressWithTurbo();

  // If Turbo is enabled but not used (for whatever reason), make sure
  // bypass_turbo() returns true.
  // TODO(simonb): If we reinstate CONNECT tunneling using Turbo, this should
  // not be done for connect requests.
  if (proxy_info_.proxy_server().is_turbo() && !using_spdy_) {
    proxy_info_.UseDirect();
  }
...
int HttpStreamFactoryImpl::Job::DoCreateStream() {
  ...
  bool direct = !(IsHttpsProxyAndHttpUrl() || CanCompressWithTurbo());
  ...
  // The SPDY session is Turbo only if it is a direct one to the proxy;
  // otherwise, it is a regular SPDY session tunneled through a Turbo proxy.
  bool is_turbo_session = proxy_info_.proxy_server().is_turbo() &&
...
int HttpStreamFactoryImpl::Job::ReconsiderProxyAfterError(int error) {
  ...
  // If we failed to connect to a Turbo proxy, don't try to be smart and let
  // URLRequestHttpJob::OnStartCompleted handle the error instead.
  if (proxy_info_.proxy_server().is_turbo()) {
    return error;
  }
...
bool HttpStreamFactoryImpl::Job::TryResolveTurboProxy() {
  if (!UseTurboForRequest(request_info_)) {
    return false;
  }

  base::WeakPtr<TurboSessionPool> turbo_session_pool =
      session_->turbo_session_pool();
  if (!turbo_session_pool) {
    return false;
  }

  ProxyServer turbo_proxy =
      turbo_session_pool->GetProxyServerForRequest(request_info_);
  if (turbo_proxy.is_turbo()) {
    proxy_info_.UseProxyServer(turbo_proxy);
    return true;
  }
...
bool HttpStreamFactoryImpl::Job::CanCompressWithTurbo() const {
  return proxy_info_.proxy_server().is_turbo() &&
         (origin_url_.SchemeIs("http") || UseTurboCompressSSL());
}

bool HttpStreamFactoryImpl::Job::UseTurboCompressSSL() const {
  return proxy_info_.is_https() && proxy_info_.proxy_server().is_turbo() &&
         TurboSessionPool::compress_ssl() &&
         // If we haven't yet connected to Turbo but the setting is enabled,
         // try to make the request with Turbo anyway. If it is indeed not
         // supported by the server, we'll get a bypass response.
         (!HttpStreamFactory::have_turbo_server_features() ||
          HttpStreamFactory::get_turbo_server_supports_https_requests()) &&
         origin_url_.SchemeIs("https");
}
