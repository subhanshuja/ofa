...
class NET_EXPORT_PRIVATE HttpStreamRequest {
 public:
  ...
  // Returns true if the request did not use Turbo proxy while Turbo proxy is
  // on.  Can happen for Intranet requests.
  virtual bool bypass_turbo() const = 0;
};
...
class NET_EXPORT HttpStreamFactory {
 public:
  ...
  // Turns turbo on or off.
  static void set_turbo1_enabled(bool value);
  static void set_turbo2_enabled(bool value);
  static void set_turbo_server_features(uint32_t features) {
    turbo_server_features_ = features;
    have_turbo_server_features_ = true;
  }

  static bool turbo_enabled() { return turbo1_enabled_ || turbo2_enabled_; }
  static bool turbo1_enabled() { return turbo1_enabled_; }
  static bool turbo2_enabled() { return turbo2_enabled_; }
  static bool get_turbo_server_supports_video_compression() {
    return (turbo_server_features_ & TURBO_FEATURE_VIDEO_COMPRESSION) != 0;
  }
  static bool get_turbo_server_supports_https_requests() {
    return (turbo_server_features_ & TURBO_FEATURE_HTTPS_REQUESTS) != 0;
  }
  static bool have_turbo_server_features() {
    return have_turbo_server_features_;
  }
  // Set turbo client ID.
  static void SetTurboClientId(std::string value);
  static std::string GetTurboClientId();

  // Turns using proxy for local servers on or off.
  static void set_proxy_for_local_servers_enabled(bool value) {
    proxy_for_local_servers_enabled_ = value;
  }

  static bool proxy_for_local_servers_enabled() {
    return proxy_for_local_servers_enabled_;
  }
 ...
 private:
  static bool turbo1_enabled_;
  static bool turbo2_enabled_;
  static bool have_turbo_server_features_;
  static uint32_t turbo_server_features_;
  static bool proxy_for_local_servers_enabled_;
  static std::string* turbo_client_id_;
