    ...
    if (ShouldCreateTileAt(info)) {
      ...
      if (tile && invalidation && TilingMatchesTileIndices(active_twin)) {
        if (const Tile* old_tile =
          ...
          bool frequently_invalidated =
              OnTileInvalidated(key.index_x, key.index_y);
...
bool PictureLayerTiling::OnTileInvalidated(int i, int j) {
  const int kMinInvalidationCount = 2;
  const base::TimeDelta kMinInterval = base::TimeDelta::FromMilliseconds(700);

  auto& iter = tile_invalidation_states_[TileMapKey(i, j)];
  if (iter.count >= kMinInvalidationCount)
    return true;

  base::TimeTicks now = base::TimeTicks::Now();
  base::TimeDelta time_since = now - iter.time;
  iter.time = now;
  iter.count = time_since < kMinInterval ? iter.count + 1 : 0;
  return iter.count >= kMinInvalidationCount;
}
  ...
  for (const auto& pair : remove_tiles) {
    ...
    if (recreate_tiles && old_tile) {
      ...
      if (Tile* tile = CreateTile(info))
        tile->SetInvalidated(invalid_content_rect, old_tile->id(), false);
    }
  }
}
...
ScopedTilePtr PictureLayerTiling::TakeTileAt(int i, int j) {
  ...
  tiles_.erase(found);
  auto iter = tile_invalidation_states_.find(TileMapKey(i, j));
  if (iter != tile_invalidation_states_.end())
    tile_invalidation_states_.erase(iter);
  return result;
}
