...
#include "cc/base/tile_compression_heuristic.h"
...
TileManager::TileManager(TileManagerClient* client,
                         base::SequencedTaskRunner* task_runner,
                         int compression_heuristic_flags,
    ...
    : client_(client),
      ...
      compression_heuristic_flags_(compression_heuristic_flags),
  ...
  for (; !raster_priority_queue->IsEmpty(); raster_priority_queue->Pop()) {
    ...
    if (!tile->is_solid_color_analysis_performed() &&
      ...
      int flags = tile->use_picture_analysis()
                      ...
                      : 0;
      // Analyze for gradient paints and big lossless bitmaps as well. It's to
      // decide to turn on or off the compression for the tile texture.
      bool analyze_lossless_bitmap = !tile->frequently_invalidated() &&
                                     (compression_heuristic_flags_ &
                                      TILE_COMPRESSION_HEURISTIC_IMAGE) != 0;
      bool analyze_gradient = !tile->frequently_invalidated() &&
                              (compression_heuristic_flags_ &
                               TILE_COMPRESSION_HEURISTIC_GRADIENT) != 0;
      if (analyze_lossless_bitmap)
        flags |= skia::AnalysisCanvas::ANALYZE_LOSSLESS_BITMAP;
      if (analyze_gradient)
        flags |= skia::AnalysisCanvas::ANALYZE_GRADIENT;
      RasterSource::AnalysisResult analysis;
      prioritized_tile.raster_source()->PerformSolidColorAnalysis(
          tile->content_rect(), tile->contents_scale(), flags, &analysis);
      if (analysis.is_solid_color) {
        tile->draw_info().set_solid_color(analysis.solid_color);
      ...
      // Turn compression on or off for this tile depending on analysis result.
      bool attempt_compress =
          (!analyze_lossless_bitmap || !analysis.has_lossless_bitmap) &&
          (!analyze_gradient || !analysis.has_gradient);
      tile->draw_info().set_attempt_compress(attempt_compress);
    }
...
void TileManager::GetMemoryStats(size_t* memory_allocated_bytes,
                                 size_t* memory_used_bytes) const {
  *memory_allocated_bytes = resource_pool_->GetTotalMemoryUsageForTesting();
  *memory_used_bytes = resource_pool_->memory_usage_bytes();
}
...
ResourceFormat TileManager::DetermineResourceFormat(const Tile* tile) const {
  bool no_compression = tile->frequently_invalidated() ||
                        !tile->draw_info().attempt_compress();
  return raster_buffer_provider_->GetResourceFormat(
      !tile->is_opaque(), no_compression);
}

bool TileManager::DetermineResourceRequiresSwizzle(const Tile* tile) const {
  bool no_compression = tile->frequently_invalidated() ||
                        !tile->draw_info().attempt_compress();
  return raster_buffer_provider_->IsResourceSwizzleRequired(
      !tile->is_opaque(), no_compression);
}
