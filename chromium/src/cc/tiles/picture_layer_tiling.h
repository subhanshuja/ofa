...
class CC_EXPORT PictureLayerTiling {
 ...
 private:
  struct CC_EXPORT TileInvalidationState {
    TileInvalidationState() : count(0) {}

    base::TimeTicks time;
    int count;
  };

  using TileInvalidationStateMap =
      std::unordered_map<TileMapKey, TileInvalidationState, TileMapKeyHash>;

  bool OnTileInvalidated(int i, int j);
