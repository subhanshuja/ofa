...
void ThreadedChannel::DidPinchGestureOnMainThread(const gfx::Point& anchor) {
  DCHECK(IsImplThread());
  MainThreadTaskRunner()->PostTask(
      FROM_HERE, base::Bind(&ProxyMain::DidPinchGestureOnMainThread,
                            impl().proxy_main_weak_ptr, anchor));
}
