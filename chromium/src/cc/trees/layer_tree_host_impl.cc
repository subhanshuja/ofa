...
LayerTreeHostImpl::LayerTreeHostImpl(
    ...
    : client_(client),
      ...
      tile_manager_(this,
                    GetTaskRunner(),
                    settings.renderer_settings.tile_compression_heuristic_flags,
...
bool LayerTreeHostImpl::PrepareTiles() {
  ...
  if (debug_state_.log_tile_memory_usage) {
    base::TimeDelta time_since_last_log =
        current_begin_frame_tracker_.Current().frame_time -
        last_manage_tiles_timeticks_;
    const base::TimeDelta kInterval = base::TimeDelta::FromMilliseconds(1000);
    if (time_since_last_log > kInterval) {
      size_t memory_allocated_bytes;
      size_t memory_used_bytes;
      tile_manager_.GetMemoryStats(&memory_allocated_bytes,
                                   &memory_used_bytes);
      LOG(INFO) << "TileMemoryUsage: " << memory_allocated_bytes;
      last_manage_tiles_timeticks_ =
          current_begin_frame_tracker_.Current().frame_time;
    }
  }
...
bool LayerTreeHostImpl::DrawLayers(FrameData* frame) {
  ...
  fps_counter_->SaveTimeStamp(CurrentBeginFrameArgs().frame_time,
                              !compositor_frame_sink_->context_provider());
  if (debug_state_.log_fps_counter) {
    fps_counter_->Log(CurrentBeginFrameArgs().frame_time);
  }
...
void LayerTreeHostImpl::CreateResourceAndRasterBufferProvider(
  ...
  if (use_zero_copy) {
    ...
    *raster_buffer_provider = ZeroCopyRasterBufferProvider::Create(
        resource_provider_.get(),
        settings_.renderer_settings.preferred_tile_format,
        settings_.renderer_settings.preferred_tile_format_with_alpha);
    return;
  }
...
void LayerTreeHostImpl::PinchGestureEnd() {
  ...
  gfx::PointF anchor = gfx::ScalePoint(
      gfx::PointF(previous_pinch_anchor_),
      1.f / active_tree_->current_page_scale_factor());
  anchor += ScrollOffsetToVector2dF(active_tree_->TotalScrollOffset());
  client_->DidPinchGesture(gfx::Point(anchor.x(), anchor.y()));
