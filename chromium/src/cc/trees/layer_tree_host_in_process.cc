...
LayerTreeHostInProcess::LayerTreeHostInProcess(
    ...
    : micro_benchmark_controller_(this),
      ...
      text_wrap_enabled_(false),
      text_wrap_pending_(false),
      visible_(false),
...
void LayerTreeHostInProcess::WillBeginMainFrame() {
  ...
  if (text_wrap_pending_ && !layer_tree_->HasPendingPageScaleAnimation()) {
    DCHECK(text_wrap_enabled_);
    // If there is a pending page scale animation, ignore current
    // text wrap. Instead wait for the pending page scale animation
    // to finish and then do text wrap.
    client_->StartTextWrap();
  }
...
void LayerTreeHostInProcess::SetEnableTextWrap(bool enable_text_wrap) {
  text_wrap_enabled_ = enable_text_wrap;
  SetNeedsCommit();
}

void LayerTreeHostInProcess::DidPinchGesture(const gfx::Point& anchor) {
  if (text_wrap_enabled_) {
    client_->SetTextWrapInterestPoint(anchor);
    text_wrap_pending_ = true;
    SetNeedsCommit();
  }
}
