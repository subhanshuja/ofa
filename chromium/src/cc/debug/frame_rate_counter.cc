...
void FrameRateCounter::Log(base::TimeTicks timestamp) {
  base::TimeDelta time_since_last_log = timestamp - last_log_timestamp;
  ...
  if (time_since_last_log > kInterval) {
    ...
    GetMinAndMaxFPS(&min_fps, &max_fps);
    LOG(INFO) << "FPS: " << current_frame_number() << ", "
              << dropped_frame_count() << ", " << GetAverageFPS() << ", "
              << min_fps << ", " << max_fps;
