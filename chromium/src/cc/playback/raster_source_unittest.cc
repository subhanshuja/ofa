...
TEST(RasterSourceTest, AnalyzeIsSolidUnscaled) {
  ...
  for (int y = 0; y <= 300; y += 100) {
    for (int x = 0; x <= 300; x += 100) {
      DisplayListRasterSource::AnalysisResult analysis;
      gfx::Rect rect(x, y, 100, 100);
      raster->PerformSolidColorAnalysis(rect, 1.0, flags, &analysis);
  ...
  DisplayListRasterSource::AnalysisResult analysis;
  raster->PerformSolidColorAnalysis(gfx::Rect(0, 0, 100, 100), 1.0, flags,
                                    &analysis);
  EXPECT_FALSE(analysis.is_solid_color);

  analysis = DisplayListRasterSource::AnalysisResult();
  raster->PerformSolidColorAnalysis(gfx::Rect(100, 0, 100, 100), 1.0, flags,
                                    &analysis);
  ...
  analysis = DisplayListRasterSource::AnalysisResult();
  raster->PerformSolidColorAnalysis(gfx::Rect(350, 0, 100, 100), 1.0, flags,
                                    &analysis);
  EXPECT_TRUE(analysis.is_solid_color);
  EXPECT_EQ(solid_color, analysis.solid_color);

  analysis = DisplayListRasterSource::AnalysisResult();
  raster->PerformSolidColorAnalysis(gfx::Rect(0, 350, 100, 100), 1.0, flags,
                                    &analysis);
  EXPECT_TRUE(analysis.is_solid_color);
  EXPECT_EQ(solid_color, analysis.solid_color);

  analysis = DisplayListRasterSource::AnalysisResult();
  raster->PerformSolidColorAnalysis(gfx::Rect(350, 350, 100, 100), 1.0, flags,
                                    &analysis);
  EXPECT_TRUE(analysis.is_solid_color);
  EXPECT_EQ(solid_color, analysis.solid_color);
}
...
TEST(RasterSourceTest, AnalyzeIsSolidScaled) {
  ...
  for (int y = 0; y <= 30; y += 10) {
    for (int x = 0; x <= 30; x += 10) {
      ...
      raster->PerformSolidColorAnalysis(rect, 0.1f, flags, &analysis);
      EXPECT_TRUE(analysis.is_solid_color) << rect.ToString();
  ...
  DisplayListRasterSource::AnalysisResult analysis;
  raster->PerformSolidColorAnalysis(gfx::Rect(0, 0, 10, 10), 0.1f, flags,
                                    &analysis);
  EXPECT_FALSE(analysis.is_solid_color);

  analysis = DisplayListRasterSource::AnalysisResult();
  raster->PerformSolidColorAnalysis(gfx::Rect(10, 0, 10, 10), 0.1f, flags,
                                    &analysis);
  EXPECT_TRUE(analysis.is_solid_color);
  ...
  // Boundaries should be clipped.
  analysis = DisplayListRasterSource::AnalysisResult();
  raster->PerformSolidColorAnalysis(gfx::Rect(35, 0, 10, 10), 0.1f, flags,
                                    &analysis);
  EXPECT_TRUE(analysis.is_solid_color);
  EXPECT_EQ(analysis.solid_color, solid_color);

  analysis = DisplayListRasterSource::AnalysisResult();
  raster->PerformSolidColorAnalysis(gfx::Rect(0, 35, 10, 10), 0.1f, flags,
                                   &analysis);
  EXPECT_TRUE(analysis.is_solid_color);
  EXPECT_EQ(analysis.solid_color, solid_color);

  analysis = DisplayListRasterSource::AnalysisResult();
  raster->PerformSolidColorAnalysis(gfx::Rect(35, 35, 10, 10), 0.1f, flags,
                                    &analysis);
  EXPECT_TRUE(analysis.is_solid_color);
  EXPECT_EQ(analysis.solid_color, solid_color);
}
...
TEST(RasterSourceTest, AnalyzeIsSolidEmpty) {
  ...
  raster->PerformSolidColorAnalysis(gfx::Rect(0, 0, 400, 400), 1.f, flags,
                                    &analysis);
