...
void RasterSource::PerformSolidColorAnalysis(const gfx::Rect& content_rect,
                                             float contents_scale,
                                             int flags,
  ...
  skia::AnalysisCanvas canvas(layer_rect.width(), layer_rect.height(), flags);
  canvas.translate(-layer_rect.x(), -layer_rect.y());
  RasterCommon(&canvas, &canvas);
  result->is_solid_color = canvas.GetColorIfSolid(&result->solid_color);
  result->has_lossless_bitmap = canvas.HasLargeLosslessBitmap();
  result->has_gradient = canvas.HasGradient();
}
