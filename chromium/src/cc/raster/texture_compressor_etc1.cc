...
void ExtractMisalignedBlock(uint8_t* dst,
  ...
  int aligned_width = (width + 3) & ~3;
  for (int y = num_rows; y < 4; ++y) {
    uint32_t* padding = wdst - aligned_width;
    for (int x = 0; x < 4; ++x)
      *wdst++ = *padding++;
  }
}
    ...
    for (int x = 0; x < width; x += 4, dst += 8) {
      ...
      if (num_rows + num_columns != 8) {
        ExtractMisalignedBlock(block_buffer, src + x * 4, num_columns, num_rows,
                               width);
        row0 = reinterpret_cast<const Color*>(block_buffer);
        stride = 4;
      } else {
        row0 = reinterpret_cast<const Color*>(src + x * 4);
        stride = width;
      }
      const Color* row1 = row0 + stride;
      const Color* row2 = row1 + stride;
      const Color* row3 = row2 + stride;
