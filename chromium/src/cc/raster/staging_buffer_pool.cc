...
std::unique_ptr<StagingBuffer> StagingBufferPool::AcquireStagingBuffer(
  ...
  // MCHR-48: Also check the resource format before trying to reuse existing
  // staging buffer. If the format has changed create a new buffer instead of
  // using the old one. Format change can happen when switching texture
  // compression on and off for individual tiles.
