...
// Set the heuristic to use during tile analysis to determine if a tile is not
// suitable for compression. Defaults to tiles with gradients and images.
const char kTileCompressionHeuristic[] = "tile-compression-heuristic";
...
// Log frame rate information once per second, without turning on continuous
// rendering. The format is:
// FPS: [current_frame] [dropped_frames] [average_fps] [min_fps] [max_fps]
const char kLogFPSCounter[] = "log-fps-counter";

// Log how much memory the tile manager is currently using. The format is:
// TileMemoryUsage: [memory_allocated_bytes]
const char kLogTileMemoryUsage[] = "log-tile-memory-usage";
