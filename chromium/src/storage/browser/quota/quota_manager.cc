...
bool ClearHostsQuotasOnDBThread(QuotaDatabase* database) {
  DCHECK(database);
  return database->ClearHostsQuotas();
}
...
void QuotaManager::ClearHostsQuotas(const ClearHostsQuotasCallback& callback) {
  PostTaskAndReplyWithResultForDBThread(
      FROM_HERE, base::Bind(&ClearHostsQuotasOnDBThread), callback);
}
