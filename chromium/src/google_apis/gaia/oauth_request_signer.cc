...
#include "url/gurl.h"
// Put here to avoid conflict between patches
...
const char kOperaTimeSkewLabel[] = "opera_time_skew";
#endif
...
#if defined(OPERA_SYNC)
  int64_t time_skew = 0;
  auto time_skew_parameter = parameters->find(kOperaTimeSkewLabel);
  if (time_skew_parameter != parameters->end()) {
    CHECK(base::StringToInt64(time_skew_parameter->second, &time_skew));
    auto timestamp_parameter = parameters->find(kOAuthTimestampLabel);
    int64_t timestamp;
    CHECK(base::StringToInt64(timestamp_parameter->second, &timestamp));
    timestamp_parameter->second = base::Int64ToString(timestamp + time_skew);
    parameters->erase(time_skew_parameter);
  }
#endif
...
#if defined(OPERA_SYNC)
// Removes parameters from |url_with_query| and appends them to |out_parameters|
// instead. Returns |url_with_query| without query parameters.
GURL ExtractParametersFromUrlQuery(
    const GURL& url_with_query,
    OAuthRequestSigner::Parameters* out_parameters) {
  // Split the query into key-value pairs.
  std::vector<std::string> key_value_pairs =
      base::SplitString(url_with_query.query(), "&", base::KEEP_WHITESPACE,
  ...
  for (const std::string& pair : key_value_pairs) {
    ...
    std::vector<std::string> key_and_value =
        base::SplitString(pair, "=", base::KEEP_WHITESPACE,
                          base::SPLIT_WANT_NONEMPTY);
    CHECK_EQ(2u, key_and_value.size());
    // Note that at this point both key and value are URL-encoded. We need to
    // unencode them (and later re-encode with oauth-specific encoding, see
    // BuildBaseStringParameters).
    const std::string key = net::UnescapeURLComponent(
        key_and_value[0], net::UnescapeRule::PATH_SEPARATORS |
        net::UnescapeRule::URL_SPECIAL_CHARS_EXCEPT_PATH_SEPARATORS);
    const std::string value = net::UnescapeURLComponent(
        key_and_value[1], net::UnescapeRule::PATH_SEPARATORS |
        net::UnescapeRule::URL_SPECIAL_CHARS_EXCEPT_PATH_SEPARATORS);
    // Append to the output. Note that RFC 5849 spec allows multiple parameters
    // with the same key (in which case they should be sorted by values).
    // We simplify by assuming keys found in the URL query don't overlap with
    // keys already found in |out_parameters|.
    (*out_parameters)[key] = value;
  }
  // The URL used for signing the header shouldn't have the query parameters.
  url::Replacements<char> clear_query;
  clear_query.ClearQuery();
  return url_with_query.ReplaceComponents(clear_query);
}
#endif  // OPERA_SYNC
...
bool OAuthRequestSigner::SignAuthHeader(
  ...
  const GURL normalized_url =
      ExtractParametersFromUrlQuery(request_base_url, &parameters);
#else  // Behave as original Chromium code
  const GURL normalized_url = request_base_url;
#endif  // OPERA_SYNC

  bool is_signed = SignParameters(normalized_url, signature_method, http_method,
                                  consumer_key, consumer_secret, token_key,
                                  token_secret, &parameters);
  if (is_signed) {
