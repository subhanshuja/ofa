...
// @copied-from chromium/src/chrome/browser/banners/app_banner_data_fetcher.cc
// @final-synchronized
...
bool AppBannerDataFetcher::OnMessageReceived(
  ...
  IPC_BEGIN_MESSAGE_MAP_WITH_PARAM(AppBannerDataFetcher, message,
                                   render_frame_host)
    IPC_MESSAGE_HANDLER(OperaViewHostMsg_AppBannerPromptReply,
                        OnBannerPromptReply)
    IPC_MESSAGE_HANDLER(OperaViewHostMsg_RequestShowAppBanner,
...
void AppBannerDataFetcher::OnBannerPromptReply(
  ...
  bool is_showing_first_time =
      !AppBannerSettingsHelper::HasShownBanner(web_contents,
                                               validated_url_,
                                               GetAppIdentifier());

  if (weak_delegate_) {
    RecordDidShowBanner();
    weak_delegate_->ShowBanner(app_icon_.get(), is_showing_first_time);
  }
...
void AppBannerDataFetcher::RecordDidShowBanner() {
  content::WebContents* web_contents = GetWebContents();
  DCHECK(web_contents);
...
void AppBannerDataFetcher::OnDidCheckHasServiceWorker(bool has_service_worker) {
  content::WebContents* web_contents = GetWebContents();
...
void AppBannerDataFetcher::OnAppIconFetched(const SkBitmap& bitmap) {
  ...
  web_contents->GetMainFrame()->Send(
      new OperaViewMsg_AppBannerPromptRequest(
        web_contents->GetMainFrame()->GetRoutingID(),
        event_request_id_,
        GetBannerType()));
}

bool AppBannerDataFetcher::IsWebAppInstalledOrBlocked() {
  content::WebContents* web_contents = GetWebContents();
  DCHECK(web_contents);

  return AppBannerSettingsHelper::IsAddedOrBlocked(
      web_contents, validated_url_, GetAppIdentifier());
}
