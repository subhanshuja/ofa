...
// @copied-from chromium/src/browser/push_messaging/push_messaging_app_identifier.cc
// @final-synchronized
...
base::DictionaryValue* GetMap() {
  opera::OperaContentBrowserClient* client =
      opera::OperaContentBrowserClient::Get();
  return client->push_messaging_storage().GetAppIdMap();
}

void SaveMap() {
  opera::OperaContentBrowserClient* client =
      opera::OperaContentBrowserClient::Get();
  client->push_messaging_storage().Save();
}
...
PushMessagingAppIdentifier PushMessagingAppIdentifier::FindByAppId(
  ...
  const base::DictionaryValue* map = GetMap();
...
void PushMessagingAppIdentifier::PersistToPrefs(
  ...
  base::DictionaryValue* map = GetMap();
