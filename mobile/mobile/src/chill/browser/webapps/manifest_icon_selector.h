...
// @copied-from chrome/browser/manifest/manifest_icon_selector.h
// @final-synchronized

#ifndef CHILL_BROWSER_WEBAPPS_MANIFEST_ICON_SELECTOR_H_
#define CHILL_BROWSER_WEBAPPS_MANIFEST_ICON_SELECTOR_H_
...
class ManifestIconSelector {
 ...
 private:
  ...
  // Ignore density values and try to find size that is the closest to the ideal
  // size but not smaller than minimum size.
  int FindBestMatchingIconForSize(
      const std::vector<content::Manifest::Icon>& icons);
...
#endif  // CHILL_BROWSER_WEBAPPS_MANIFEST_ICON_SELECTOR_H_
