...
// @copied-from chromium/src/components/omnibox/browser/autocomplete_input.cc
// @final-synchronized 475c9e32d72fcb59ef281944324619d784479409

#include "common/suggestions/op_autocomplete_input.h"
...
AutocompleteInput::Type GetInputTypeForScheme(const std::string& scheme) {
  if (base::IsStringASCII(scheme) &&
      (base::LowerCaseEqualsASCII(scheme, content::kLegacyOperaUIScheme) ||
       base::LowerCaseEqualsASCII(scheme, opera::kOperaAboutScheme) ||
       base::LowerCaseEqualsASCII(scheme, opera::kOperaUIScheme) ||
       base::LowerCaseEqualsASCII(scheme, url::kJavaScriptScheme) ||
       base::LowerCaseEqualsASCII(scheme, url::kDataScheme) ||
       base::LowerCaseEqualsASCII(scheme, url::kFtpScheme))) {
    return AutocompleteInput::URL;
  }
  return AutocompleteInput::UNKNOWN;
}

}  // namespace

// static
AutocompleteInput::Type AutocompleteInput::Parse(const base::string16& text,
  ...
  *canonicalized_url =
  ...
  if (parts->scheme.is_nonempty() &&
    ...
    Type type = GetInputTypeForScheme(parsed_scheme_utf8);
    if (type != UNKNOWN)
      return type;
...
// static
AutocompleteInput::Type AutocompleteInput::ParseAutocompleteInput(
    const base::string16& text) {
  return AutocompleteInput::Parse(text, "", nullptr, nullptr, nullptr);
}
