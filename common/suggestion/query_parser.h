...
// @copied-from chromium/src/components/query_parser/query_parser.h
// @last-synchronized 677ac36197869043e6567699f79e0365f9795509

#ifndef COMMON_SUGGESTION_QUERY_PARSER_H_
#define COMMON_SUGGESTION_QUERY_PARSER_H_
...
class QueryNode {
 public:
  ...
  virtual bool HasMatchIn(const QueryWordVector& words,
                          SuggestionMatchPositions* match_positions) const = 0;
...
class QueryParser {
 public:
  ...
  // Note: for queries containing more then one word, the only rejected words
  // are the ones that are one character long, and the character does not
  // encode a code point.
  static bool IsWordLongEnoughForPrefixSearch(
      const base::string16& word,
      MatchingAlgorithm matching_algorithm,
  ...
  static void SortAndCoalesceMatchPositions(SuggestionMatchPositions* matches);
...
#endif  // COMMON_SUGGESTION_QUERY_PARSER_H_
