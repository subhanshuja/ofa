...
std::unique_ptr<opera::TimeSkewResolver> CreateTimeSkewResolver(
    opera::TimeSkewResolverFactory time_skew_resolver_factory) {
  if (time_skew_resolver_factory.is_null())
    return nullptr;
  return time_skew_resolver_factory.Run();
}
#endif  // OPERA_SYNC
  ...
  if (!GetSyncApiComponentFactory()) {
    ...
    component_factory_.reset(new ProfileSyncComponentsFactoryImpl(
        ...
        *base::CommandLine::ForCurrentProcess(),
        ...
        url_request_context_getter, web_data_service_,
        password_store_));
  }
}
...
autofill::PersonalDataManager* ChromeSyncClient::GetPersonalDataManager() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  // TODO(mzajaczkowski): Should this even be called on ChromeSyncClient
  // if we don't have the autofill datatype enabled?
#if defined(OPERA_SYNC)
  NOTREACHED();
  return nullptr;
#else
...
ChromeSyncClient::GetBookmarkDuplicateTracker() {
  return opera::DuplicateTrackerFactory::GetForProfile(profile_);
}

opera::SyncAccount* ChromeSyncClient::CreateSyncAccount(
  ...
  DCHECK(!account_.get());
  opera::ExternalCredentials credentials;
  account_ = opera::SyncAccount::Create(
      profile_->GetRequestContext(), base::Bind(&CreateAccountService),
      CreateTimeSkewResolver(time_skew_resolver_factory), std::move(login_data_store),
      auth_data_updater_factory, stop_syncing_callback, credentials);
  return GetSyncAccount();
}

opera::SyncAccount* ChromeSyncClient::GetSyncAccount() {
  DCHECK(account_.get());
  return account_.get();
}

opera::SyncErrorHandler* ChromeSyncClient::CreateSyncErrorHandler(
  ...
  return new opera::SyncErrorHandler(
      profile_, show_status_delay, std::move(delay_provider), show_status_callback,
      show_login_prompt_callback);
}

opera::oauth2::AuthService* ChromeSyncClient::GetOperaOAuth2Service() {
  return opera::oauth2::AuthServiceFactory::GetForProfile(profile_);
}

opera::sync::SyncAuthKeeper* ChromeSyncClient::GetOperaSyncAuthKeeper() {
  return opera::sync::SyncAuthKeeperFactory::GetForProfile(profile_);
}

void ChromeSyncClient::OnSyncAuthKeeperInitialized(
    opera::sync::SyncAuthKeeper* keeper) {
  DCHECK_EQ(keeper, GetOperaSyncAuthKeeper());
  GetSyncService()->OnSyncAuthKeeperInitialized();
}

#if defined(OPERA_DESKTOP)
LoginUIService::LoginUI* ChromeSyncClient::GetLoginUI() {
  return LoginUIServiceFactory::GetForProfile(profile_)->current_login_ui();
}

opera::OtherSpeedDialsCleaner* ChromeSyncClient::GetOtherSpeeddialsCleaner() {
  if (!other_speed_dials_cleaner_.get()) {
    opera::AccountService* account_service = nullptr;
    if (!opera::SyncConfig::UsesOAuth2()) {
      DCHECK(account_.get());
      account_service = account_->service().get();
    }
    other_speed_dials_cleaner_ = opera::OtherSpeedDialsCleanerFactory::Create(
        account_service, profile_);
  }
  return other_speed_dials_cleaner_.get();
}

scoped_refptr<opera::SyncPasswordRecoverer>
ChromeSyncClient::CreateOperaSyncPasswordRecoverer() {
  return make_scoped_refptr(new opera::SyncPasswordRecoverer(profile_));
}
#endif  // OPERA_DESKTOP
...
ChromeSyncClient::GetSyncableServiceForType(syncer::ModelType type) {
  ...
  switch (type) {
    ...
    case syncer::PREFERENCES:
      ...
      return base::WeakPtr<syncer::SyncableService>();
#endif  // OPERA_DESKTOP
#ifdef SYNC_TYPES_NOT_ENABLED_IN_OPERA
...
#endif
#endif  // SYNC_TYPES_NOT_ENABLED_IN_OPERA
    ...
    case syncer::SESSIONS: {
      DCHECK(opera::SyncConfig::ShouldSynchronizeTabs());
      return ProfileSyncServiceFactory::GetForProfile(profile_)->
    ...
    case syncer::PASSWORDS: {
      ...
      return password_store_.get()
                 ...
                 : base::WeakPtr<syncer::SyncableService>();
#else
      // Mobile builds fail on GetPasswordSyncableService().
      return base::WeakPtr<syncer::SyncableService>();
#endif  // OPERA_DESKTOP
    }
#if defined(OS_CHROMEOS)
    ...
    default:
      ...
      // Bookmarks
      NOTREACHED() << syncer::ModelTypeToString(type);
      return base::WeakPtr<syncer::SyncableService>();
  }
}
