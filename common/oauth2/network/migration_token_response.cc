...
MigrationTokenResponse::MigrationTokenResponse() : auth_error_(OAE_UNSET) {}

MigrationTokenResponse::~MigrationTokenResponse() {}

NetworkResponseStatus MigrationTokenResponse::TryResponse(
    int http_code,
...
NetworkResponseStatus MigrationTokenResponse::ParseForHTTP200(
    const std::string& response_data) {
  ...
  // Should be there but is ignored.
  ...
  parser.Expect(kUserId, ResponseParser::STRING, ResponseParser::IS_REQUIRED);
...
NetworkResponseStatus MigrationTokenResponse::ParseForHTTP400(
    const std::string& response_data) {
  std::unique_ptr<base::DictionaryValue> dict(
      base::DictionaryValue::From(base::JSONReader::Read(response_data)));
  if (!dict.get())
...
NetworkResponseStatus MigrationTokenResponse::ParseForHTTP401(
    const std::string& response_data) {
  std::unique_ptr<base::DictionaryValue> dict(
      base::DictionaryValue::From(base::JSONReader::Read(response_data)));
  if (!dict.get())
