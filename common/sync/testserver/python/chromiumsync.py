...
ALL_TYPES = (TOP_LEVEL,  # The type of the 'Google Chrome' folder.
             APPS,
             APP_LIST,
             APP_NOTIFICATION,
             APP_SETTINGS,
...
SYNC_ERROR_FREQUENCY = (ERROR_FREQUENCY_NONE, ERROR_FREQUENCY_ALWAYS,
...
def ExtractEntryData(entry):
  ...
  for descriptor, v in fields:
    if descriptor.type in mapping:
      mapped_type = mapping[descriptor.type]
    elif descriptor.type == FD.TYPE_MESSAGE:
      mapped_type = ExtractEntryData
    else:
      print "Field %s (%d) type unknown" % (descriptor.name, descriptor.type)
      continue
    if descriptor.label == FD.LABEL_REPEATED:
      field_data = []
      for i in v:
        field_data.append(mapped_type(i))
    else:
      field_data = mapped_type(v)
    item_data[descriptor.name] = field_data
...
def MakeNewKeystoreKey():
  """Returns a new constant keystore key."""
  return "dontchangeit"
...
def GenerateOperaAuthToken(token_length):
  import random
  import string
  return ''.join(random.choice(string.ascii_uppercase + string.digits)
...
class UpdateSieve(object):
  ...
  def GetFirstTimeTypes(self):
    ...
    return [datatype
            for datatype, timestamp in self._state.iteritems()
            if timestamp == 0]
  ...
  def GetCreateMobileBookmarks(self):
    ...
    """
    return (
        self._original_request.HasField('create_mobile_bookmarks_folder') and
        self._original_request.create_mobile_bookmarks_folder)
...
class SyncDataModel(object):
  ...
  _PERMANENT_ITEM_SPECS = [
      PermanentItem('google_chrome_apps',
                    name='Apps',
                    parent_tag=ROOT_ID,
                    sync_type=APPS),
      PermanentItem('google_chrome_app_list',
                    name='App List',
                    parent_tag=ROOT_ID,
                    sync_type=APP_LIST),
      PermanentItem('google_chrome_app_notifications',
                    name='App Notifications',
                    parent_tag=ROOT_ID,
                    sync_type=APP_NOTIFICATION),
      PermanentItem('google_chrome_app_settings',
                    name='App Settings',
                    parent_tag=ROOT_ID,
                    sync_type=APP_SETTINGS),
      PermanentItem('google_chrome_bookmarks',
                    name='Bookmarks',
                    parent_tag=ROOT_ID,
                    sync_type=BOOKMARK),
      PermanentItem('bookmark_bar',
                    name='Bookmark Bar',
                    parent_tag=ROOT_ID,
                    sync_type=BOOKMARK),
      PermanentItem('other_bookmarks',
                    name='Other Bookmarks',
                    parent_tag=ROOT_ID,
                    sync_type=BOOKMARK),
      PermanentItem('user_root_bookmarks',
                    name='User Bookmarks',
                    parent_tag=ROOT_ID,
                    sync_type=BOOKMARK),
      PermanentItem('unsorted_bookmarks',
                    name='Unsorted Bookmarks',
                    parent_tag=ROOT_ID,
                    sync_type=BOOKMARK),
      PermanentItem('shared_bookmarks',
                    name='Shared Bookmarks',
                    parent_tag=ROOT_ID,
                    sync_type=BOOKMARK),
      PermanentItem('synced_bookmarks',
                    name='Synced Bookmarks',
                    parent_tag=ROOT_ID,
                    sync_type=BOOKMARK,
                    create_by_default=False),
      PermanentItem('google_chrome_autofill',
                    name='Autofill',
                    parent_tag=ROOT_ID,
                    sync_type=AUTOFILL),
      PermanentItem('speeddial_bookmarks',
                    name='Speed Dial Bookmarks',
                    parent_tag=ROOT_ID,
                    sync_type=BOOKMARK),
      PermanentItem('trash_bookmarks',
      ...
      PermanentItem('google_chrome_autofill_profiles',
                    ...
                    parent_tag=ROOT_ID,
                    sync_type=AUTOFILL_PROFILE),
      PermanentItem('google_chrome_autofill_wallet_metadata',
                    name='Autofill Wallet Metadata',
                    parent_tag=ROOT_ID,
                    sync_type=AUTOFILL_WALLET_METADATA),
      PermanentItem('google_chrome_device_info',
                    name='Device Info',
                    parent_tag=ROOT_ID,
                    sync_type=DEVICE_INFO),
      PermanentItem('google_chrome_experiments',
                    name='Experiments',
                    parent_tag=ROOT_ID,
                    sync_type=EXPERIMENTS),
      PermanentItem('google_chrome_extension_settings',
                    name='Extension Settings',
                    parent_tag=ROOT_ID,
                    sync_type=EXTENSION_SETTINGS),
      PermanentItem('google_chrome_extensions',
                    name='Extensions',
                    parent_tag=ROOT_ID,
                    sync_type=EXTENSIONS),
      ...
      PermanentItem('google_chrome_passwords',
                    ...
                    parent_tag=ROOT_ID,
                    sync_type=PASSWORD),
      PermanentItem('google_chrome_preferences',
                    name='Preferences',
                    parent_tag=ROOT_ID,
                    sync_type=PREFERENCE),
      PermanentItem('google_chrome_priority_preferences',
                    name='Priority Preferences',
                    parent_tag=ROOT_ID,
                    sync_type=PRIORITY_PREFERENCE),
      PermanentItem('google_chrome_synced_notifications',
                    name='Synced Notifications',
                    parent_tag=ROOT_ID,
                    sync_type=SYNCED_NOTIFICATION),
      PermanentItem('google_chrome_synced_notification_app_info',
                    name='Synced Notification App Info',
                    parent_tag=ROOT_ID,
                    sync_type=SYNCED_NOTIFICATION_APP_INFO),
      PermanentItem('google_chrome_search_engines',
                    name='Search Engines',
                    parent_tag=ROOT_ID,
                    sync_type=SEARCH_ENGINE),
      PermanentItem('google_chrome_sessions',
                    name='Sessions',
                    parent_tag=ROOT_ID,
                    sync_type=SESSION),
      PermanentItem('google_chrome_themes',
                    name='Themes',
                    parent_tag=ROOT_ID,
                    sync_type=THEME),
      PermanentItem('google_chrome_typed_urls',
                    name='Typed URLs',
                    parent_tag=ROOT_ID,
                    sync_type=TYPED_URL),
      PermanentItem('google_chrome_wifi_credentials',
                    name='WiFi Credentials',
                    parent_tag=ROOT_ID,
                    sync_type=WIFI_CREDENTIAL),
      PermanentItem('google_chrome_dictionary',
                    name='Dictionary',
                    parent_tag=ROOT_ID,
                    sync_type=DICTIONARY),
      PermanentItem('google_chrome_articles',
                    name='Articles',
                    parent_tag=ROOT_ID,
                    sync_type=ARTICLE),
  ]
  ...
  def ResetAccount(self, birthday=None, version=None):
    print 'SYNC: Account has been reset (birthday = %s, data version = %s)' % \
        (birthday, version)
    ...
    self.ResetStoreBirthday(birthday)
  ...
  def _ClientIdToId(self, datatype, client_guid, client_item_id):
    ...
    return self._MakeCurrentId(datatype, '<server ID originally>%s/%s' %
                               (client_guid, client_item_id))

  def _MakeCurrentId(self, datatype, inner_id):
    return '%d^%d^%s' % (
        datatype, self.migration_history.GetLatestVersion(datatype), inner_id)
  ...
  def _CreatePermanentItem(self, spec):
    ...
    if self._ItemExists(id_string):
      return
    print 'SYNC: Creating permanent item: %s' % spec.name
  ...
  def ResetStoreBirthday(self, birthday=None):
    """Resets the store birthday to a random value."""
    # TODO(nick): uuid.uuid1() is better, but python 2.5 only.
    if birthday is None:
      birthday = '%0.30f' % random.random()
    self.store_birthday = birthday
    print "SYNC: Store birthday is now set to %s" % self.store_birthday
  ...
  def _CopyOverImmutableFields(self, entry):
    ...
    if entry.id_string in self._entries:
      if self._entries[entry.id_string].HasField('server_defined_unique_tag'):
  ...
  def ValidateCommitEntries(self, entries):
    ...
    self.migration_history.CheckAllCurrent(dict((k, min(
        v)) for k, v in versions_present.iteritems()))
  ...
  def CommitEntry(self, entry, cache_guid, commit_session):
    ...
    if entry.deleted:
      ...
      child_ids = [child.id_string
                   for child in self._entries.itervalues()
    ...
    if GetEntryType(entry) == NIGORI:
      self._DumpOperaNigoriDiagnostics(base_entry, entry)

    if GetEntryType(entry) == PASSWORD:
      self._DumpOperaPasswordDiagnostics(base_entry, entry)
  ...
  def _IndicateOperaKeyName(self, key_name):
    import re
    opera_key_pattern = "op_.+"
    if re.match(opera_key_pattern, key_name):
      return key_name
    return "!!%s" % key_name

  def _DumpOperaNigoriDiff(self, existing_dict, new_dict, level=1):
    for new_key in new_dict.keys():
      if isinstance(new_dict[new_key], dict):
        print "%sSUB '%s'" % ('\t' * level, self._IndicateOperaKeyName(new_key))
        self._DumpOperaNigoriDiff(
            existing_dict.get(new_key, None), new_dict[new_key], level + 1)
      else:
        if (existing_dict is None) or (not new_key in existing_dict.keys()):
          print "%sNEW '%s': '%s'" % ('\t' * level,
                                      self._IndicateOperaKeyName(new_key),
                                      new_dict[new_key])
        else:
          if not new_dict[new_key] == existing_dict[new_key]:
            print "%sCHANGED '%s': '%s' => '%s'" % (
                '\t' * level, self._IndicateOperaKeyName(new_key),
                existing_dict[new_key], new_dict[new_key])

  def _DumpOperaNigoriDiagnostics(self, existing_entry, new_entry):
    print "\n"
    print "SYNC: Client sent NIGORI update"
    existing_specifics = ExtractEntryData(existing_entry.specifics.nigori)
    new_specifics = ExtractEntryData(new_entry.specifics.nigori)
    self._DumpOperaNigoriDiff(existing_specifics, new_specifics)
    print "\n"

  def _DumpOperaPasswordDiagnostics(self, existing_entry, new_entry):
    msg = ""
    print "SYNC: PASSWORD commit: "
    if existing_entry is not None:
      existing_specifics = \
        ExtractEntryData(existing_entry.specifics.password.encrypted)
      msg = "key_name(%s)" % str(existing_specifics.get('key_name', '<EMPTY>'))
    else:
      msg = "(none)"
    ...
    if new_entry is not None:
      new_specifics = ExtractEntryData(new_entry.specifics.password.encrypted)
      msg = "%skey_name(%s)" % (msg,
                                str(new_specifics.get('key_name', '<EMPTY>')))
    else:
      msg = "%s%s" % (msg, "(none)")

    if new_entry.specifics.password.HasField('client_only_encrypted_data'):
      print "*" * 80
      print "!!! Client sent client_only_encrypted_data, server exits NOW !!!"
      print "*" * 80
      sys.exit(1)
  ...
  def TriggerEnableKeystoreEncryption(self):
    ...
    keystore_encryption_id = self._ClientTagToId(
        EXPERIMENTS, KEYSTORE_ENCRYPTION_EXPERIMENT_TAG)
  ...
  def TriggerAcknowledgeManagedUsers(self):
    ...
    managed_users = [copy.deepcopy(entry)
                     for entry in self._entries.values()
                     if entry.specifics.HasField('managed_user') and
                     not entry.specifics.managed_user.acknowledged]
  ...
  def TriggerEnablePreCommitGetUpdateAvoidance(self):
    ...
    pre_commit_gu_avoidance_id = self._ClientTagToId(
        EXPERIMENTS, PRE_COMMIT_GU_AVOIDANCE_EXPERIMENT_TAG)
  ...
  def SetInducedError(self, error, error_frequency, sync_count_before_errors):
    ...
    print "SYNC: Induced error set up to:\n\tError: %s\n\tFrequency: %s\n\t" \
    "Sync count before errors: %s" % (re.sub('\\n','; ',str(error)), \
    str(error_frequency), str(sync_count_before_errors))
...
class TestServer(object):
  ...
  def __init__(self):
    ...
    self.client_name_generator = ('+' * times + chr(c)
                                  for times in xrange(0, sys.maxint)
                                  for c in xrange(
                                      ord('A'), ord('Z')))
    self.transient_error = False
    self.one_transient_error = False
    self.sync_count = 0
    ...
    self._opera_flags = None
    self._logging_delegate = None

  def SetLoggingDelegate(self, logging_delegate):
    self._logging_delegate = logging_delegate

  def SetXMPPServer(self, xmpp_server):
    self._xmpp_server = xmpp_server

  def SetOperaAuthServer(self, opera_auth_server):
    self._opera_auth_server = opera_auth_server

  def SetOperaFlags(self, opera_flags):
    self._opera_flags = opera_flags

  def GetOperaAuthServer(self):
    return self._opera_auth_server
  ...
  def CheckStoreBirthday(self, request):
    ...
    if self.account.StoreBirthday() != request.store_birthday:
      print "Birtday mismatch! local = %s; client = %s" % \
          (self.account.StoreBirthday(), request.store_birthday)
  ...
  def SetOneTransietError(self):
    self.one_transient_error = True

  def HandleOneTransientError(self):
    if self.one_transient_error:
      self.one_transient_error = False
      raise TransientError
  ...
  def CheckSendError(self):
    ...
    if (self.account.induced_error.error_type !=
      ...
      # the last 2.
      elif (self.account.induced_error_frequency == ERROR_FREQUENCY_TWO_THIRDS):
        if (((self.sync_count - self.account.sync_count_before_errors) % 3) !=
            0):
          raise SyncInducedError
      else:
  ...
  def HandleSetInducedError(self, path):
    ...
    try:
      error_type = urlparse.parse_qs(query).get('error', None)
      action = urlparse.parse_qs(query).get('action', None)
      if not error_type or not action:
        code = 404
        response = """
  Usage:
      /chromiumsync/error?error=ErrorType&action=Action[&url=URL][&error_description=DESC][&frequency=FREQ]
  ...
  enum Action {
    ...
    """
        ...
        response = ('Error = %d, action = %d, url = %s, description = %s' %
                    (error.error_type, error.action, error.url,
                     error.error_description))
    except error:
      ...
      self.account_lock.release()
    return (code, '<html><title>SetError: %d</title><pre>%d %s</pre></html>' %
            (code, code, response))
  ...
  def HandleEnableKeystoreEncryption(self):
    ...
    return (200, '<html><title>Enable Keystore Encryption</title>'
  ...
  def HandleRotateKeystoreKeys(self):
    ...
    return (200, '<html><title>Rotate Keystore Keys</title>'
  ...
  def HandleEnableManagedUserAcknowledgement(self):
    ...
    return (200, '<html><title>Enable Managed User Acknowledgement</title>'
  ...
  def HandleEnablePreCommitGetUpdateAvoidance(self):
    ...
    return (200, '<html><title>Enable pre-commit GU avoidance</title>'
            '<H1>Enable pre-commit GU avoidance</H1></html>')

  def HandleEvent(self, query, raw_request):
    request = sync_pb2.EventRequest()
    request.MergeFromString(raw_request)

    if request.HasField('sync_disabled'):
      sync_disabled_event = request.sync_disabled

      params = {}
      params['guid'] = sync_disabled_event.cache_guid
      params['birthday'] = sync_disabled_event.store_birthday

      self.GetOperaAuthServer().HandleSyncEvent('sync_disabled_event_sent',
                                                params)
    else:
      print 'SYNC: Unknown event received!'
  ...
  def HandleCommand(self, query, raw_request):
    ...
    def print_context(direction):
      print 'SYNC: [Client %s %s %s.py]' % (self.GetShortClientName(query),
                                            direction, __name__),
    ...
    try:
      ...
      if self.account.StoreBirthday() is None:
        print "SYNC: Error: This should not happen"
        sys.exit(0)
  ...
  def HandleCommit(self, commit_message, commit_response):
    ...
    entry_count_per_type = {}

    for entry in commit_message.entries:
      etype = SyncTypeToString(GetEntryType(entry))
      count_dict = entry_count_per_type.get(etype, {'all': 0, 'deleted': 0})
      count_dict['all'] += 1
      if entry.deleted:
        count_dict['deleted'] += 1
      entry_count_per_type[etype] = count_dict
      ...
      if not server_entry:
        ...
        self._xmpp_server.RequestSendInvalidation(
            SyncTypeToString(GetEntryType(entry)), reply.version)

    if self._logging_delegate is not None:
      self._logging_delegate.on_sync_command({
          'type': 'COMMIT',
          'entry_count_per_type': entry_count_per_type,
  ...
  def HandleSetOauth2Token(self, response_code, request_token, access_token,
    ...
    return (200, '<html><title>Set OAuth2 Token</title>'
  ...
  def HandleShowData(self):
    import json
    all_items = []

    for e in self.account._entries:
      entry = self.account._entries[e]
      all_items.append(ExtractEntryData(entry))

    return (200, json.dumps(all_items))

  # Returns the current state of the local DB.
  # requested_format - a string denoting the requested return format. Allowed values: 'json'.
  def HandleGetData(self, requested_format):
    count = len(self.account._entries)
    print "SYNC: GET data request:\n\tFormat '%s'\n\tItems: %d\n\t"\
          "Birthday: '%s'\n\tData version: %d" % \
          (requested_format, count, self.account.StoreBirthday(), \
           self.account.CurrentVersion())
    if requested_format == 'json':
      ...
      dict_of_items = {}
      for entry_id in self.account._entries:
        entry = self.account._entries[entry_id]
        serialized = entry.SerializeToString()
        dict_of_items[entry_id] = serialized
      final_data = {}
      # The pbuf_version should be increased both here and in the HandlePutData
      # method in case any changes are introduced to the protobuf dump
      # format that make it incompatible.
      # This is to make it possible to differentiate the versions in PutData.
      final_data['pbuf_version'] = 1
      final_data['birthday'] = self.account.StoreBirthday()
      final_data['data_version'] = self.account.CurrentVersion()
      final_data['entries'] = dict_of_items
      return (200, pickle.dumps(final_data))
    else:
      return (404, "Unknown requested format '%s'" % requested_format)

  def HandlePutData(self, requested_format, raw_data):
    print "SYNC: Put data request in format '%s'" % requested_format
    new_dict = {}
    if requested_format == 'json':
      return (404, "Import from JSON not supported.")
    elif requested_format == 'protobuf':
      final_data = pickle.loads(raw_data)
      version = final_data.get('pbuf_version', None)
      # Note that we should handle any version if possible. We only have version
      # 1 at this moment.
      if version != 1:
        err_str = "The protobuf file was generated with an older server " \
            "version (%s). Please regenerate." % version
        print "SYNC: %s" % err_str
        return (404, err_str)

      birthday = final_data['birthday']
      data_version = final_data['data_version']
      dict_of_encoded = final_data['entries']
      for entry_id in dict_of_encoded.keys():
        serialized = dict_of_encoded[entry_id]
        entry = sync_pb2.SyncEntity()
        entry.ParseFromString(serialized)
        new_dict[entry_id] = entry
    else:
      return (404, "Unknown requested format '%s'" % requested_format)

    self.HandleResetData(birthday, data_version)
    self.account._entries = new_dict
    count = len(new_dict)
    status = "PUT data request:\n\tFormat '%s'\n\tItems: %d\n\t" \
        "Birthday: '%s'\n\tData version: %d" % \
        (requested_format, count, self.account.StoreBirthday(), \
         self.account.CurrentVersion())
    print "SYNC: %s" % status
  ...
  def HandleResetData(self, birthday=None, version=None):
    print "SYNC: ***** Account has been reset *****"
    self.account.ResetAccount(birthday, version)
    self.SendTrickInvalidation()
    return (200, "User data was reset, datastore birthday changed")

  def SendTrickInvalidation(self):
    # This method sends a 'fake' invalidation regarding the Nigori node.
    # This is used to notify the client that something happened on the
    # server, i.e. database was reset or the auth password was changed.
    # Client then tries to update its state against the server and
    # is notified about the problem.
    # This is the same thing we do on production.
    self._xmpp_server.RequestSendInvalidation(
        SyncTypeToString(DataTypeStringToSyncTypeLoose('nigori')), 0)
